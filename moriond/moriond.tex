...
our site at \url{http://moriond.in2p3.fr/2026/download/moriond.tar.gz}.}
\begin{description}
...
package (which is recommanded). 
%In this sample we have used \verb^\bibitem^ to produce the bibliography.
In this sample we have used \verb^moriond.bib^ to produce the bibliography.
...
All files (.tex, figures and .pdf) should be sent by the {\bf April 2026}
($12^{th}$ for EW and for VHEPU, $18^{th}$ for QCD and for Cosmology) by e-mail to 
{\bf moriond@in2p3.fr}.
...
is that of Bolotov {\it et al}.~\cite{bo}
        There are two
...
The reason is that such a matrix does not cover all of the parameter space available.
\begin{equation}
...
form just one true parametrization. Other useful informations can be 
found in references ~\cite{ma,bu,bd}.
\bea
