...
In the current \cosmoglobe\ DR2 data release, we apply the same process to the DIRBE data with the goal of characterizing the CIB monopole spectrum from 1 to 240$\,\mu\mathrm{m}$. Even after this work, several key problems remain that will require further investigation, both in terms of astrophysical and instrumental modeling. The following results therefore are not the definitive results that ultimately can be derived from DIRBE. Rather, they are the first step in a long process, in which the CIB is gradually being harnessed through the same Bayesian end-to-end methods that have proven effective for analyzing CMB experiments, with the ultimate goal of seamlessly merging the two fields. Two natural next steps in this process are, first, to include high-resolution experiments such as IRAS \citep{neugebauer:1984} and AKARI \citep{murakami:2007} that can break important degeneracies in the current zodiacal light model, and, second, to reprocess the \Planck\ HFI measurements at the level of raw time-ordered data (TOD); preliminary steps towards both of these projects have recently been made, and volunteers are welcome to join this Open Source effort.
...
contribution. In the current analysis, we adopt the following sky model: \aism{the equation needs to be updated (add Halpha correlated dust and I guess all dust are from templates?)}
\begin{alignat}{4}
...
all of which interact non-trivially. \aism{the dust are all templates, right? not a free amplitude per pixel?}
...
sample, and then searches for the local minimum. \aism{it's searching for the global minimum, right? otherwise i guess it should be "local minima"} This algorithm is
...
\paragraph{\Planck\ HFI:} We include one \Planck\ HFI PR4 \citep{npipe} detector sky map for each of the 100, 143, 217,
  ...
  release. \aism{should it be a hot dust template instead of cii?} This component has to our knowledge never been accounted
...
analysis. \aism{the numbers probably need to be updated? i thought there was also different burn-ins for different things} The total wall time was one month.
...
of Fig.~\ref{fig:sidelobe}. \aism{the figure was commented out} In this figure, the Sun is located in the
...
Fig.~\ref{fig:sidelobe}. \aism{the figure was commented out} Based on the updated analysis of
...
elongations. \aism{the figure was commented out}
...
channel is unchanged. \aism{the figure was commented out}
...
illustrated in the bottom panel of Fig.~\ref{fig:sidelobe}. \aism{the figure was commented out} In this
...
channel. \aism{the figure was commented out} In the final production analysis, this component is then kept
...
higher-level products. \aism{the figure was commented out} However, since the zero-level of this signal is
...
\begin{figure*}
	...
	\caption{
		...
		All upper limits are denoted by downward-facing triangles at the 95\,\% upper limit as calculated in Table \ref{tab:CIB_monopole}, while all error bars are 68\,\% confidence intervals. \aism{so does that mean we're claming all detections, no upper limits? that doesn't match with the text}
	}
...
first 20 samples in each chain as suggested by \citet{CG02_02}. \aism{the figure says 15 samples, not 20} The
...
uncertainty to this value, as described by \citet{CG02_01}.  For channels without a positive detection, we define the
