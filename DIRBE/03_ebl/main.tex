...
\begin{figure}
  ...
  \caption{(\emph{Top:}) Default excess radiation model, $\a_{\mathrm{static}}$, for the 25\,$\mu$m channel, plotted in solar-centric coordinates; see \citet{CG02_01} for full discussion. (\emph{Bottom:}) Same as above, but with an additional offset of +0.8\,MJy/sr, corresponding to the measured monopole at 25\,$\mu$m reported in this paper.}
  \label{fig:sidelobe}
\end{figure}
...
\citet{hauser1998}, the DIRBE optics was specifically designed to
...
Fig.~\ref{fig:sidelobe} ({\bf ADD LINES}).
...
opted to set the zero-level of the $\a_{\mathrm{static}}$ map at each
channel to the lowest possible value that still results in a positive
signal within instrumental noise fluctuations. The motivation for this
is one of simplicity of interpretation: Whether the excess signal is
...
case, we have added an extra offset of +0.8\,MJy/sr to
$\a_{\mathrm{static}}$ at $25\,\mu\mathrm{m}$; both versions will lead
to an identical overall $\chi^2$ after fitting the monopole $m_{\nu}$.
Both appear as physically plausible, but the bottom case will yield a
CIB monopole that is lower by 0.8\,MJy/sr; the reason for choosing
precisely this value will become evident in Sect.~\ref{sec:results}. Conversely, it is not possible to add a negative offset of
-0.8\,MJy/sr, since the template will then become significantly
negative beyond what is allowed by instrumental noise.

To summarize, we model this excess radiation in terms of a free
...
\endPlancktablewide
\tablenote {{\rm a}} CIO baseline (or offset) uncertainty as estimated by the DIRBE team; reproduced from Table~1 of \citet{hauser1998}.\par
...
\tablenote {{\rm f}} Total monopole uncertainty obtained by adding columns (a)--(e) in quadrature.\par
...
\tablenote {{\rm i}} \cosmoglobe\ DR2 CIB constraint as derived with the tuned CIB monopole estimator discussed in Sect.~\ref{sec:results}. This uses a more conservative mask than the Gibbs chain result in column (h), and we consider this as the final result.\par
\par
\end{table*}
...
key. Finally, even in the far-infrared wavelength regime are our
results significantly lower than the legacy results; for instance, at
240\,$\mu\mathrm{m}$ our monopole is 35,\% lower than the DIRBE
result. Furthermore, the shape of the legacy CIB monopole spectrum
strongly suggests a non-negligible zodiacal light component, even at
140 and 240$\,\mu\mathrm{m}$, for which the DIRBE team claimed strong
detections. Based on the results listed in
Table~\ref{tab:CIB_monopole}, it appears in retrospect that both those
estimates were biased high by zodiacal light residuals by about
$2\,\sigma$.
...
In this work, we have have analyzed the reprocessed DIRBE maps within the \cosmoglobe\ framework, improving the legacy monopole measurements at every wavelength. This was achieved through global end-to-end Bayesian sampling, in which the zodiacal emission and Galactic emission were sampled jointly. The reprocessed DIRBE maps, as presented in \citet{CG02_01}, have been improved from the legacy processing in large part due to improved zodiacal dust modeling, summarized in \citet{CG02_02}, as well as improved stellar modeling \citep{CG02_04} and thermal dust modeling \citep{CG02_05}. Notably, the presence of static excess radiation was identified and removed in the 4.9, 12, 25, 60, and $100\,\mathrm{\mu m}$ bands.


This work, using half-mission splits from the  main \cosmoglobe\ DR2 chain, estimated the monopoles both from the main \citet{CG2_01} analysis and by measuring the monopole within dedicated conservative regions with that are well-described by the full sky model, and have pixels that were observed in both half missions. In this dedicated analysis, we find monopole values that are consistent with those estimated as part of the full \cosmoglobe\ DR2 chain using a more modest mask. We take the  consistency between these two results, as well as our uncertainty estimates, as an indication of the robustness of our results.


As demonstrated here, a major limitation in constraining the CIB has been due to limited usage of complementary data. In this work, the reanalysis of the DIRBE CIO's was improved due partially due to larger compute resources relative to data volume, but primarily due to the use of a sky model spanning $100\,\mathrm{GHz}$ to $1\,\mathrm{\mu m}$. While of this comes from high-resolution data, such as \Planck\ HFI and \textit{Gaia}, which constrain the sky at wavelengths inaccessible to DIRBE, the use of data collected using diverse observation strategies, detector technologies, and taken at different times made it much simpler to identify features peculiar to one instrument rather than another. The confusion between zodiacal and Galactic dust in particular remains the single largest source of uncertainty for CIB monopole determination.

Currently, there are degeneracies that cannot be broken with data used in these studies. In particular, the sources of monopoles at 4.9, 12, 25, and 60\,$\mathrm{\mu m}$ cannot be used to distinguish between zodiacal emission, galactic emission, and the CIB. The addition of time-ordered data at similar frequencies and complementary scan strategies can be used to better determine the absolute brightness of zodiacal dust. In particular, the \textit{Infrared Astrophysical Observatory} (\textit{IRAS}, \citealt{boggess92}) created nearly full-sky maps at 12, 25, 60, and 100 $\mathrm{\mu m}$ with resolution between 0.5 arcmin to 2 arcmin. A full end-to-end joint analysis of \textit{IRAS} and DIRBE will leverage the unique properties of both datsets, and enable robust characterization of the CIB monopole spanning the entire infrared spectrum. Other full-sky experiments, including the \textit{AKARI} \citep{murakami:2007} and the upcoming \textit{SPHEREx} \citep{dore:2014} satellites, will be essential for determining the three-dimensional structure of the zodiacal dust and determining the spectrum of the CIB.
