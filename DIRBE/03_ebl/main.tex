...
In parallel with this effort, the \Cosmoglobe\ collaboration was formed as a direct response to a second major lesson learned from \Planck, namely that better results are obtained when analyzing multiple experiments together. The goal of this work is simply to establish a single coherent model of the astrophysical sky from radio to infrared frequencies by combining information from as many state-of-the-art experiments as possible. The first application of this framework was described in \cosmoglobe\ Data Release 1 (DR1; \citealp{CG02_01}), which performed the first joint analysis of time-ordered data from both the Wilkinson Microwave Anisotropy Probe (WMAP; \citealp{bennett:2013}) and \Planck\ LFI. As a result of this joint analysis, the quality of the sky maps from both experiments improved, and a long-standing discrepancy between the two experiments was finally resolved; ultimately, it was tracked down to calibration uncertainties in both experiments.
...
otherwise noted, this is usually set to unity (\textbf{why not use the mean and uncertainty quoted by the DIRBE team as a Gaussian prior for the gain?}), and we mostly adopt the
official DIRBE calibration as is. Moving on, $\P$ represents the
satellite pointing matrix defined in Galactic coordinates; and $\B$
represents an instrumental beam convolution operator. The sum runs
over $\n_{\mathrm{comp}}$ astrophysical components, each described by
a free amplitude $\a_c$ and a mixing matrix, $\M_c$; for full details,
see \citet{CG02_04, CG02_05}. Next, $\s_{\mathrm{zodi}}$ denotes
zodiacal light emission, which in the current model is described by
$\sim$\,70 free parameters \citep{CG02_02}, and $\n$ represents Gaussian
instrumental noise, which may or may not include correlated noise; the
correlated component is only included for the 240$\,\mu\mathrm{m}$
channel. We will return to the static component described by
$\s_{\mathrm{static}}$ in the next section.
...
contribution. In the current analysis, we adopt the following model \citep{CG02_04,CG02_05},
\begin{alignat}{4}
  ...
  + &m_{\nu} && \textrm{(Monopole)}, \nonumber
\end{alignat}
where rows describe respectively, from top to bottom, 1) cold dust emission; 2)
hot dust emission; 3) nearby dust emission; 4) free-free emisssion; 5)
CO emission; 6) CII emission; 7) bright starlight emission; 8) faint
starlight emission; 9) other compact sources; and 10) one monopole for
each band. The specific meaning of each symbol is not particularly
relevant for the purposes of this particular paper, and we therefore
refer the interested reader to the respective paper for full
details. However, what is key for the current analysis is the fact
that all these parameters are fitted simultaneously with the zodiacal
light and instrumental parameters, leading to one overall global
posterior distribution.

\subsubsection{Excess radiation in solar-centric coordinates}

The only term in Eq.~\eqref{eq:model} not discussed so far is
$\s_{\mathrm{static}}$, which denotes the contribution from a
hither-to unappreciated signal that appears stationary in the
Sun-Earth coordinate system \citet{CG02_01}. Such radiation could
arise from at least two potential sources. First, any zodiacal light
component that is actually stationary in solar-centric coordinates
would be described by this term; two well-known examples of such are
the so-called ``circumsolar ring'' and ``Earth-trailing feature'' in
the K98 model \citep{kelsall1998}, originating from dust particles
trapped in the gravitational field of the Earth. Another possible
source is straylight contamination from the Sun. As discussed by
\citet{hauser:1998}, the DIRBE optics was specifically designed to
minimize such contamination, and no corrections were made for such
radiation in the legacy analysis. However, early signs of unexplained
excess radiation with no clear zodiacal light counterpart were
reported already by \citet{leinert:1998}, as shown in their Figure~54,
but these were to our knowledge never followed up systematically or
explained physically.

A more detailed study of this signal has now been performed by
\citet{CG02_01} as part of the current analysis, and the excess
radiation has been mapped for each DIRBE frequency band by binning the
residual TOD into solar-centric coordinates. The strongest signal was
found at the 25\,$\mu\mathrm{m}$ channel, as reproduced in the top
panel of Fig.~\ref{fig:sidelobe}.
...
3.5$\,\mu\mathrm{m}$ only weak signatures are visible, and at the
longer wavelengths there is no evidence at all. In this respect it is
worth noting that the ten DIRBE detectors are divided into three
groups in the optical path of the instrument (\textbf{add reference}), with the
same grouping as observed for the strength of the solar-centric excess
signal; in this figure, the Sun is located in the center, and the
equator is aligned with the Ecliptic plane. Gray pixels are directions
that are observed by the DIRBE instrument.
...
straylight from the Sun; with an amplitude of up to 5\,MJy/sr, it must
obviously be fitted and removed from the raw time-ordered data prior
to mapmaking in order to prevent it from compromising any CIB results
derived from these data. Unfortunately, since we do not yet have a
detailed physical model for the signal, it must be fitted freely
pixel-by-pixel in solar-centric coordinates for each frequency
band. This, however, introduces a perfect degeneracy between the
zero-level of the static signal component and the CIB monopole for the
affected channels; if one adds an arbitrary offset to the map in the
top panel in Fig.~\ref{fig:sidelobe}, and subtracts exactly the same
value from the CIB monopole at the same channel, the total
goodness-of-fit at that channel is unchanged.
...
To summarize, we include a model for the static component,
$\s_{\mathrm{s}}$, by fitting its amplitude freely and separately at
each wavelength band between 4.9 and 100$\,\mu\mathrm{m}$. As seen in
Fig.~\ref{fig:sidelobe}, this results in a high signal-to-noise ratio
measurement, and it does not significantly increase the overall noise
level of the higher-level products. However, since the zero-level of
this signal is unknown, up to positivity, all CIB monopole results
between 4.9 and 100$\,\mu\mathrm{m}$ quoted in the following are upper
limits. An important goal for future work is to derive a physical
model for this signal, and only if that is successful can these upper
limits be converted into actual detections.
...
\tablenote {{\rm b}} CIO gain uncertainty; estimated by multiplying the gain uncertainty in Table~1 of \citet{hauser:1998} with the \cosmoglobe\ posterior mean values listed in the ninth column (\textbf{why not say "column (g)"?}).\par
