...
\noalign{\vskip 3pt} 
\omit\sc $\lambda$ ($\mu\mathrm{m}$)\hfil& U_{B_\nu\rightarrow\nu I_\nu}^{(\mathrm{a})} &\sigma_b^{\mathrm{(b)}} & \sigma_g^{\mathrm{(c)}} & \sigma_\mathrm{MC}^{\mathrm{(d)}} & \sigma_\mathrm{HM}^{\mathrm{(e)}}  & \sigma_\mathrm{Total}^{\mathrm{(g)}}  & \mathrm{DIRBE}^{\mathrm{(h)}} & \mathrm{Gibbs}^{(\mathrm{i})} & \mathrm{Final\,DR2}^{(\mathrm{j})} & f_{\mathrm{sky}}^{(\mathrm{k})} (\%) \cr
...
\tablenote {{\rm f}} Rms of the HMHD maps in Fig.~\ref{fig:hmhd}, downgraded to a HEALPix resolution of $N_{\mathrm{side}}=4$.\par
