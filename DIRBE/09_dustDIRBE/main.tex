...
The main topic of the last three papers in the series is modelling thermal dust radiation efficiently on large angular
...
polarization data \citep{bicep2021} to measurements of dark energy using distant
...
pointing matrix which projects the pixelated sky onto a $n_{\mathrm{tod}}$-sized
space, $\B$ is the instrumental beam convolution operator, and $\n$ is the
instrumental noise. The physical sky is represented by three terms. Firstly, a sum over sky components that can be modelled as constant at every
point in time. We sum HEALPix maps of
...
As it stands, this is a ``complete'' model for the frequency domain we are interested in this paper, but we expect that it will have to expand as additional data is added and more discoveries are made. The first four
terms form the complete model of thermal dust, which is the main focus of this
work, and are elaborated on below. The fifth term models the free-free emission, which is expected to contribute moderately at all relevant
frequencies without becoming a dominant term. Then, there are two point source
terms, all of which are dealt with in \citet{CG02_04}. Finally, the last term
represents the monopole at each frequency, which is treated in
\citet{CG02_03}.
...
(\ref{eq:skymodel}), leaving the treatment of stars to \citet{CG02_04}, monopoles to \citet{CG02_02}, and condition on free-free emission as determined by \citet{planck2014-a12}.
...
in its own right, but knowing its properties is critical for better and more
...
\label{eq:mbb}
\end{equation}
where $\nu$ is frequency, $B$ is the Planck law for a perfect blackbody at a certain frequency $\nu$ and temperature $T$,
and $\beta$ is the spectral index. Typical temperatures of this blackbody in
the diffuse ISM are around $\sim$ 20 K, meaning that the distribution typically
...
\citep{edenhofer:2024}. In this paper, we showed that using the linear minimization
five such such templates, we can explain more than 95\% of the signal variance at dust-dominated
...
similar four-component thermal dust model, in which four spatial templates
are used for the first four terms of Eq. (\ref{eq:skymodel}), constraining
these components morphologically. One of these templates, used in both
\citet{CG02_05} and \citet{CG02_06}, was derived from the
\citet{edenhofer:2024} extinction maps (see \citealt{CG02_05} for more details
on how this template was constructed), while the three others were the best-fit
solutions arrived at in \citet{CG02_06}.
...
band, as shown in \cref{tab:bands}. Each dust component is then set to be
constant within a given bin, meaning that there is one free parameter per bin
per component, which are then sampled over in the Gibbs chain.

\begin{table*}[htb]
    \centering
    \caption{Components enabled for each frequency band. Band widths correspond to the width of the corresponding dust band used in this analysis, not the instrumental bandwidths. }
    \begin{tabular}{c|c|c|c|c|c|c}
        \label{tab:bands}
        Band & \makecell{Dust Band\\Width (GHz)} & Hot dust & Cold dust & Nearby dust & H$\alpha$-correlated dust \\
        \hline
        DIRBE 240 $\mu m$ & 617 & x & x & x & x \\
        DIRBE 140 $\mu m$ & 873 & x & & x & x \\
        DIRBE 100 $\mu m$ & 1524 & x & & x & x \\
        DIRBE 60 $\mu m$ & 4936 & x & & x & x \\
        DIRBE 25 $\mu m$ & 9100 & x & & x & x \\
        DIRBE 12 $\mu m$ & 27400 & x & & x & x \\
        DIRBE 4.9 $\mu m$ & 24715 & x & & & \\
        DIRBE 3.5 $\mu m$ & 39275 & x & & & \\
    \end{tabular} 
\end{table*}
...
The analysis consisted of five Gibbs chains, each of which consisted of
approximately 250 samples. Out of these, in what follows (except for the
traceplots), the first 100 samples are considered the burn-in period, since (as
explained below) all chains seem to have converged by then.
...
exhibit any long correlations and seem to mix fairly well. The only exception
is the 240 and 120 $\mu$m H$\alpha$-correlated dust bins, where we see more
...
\begin{figure}[htb]
  ...
  \caption{H$\alpha$ dust amplitudes as a function of the iteration for the six lowest frequency DIRBE channels, with all five sampling chains overplotted. Wee see that the 25 and 12$\mu$m channels exhibit slower mixing than the others, but still manage to explore the full parameter space.}
...
strongest correlations are internally between the near dust bins. Most of them
are correlated with each other, but they are anticorrelated with the 12 $\mu$ m
amplitude. Those same bins, and in particular the 100$\mu$m one, are also
anticorrelated with the 100 $\mu$m hot dust amplitude. Thus, it might seem that
the morphologies of hot dust and nearby dust are similar enough that whenever the
one increases, the other can decrease to compensate for the change. Since the
morphology of the nearby dust component is just defined by mapping dust that is
close to us, it makes sense that this component ends up being less
``orthogonal'' to the others -- this might indicate that \emph{some} of the
nearby dust have spectral characteristics more similar to hot dust, perhaps.
...
In \cref{fig:dustmaps}, we show the dust frequency maps -- that is, the frequency
maps with starlight and free-free signal subtracted -- as well as the residual maps after subtracting the four-component dust model used in this
analysis. The grey pixels indicate the masks used at each frequency, as
described in \cref{sec:masks}.
...
IS THE EFFICIENCY FIGURE ACCURATE? \cref{fig:efficiency}
...
The final posterior fits for the dust components are shown in
\cref{fig:templates}.  As is evident, the maps are morphologically distinct, and
similarly to what was concluded in \citet{CG02_06}, the morphologies correlate
with those of \CII\ and \HI\ emission maps, respectively for the hot and cold dust populations (DON'T KNOW IF WE WANT THIS). The
H$\alpha$ component is also morphologically similar to the WHAM
...
Applying this model to DIRBE data, and inspired by the aforementioned
