...
In a series of seven companion papers, of which this is the last,
...
full-sky DIRBE frequency maps, presented in \citet{CG02_01}, covering the infrared frequency range
...
supernovae \citep{something here}.
...
deep CMB $B$-mode polarization experiments \citep{litebird, so}, and polarized thermal dust
...
the joint posterior of all involved unknowns, and thus intricately bound up with the
...
(\ref{eq:skymodel}), and leave the other components for other papers \citep{CG02_01, CG02_02, CG02_03, CG02_04}.
...
\citep{edenhofer:2024}. In that paper, we showed that using the linear minimization
of five such templates, we can explain more than 95\% of the signal variance at dust-dominated
frequencies in \Planck\ (353-857\,GHz) and DIRBE (240 - 60 $\mu$m), and that it
still accounted for more than 80\% of the signal variance at the DIRBE bands with known zodiacal residuals (25 and 12 $\mu$ m).

In another recent paper \citep{CG02_06}, we applied the main idea behind that
result to the \Planck\ HFI data using an extensive search of the full parameter space. We showed
that by using the GAIA extinction template, and by assuming three other
...
band, as shown in table \ref{tab:bands}. Each dust component is then set to be
