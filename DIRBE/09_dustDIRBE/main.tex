...
modelled as constant at every point in time. We sum HEALPix maps of
...
present work is the first of the three terms, which will be expaned on in what
follows.
...
were the best-fit solutions arrived at in \citet{CG02_06}. We plot these
...
\ref{tab:bands}. NEED EXPLANATIN HERE.
...
enough, looking at Fig. \ref{fig:templates}, that there is overlap between the components, and as long as 
...
In the same plot, we also plot posterior total SED, when all four components
are summed up, and we also show the Astrodust+PAH model that is the best fit to
...
At the higher frequencies, the model is clearly struggling to account for
...
derived from the first and second data releases (\textbf{is this true?}),
...
directly taken from the Gibbs chains as presented in \citet{CG01_01} and
