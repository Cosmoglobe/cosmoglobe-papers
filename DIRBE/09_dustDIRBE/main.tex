...
is a term representing the zodiacal emission, which cannot be treated as being
constant in time (which for most other components is an unproblematic
assumption). Finally there is a term representing a component that is static in
...
present work we focus on the first of the three terms, which will be expanded on in \cref{sec:bayseian,sec:modelling}.
...
wavelengths. Understanding this material is interesting in its own right,
however, it is also crucial to improve astrophysical foreground removal,
especially when interstellar dust emission contaminates other signals of
interest.\footnote{For a recent review, see \citet{Hensley2021}.}.

Recently, the ``astrodust+PAH'' model \citep{Hensley2023} was introduced, wherein
the diffuse interstellar medium (ISM) is hypothesized to be made up of a single
...
\label{eq:mbb}
\end{equation}
where $\nu$ is the frequency, $B$ is the Planck law for a perfect blackbody at a
...
and \HI) and from inference of nearby dust structures via starlight extinction
...
that, by using the \GAIA\ extinction template, and the H$\alpha$ template and by assuming two other
...
Two of these templates, used in both \citet{CG02_05} and \citet{CG02_06}, were
derived from the \citet{edenhofer:2024} extinction maps and from the WHAM H$\alpha$ \citep{wham:2003} (see \citealt{CG02_05}
for more details on how these templates were constructed),  while the other two
were the best-fit solutions obtained in \citet{CG02_06}. We plot these
templates in \cref{fig:templates}.
...
With this, then, the total sky model (the third term of \cref{eq:model}) in
...
Hence, in this paper, we mainly focus on the four first components of Eq.
