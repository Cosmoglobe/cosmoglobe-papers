...
dust modified blackbody curve at the lower frequency bands, mirroring the
results found in \citet{CG02_05}. At higher frequencies, we also see that they
exhibit the characteristic rise in the SED, where models like Astrodust+PAH
predicts emission lines from the nanoscale particles. The figure also shows
that fitting the total (i.e. the sum of all the components) dust model to the
Astrodust+PAH model yields a version of the latter that seems to fit very well
with the posterior mean SED values.
