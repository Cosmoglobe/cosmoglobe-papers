...
prior term) equivalent to sampling from
$P(\dv|\theta)\equiv\mathcal{L}(\theta)$, the so-called \emph{likelihood}
function.

The number of parameters involved in our model (see \cref{eq:model}) is of the
...
then says that by combining these samples for all the
...
Hence, in this paper, we mainly focus on the three first components of \cref{eq:skymodel}, leaving the treatment of stars to \citet{CG02_04},
...
emission contaminates the other signals of interest\footnote{For a recent
