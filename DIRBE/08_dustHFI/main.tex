...
When considering our summary statistics in \cref{sec:correlations} we compare the new dust templates with the \ion{C}{ii} line emission as measured by the FIRAS \ion{C}{ii} map \RS{CITATION}, and to the HI4PI data \citep{HI4PI2016}. 
...
\begin{align}
    ...
    \label{eq:dataModel}
\end{align}
where $g_j$ are the instrumental gain for detector $j$, $\mathsf{B}$ the beam convolution, $\mathsf{M}_{sj}(\beta_p,\delta_j)a_{p,j}$ the foregrounds mixing matrix with spectral parameters $\beta$, bandpass specifications $\Delta_{j,\mathrm{bp}}$ and amplitudes $a_{p,j}$ per pixel $p$ for the various sky components ($s$). 
...
$\vec{a}_{s}$ for $s\in (\mathrm{c,h,co})$ are the amplitudes sampled per-pixel for the CMB ($\mathrm{cmb}$), cold dust ($\vec{a}_\mathrm{c}$), hot dust ($\vec{a}_\mathrm{h}$), and the CO lines (($\vec{a}^{\{100,217,353\}}_\mathrm{co}$)). We keep the CO line-ratios ($h_\nu^{\{100,217,353\}}$) constant for this analysis from the \planck\ results \cite{planck2020-LVI}. We also keep the free-free ($\vec{a}_\mathrm{ff}$) fixed due to the low constraining power of the HFI bands on the free-free component. The template we use has electron temperature ($T_e$) of 7000 K and amplitudes set from the Beyond Planck analysis \cite{bp01}. $a_\mathrm{near}$ and $a_\mathrm{H\alpha}$ are amplitude scale value applied to templates ($\vec{A}_\mathrm{Eidenhofer}$ and $\vec{A}_\mathrm{WHAM}$) for the nearby dust ($\mathrm{near}$) and \Ha\ correlated (\Ha) dust respectively.   
Finally, the monopoles and dipoles ($m_\nu$) are sampled for each frequency band. As mentioned in \cref{sec:planckhfi} we have corrected the maps for the CIB and zodiacal light, so these are not included here, as well as synchrotron which is subdominant in the HFI frequency range. 
...
\begin{align}
\vec{a}_i &\leftarrow P({\bf a}_i |{\bf d}, \phantom{\vec{a}_i ,} g_\nu,m)\\
g_\nu &\leftarrow P(g_\nu |{\bf d}, \vec{a}_i,\phantom{g_\nu,}m)\\ 
m &\leftarrow P(m |{\bf d}, \vec{a}_i,g_\nu,\phantom{m}) 
\end{align}
where the amplitudes for the maps (as mentioned in \cref{sec:skymodel}) are sampled while holding the 857~GHz gains and monopoles and dipoles fixed, and proceeding similarly through the rest of the sampled parameters. For a detailed discussion of the Gibbs sampling see \cite{bp03,eriksen2008} and references therein. 
\RS{Should I also have Cls for the CMB}
\RS{Add references} 
...
% \label{sec:bestfit}
The results for the best fit model can be seen in \cref{fig:dust_figures} with the MBB parameters in \cref{tab:SEDs} and plotted in \cref{fig:dustSED}. Results for the gain estimation can be seen in \cref{tab:gains}. \RS{Should the 857 gains and 545 gains have error bars?? maybe from NPIPE paper...} The sky component fits can be seen in \cref{fig:SkyModel}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics{figures/correlation_plot_nodame.pdf}
...
\begin{figure*}[htbp]
    ...
    \caption{Monopole subtracted ratio of the cold dust amplitudes compared to the fiducial value (central panel) for varying $T_\mathrm{c}$ and $\beta_\mathrm{c}$ values with all other grid parameters set to the fiducial values as recorded in \cref{tab:SEDs}. The corresponding hot dust maps can be seen in \cref{fig:cold_hot_dust_set}.}
...
\begin{figure*}[htbp]
    ...
    \caption{Monopole subtracted ratio of the hot dust amplitudes compared to the fiducial value (central panel) for varying $T_\mathrm{h}$ and $\beta_\mathrm{h}$ values with all other grid parameters set to the fiducial values as recorded in \cref{tab:SEDs}. The corresponding cold dust maps can be seen in \cref{fig:hot_cold_dust_set}}
