...
%\title{\bfseries{\Cosmoglobe\ DR2. VI. High-resolution decomposition of hot and cold thermal dust emission derived from Planck HFI}}
   ...
   \abstract{
    We present the first high resolution hot and cold dust components in a four component dust model, finding a strong correlation between the hot dust and ionized-carbon (\ion{C}{II}) emission. This model is used in the Cosmoglobe DR2 reanalysis of the DIRBE dataset, which produces the best DIRBE maps and new contraints on the cosmic infrared background in the DIRBE bands. Aside from the codl and hot dust component, we additionally use a nearby dust template based on the WISE extinction from Endenhofer et al. \cite{edenhofer:2024}, and a dust extinction template based on the Wisconsin H-$\alpha$ mapper (WHAM) data \cite{wham:2003}. This new dust model was produced using a large multi-fit Gibbs analysis and a parameter grid search to produce an optimal and physical solution with the \planck\ HFI bands. We find a strong correlation between the cold dust component and the neutral hydrogen emission, and between the hot dust component and the \ion{C}{II} emission. This is an exciting though expected result as the \ion{C}{II} emission exists in hot areas of the Milky-Way, near large stars for example. This four component dust model fits the HFI data very well, GOODNESS OF FIT. 
    This new dust template will be critical for future temperature and polarization measurements from for example Simons Observatory and LiteBIRD, where high-precision measurements of the dust will be critical for measuring $B$-modes in the CMB. 
