...
\label{sec:otherdata}
The nearby dust relative amplitudes are fixed to the \GAIA-based dust extinction template produced by \citet{edenhofer:2024}, which covers distances up to 1.25 kpc. This primarily uses \GAIA\ distances and extinction estimates to build a 3D map of the nearby dust from \citet{2023Zhang}, which also leverages the two micron all sky survey (2MASS), the wide-field infrared survey explorer (WISE and unWISE), the large sky area multi-object fibre spectroscopic telescope (LAMOST). We use the total integrated dust map as our model in this analysis. 
...
% \label{sec:bestfit}
The results for the best fit model can be seen in the top two panels of \cref{fig:dust_figures}. It is striking that even though the amplitudes were free to vary, the shape of the cold dust has a close match to the \ion{H}{i} map, shown in the bottom left panel. The cold dust is concentrated in the plane of the galaxy, with still a significant amount away from the galactic center along the galactic plane. The shape of the hot dust also has a striking resemblance to the \ion{C}{ii} map, in the bottom right panel. There is a clear excess closer to the galactic center, with small bubbles (e.g. the gum nebula at $l=256^\circ,b=-9^\circ$) matching in both maps. This will be discussed further in \cref{sec:correlations}. The best fit MBB parameters are recorded in \cref{tab:SEDs}, where the error bars are interpreted from the grid search results in \cref{fig:SEDGridContours}. In \cite{CG02_05} we found cold dust temperatures (corresponding to the CO and \ion{H}{i} dust) of 12 and 16~K, with spectral indices of 1.9 and 1.4, so we find consistency with the HFI channels as to the template fits. The hot dust as fit by HFI is slightly hotter than the fit from DIRBE templates, but with a relatively wide error bar they are still consistent. As mentioned previously, the \Ha\ correlated dust parameters were taken from the DIRBE template fits. Lastly, the near dust (known as the GAIA extinction in \cite{CG02_05}) was found to have a slightly hotter fit for the HFI, but still consistent, at 18~K with spectral index of 1.7 compared to 14.9 and 2.37 from the DIRBE templates. This dust, unlike the cold and hot dust, is much less concentrated near the galactic plane (as might be expected). The relative fraction of the dust can be seen in \cref{fig:dustFrac} and discussion in \cref{app:dustCharacterization}. \Cref{fig:dustSED} shows the MBB fits as a function of frequency, with amplitudes set by the RMS amplitude of the map for sky cuts about the galactic plane (94\% and 70\% $f_\mathrm{sky}$). This emphasizes both the challenge of doing such a fit with only HFI data (since within the bands fit by HFI the spectra are relatively flat) but also the incredible power additional data sets (such as the \cite{edenhofer:2024} and \cite{wham:2003,2016WHAM}) bring, allowing us to effectively separate these dust components. Results for the gain estimation can be seen in \cref{tab:gains}. The 857~GHz gain values are very important when considering the dust, since they shift the measured spectrum in those bands, and the higher frequencies give us the most control of the dust (see again \cref{fig:dustSED}). They are very difficult to control though, and have a large error range as published by the \planck\ collaboration \citep{npipe}. This is a large contributor to our error budget on the dust parameters. 

The sky component fits can be seen in \cref{fig:SkyModel}, we have found fantastic fits to the CMB for example, with minimal residuals in the galactic plane. We expect with future analysis and allowing the CO line ratios to fluctuate (and perhaps including the LFI bands to better control the low-frequency foregrounds) that the CO maps could also be improved with this new dust model. 
...
\label{sec:gof}
We can assess the goodness-of-fit of the model, by comparing the data input minus the non-dust components (since we are not concerned with those fits here), leaving just the input dust plus noise essentially (and any other unmodeled foregrounds), and take the squared difference with the residual map, divided by the input dust map. This effectively tells us what fraction of the signal has been modeled, and  we see that the four-component dust model successfully models over 99\% of the signal across all \planck\ HFI bands in \cref{fig:efficiency}. This can also be seen in the residual averages in \cref{fig:MapsVsResiduals}, where the central column shows the residuals at the same scale range as the maps (in the right most column). The left column shows the residuals at a tighter scale range and while there is still structure to the maps (where ideally we would have white noise), this indicates there are perhaps future improvements to be made to the gain, zodiacal light modeling and potentially carbon monoxide lines. It is an impressively good fit nevertheless, and further shows what we see in \cref{fig:efficiency}. The residuals per band can be found in \cref{app:residuals}.
...
One of the great successes of the four-component dust model is the high degree of correlation between the \ion{H}{i} emission, a known dust tracer, and the cold dust, with a Pearson's correlation coefficient of 93\% and the correlation between the new dust tracer \ion{C}{ii}, which traditionally maps the hot ionized medium and traces well the hot dust component, with a correlation of 94\% (see also \cref{fig:hotCiiCorr}). The correlation lines in the hot and cold dust grids, as plotted in \cref{fig:SEDGridContours}, show an interesting feature, that the areas which were unphysical also trace the areas where the correlation between the dust and their relevant tracer were lower (e.g. the 95\% Hot-\ion{C}{ii} contour line traces the grey gridded region in the cold dust $\chi^2$ contour plot, and the 92\% Cold-\ion{H}{i} contour line also maps ). Since these tracers are not included in the fitting, and are simply calculated from the resulting maps, it appears these are phenomena that arise in tandem. If the correlation between the dust and its tracer dips below a certain value, the final dust fit will become unphysical and be rejected. 
