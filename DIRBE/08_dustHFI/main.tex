...
\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{figures/HotDust_Grid.pdf}
    \caption{Hot dust (\ion{C}{II} correlated dust) for varying $T_\mathrm{H}$ and $\beta_\mathrm{H}$ values with all other grid parameters set to the fiducial values as recorded in \cref{tab:SEDs}. }
    \label{fig:hot_dust_set}
\end{figure*}
...
\begin{figure}
    ...
    \caption{Efficiency of the model representing the data. Taking the input map squared minus the residuals, divided by the residuals and summed across the map we can measure how much of the signal is represented by the model and find that it is greater than 98\% across all HFI bands. There is one point for each band, though many are overlapping.}
