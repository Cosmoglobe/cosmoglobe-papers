...
%\title{\bfseries{\Cosmoglobe\ DR2. VI. High-resolution decomposition of hot and cold thermal dust emission derived from Planck HFI}}
   ...
   \abstract{
       ...
       We use a parameter grid search (for global parameters) coupled to a Bayesian Gibbs sampler (for per-pixel parameters) to fit this model to \textit{Planck} HFI data. The fiducial MBB temperatures for the hot and cold dust components are $T_{\mathrm{hot}} = 30\pm3\,\mathrm{K}$ and $T_{\mathrm{cold}} = 11\pm1\,\mathrm{K}$, and the corresponding fiducial spectral indices are $\beta_{\mathrm{hot}}\ge1.75$ and $\beta_{\mathrm{cold}}=1.85\pm0.10$, respectively. In agreement with the companion low-resolution analysis, we find that the hot dust component is strongly correlated with the FIRAS \ion{C}{ii} map, while the cold dust component is strongly correlated with the HI4PI \ion{H}{i} map.
...
\begin{figure}[t]
    ...
    \includegraphics[width=0.98\linewidth]{figures/BothHotColdCorr_v2.pdf}
