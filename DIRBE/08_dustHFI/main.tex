...
\subsection{Masks}
\label{sec:masks}
When computing the fit for the hot and cold dust we used the full sky, however during the gain computation for the 857~GHz maps we used a mask of the highest signal areas of the galactic plane, 
\cref{fig:gainmask}, during the computation of the monopoles and dipoles for FIRAS we used a 30 degree galactic cut mask (see \cref{fig:firasmask}) and for HFI we used \cref{fig:HFImask}. For the $\chi^2$ calculations comparing the grid search values we use the mask in \cref{fig:chimask}. 

% \begin{figure}
%     \centering
%     \includegraphics[width=\linewidth]{figures/gain857_mask.pdf}
