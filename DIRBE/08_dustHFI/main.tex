...
\section{Data}
\label{sec:data}
Building on the work of \cite{CG02_05}, this analysis uses the \planck\ PR4 data (\ref{sec:planckhfi}), several FIRAS maps (\ref{sec:firas}), and additional ancillary data (\ref{sec:otherdata}) to constrain a new high resolution four component dust model. We discuss our masks in \cref{sec:masks}. 
...
\label{sec:otherdata}
The near dust relative amplitudes are fixed to the \GAIA-based dust extinction template produced by \citet{edenhofer:2024}, which covers distances up to 1.25 kpc. This primarily uses \GAIA\ distances and extinction estimates to build a 3D map of the near dust from \citet{2023Zhang}, which also leverages the two Micron All Sky Survey (2MASS), the Wide-field Infrared Survey Explorer (WISE and unWISE), the Large sky Area Multi-Object fibre Spectroscopic Telescope (LAMOST). We use the total integrated dust map as our model in this analysis. 
Additionally, we use the Wisconsin H-$\alpha$ Mapper (WHAM) data as a template for the \Ha\ correlated dust, and the results from \citep{CG02_05} as the fiducial values for the \Ha\ MBB, since we find that it is poorly constrained by the \planck\ bands (see \cref{sec:results}). As with DIRBE, the \Ha\ dust is a small extinction component in the \Planck\ bands.  
...
\begin{figure}[htbp]
    ...
    \caption{Spectral energy distribution (SED) for the four dust components across the HFI bands. Note that the \Ha\ dust is a negative amplitude since it is an extinction component, but the absolute value is plotted for clarity. The lower bound is set by a 70\% $f_\mathrm{sky}$ and the upper bound by a 94\% $f_\mathrm{sky}$. The grey vertical lines are the locations of the 6 \planck\ HFI bands. }
    \label{fig:dustSED}
\end{figure}
...
\noalign{\vskip 3pt}
Near dust& $ 18 \pm 2$ & $ 1.7 \pm 0.1 $& $\le 1^\mathrm{b}$\cr
\noalign{\vskip 3pt}
...
%MG: this table had a huge amount of wasted space, I have reformatted it below
%\begin{table}[tbp!]
%\begingroup
%\newdimen\tblskip \tblskip=5pt
%\nointerlineskip
...
%     \hline
%     \hline
%     Band & 545-1 & 545-2 & & 545-4\\ 
%     Gain & 0.99145 & 1.00487 & & 1.00830 \\
%     \hline
% 	\\
%     \hline
%     Band & 857-1 & 857-2 & 857-3 & 857-4\\
%     Gain & 0.99362 & 1.10242 & 1.02393 & 1.13470\\
%     \hline
%     \end{tabular}
...
Gain$^a$ scale& 0.99145 & 1.00487 & & 1.00830\cr
\noalign{\vskip 4pt\hrule\vskip 6pt}
\noalign{\vskip 3pt}
Band & 857-1 & 857-2 & 857-3 & 857-4\cr
Gain$^b$ & 0.99362 & 1.10242 & 1.02393 & 1.13470\cr
\noalign{\vskip 3pt}
\noalign{\vskip 3pt\hrule\vskip 4pt}}}
\endPlancktable
\endgroup
\caption{Gain values for 545 and 857~GHz bands. The gains for 857 GHz were sampled during the Gibbs loop, the 545 gains were fixed to input values.}
\tablewidth=\columnwidth
\tablenote {{\rm a}} {\it \footnotesize{The 545~GHz gains from \npipe\ have an uncertainty around 2 percent.}}\par
\tablenote {{\rm b}} {\it \footnotesize{The 857~GHz gains sampled from the grid search and in the Gibbs sampling show uncertainties at the 0.0001 level.}}\par
\label{tab:gains}
\end{table}
...
\begin{figure*}[t]
    ...
    \caption{Left column shows the sum of the dust maps averaged for each band (plus their residuals), and the middle column the corresponding residuals (with the same color bar range). The final column shows the residuals with the color bar scaled down by a factor of 100 to more easily see the structure in the residuals. 
    %The structure in the residuals indicates there may be still some zodiacal light, and that the CO lines, particularly in the 100~GHz bands could be improved. 
...
\begin{figure}[t]
	...
	% \\
	%\textbf{Duncan question: how much does the efficiency depend on the pivot / signal-to-noise?}
	}
...
\begin{figure}[t]
    ...
    %Full size seemed a bit too large but if smaller is desired, use correlation_plot_nodame.pdf
    }
...
For the two template maps, the near and \Ha\ dust, we additionally explore the space of amplitude scale factors. This is simple a multiplicative factor applied to the template map, scaling the power up and down equally across the sky (see \cref{fig:SEDGridAmplitude}). For the nearby dust we again want to reduce the correlation between the nearby dust and the hot and cold dust as much as possible, while satisfying the other physicality requirements discussed earlier.  
...
% MG: \cref{fig:SEDGridAmplitude} is something that should be discussed here but I don't understand what it shows.
...
The best fit MBB parameters are recorded in \cref{tab:SEDs}, where the error bars are interpreted from the grid search results in \cref{fig:SEDGridContours}. In \cite{CG02_05} we found cold dust temperatures (corresponding to the CO and \ion{H}{i} dust) of 12 and 16~K, with spectral indices of 1.9 and 1.4, so we find consistency between the HFI channels and the CO correlated dust of that analysis. The hot dust as fit by HFI is slightly hotter than the fit from DIRBE templates (found to be 24.7K), but with a relatively wide error bar they are still consistent. As mentioned previously, the \Ha\ correlated dust parameters were taken directly from the DIRBE template fits, and thus are consistent by construction. Lastly, the near dust (known as the GAIA extinction in \cite{CG02_05}) was found to have a slightly hotter fit for the HFI channels, at 18~K with spectral index of 1.7 compared to 14.9~K and 2.37 from the DIRBE templates. This dust, unlike the cold and hot dust, is much less concentrated near the galactic plane, as is expected (see \cref{app:dustTemplates} for the template fits and discussion). 

\Cref{fig:dustSED} shows the MBB fits as a function of frequency, with amplitudes set by the RMS amplitude of the map for sky cuts about the galactic plane (94\% and 70\% $f_\mathrm{sky}$). This emphasizes both the challenge of doing such a fit with only HFI data (since within the bands fit by HFI the spectra are relatively flat) but also the incredible power additional data sets (such as the \cite{edenhofer:2024} and \cite{wham:2003,2016WHAM}) bring, allowing us to effectively separate these dust components. A different visualization of the SEDs (spectral energy distributions), the relative fraction of each dust component at 545~GHz, can be seen in \cref{fig:dustFrac} and discussion in \cref{app:dustCharacterization}.

Results for the gain estimation for the fiducial model can be seen in \cref{tab:gains}. The 857~GHz gain values have a large impact on the dust, since they shift the measured spectrum in those bands, and the higher frequencies give us the longest ``lever'' on the dust (see again \cref{fig:dustSED}). They are very difficult to control and have a large error range as published by the \planck\ collaboration \citep{npipe}. This is a large contributor to our error budget on the dust parameters. 

The sky component fits can be seen in \cref{fig:SkyModel}. Here, we have been able to recover the CMB with great accuracy, with minimal residuals, even in the galactic plane. The free-free is a fixed input template, and the recovery of the CO lines is has worked well enough that they have been removed from the dust templates. The CO line recovery could be greatly improved in a future analysis where we allow the line ratios to fluctuate and  include \Planck\ LFI data to better control the low-frequency foregrounds. 

Finally, the monopole values are simply offsets for each map, which are effectively a correction term for any residual offset that might have been introduced during mapmaking, and as such are not presented here due to space constraints.
...
One of the great successes of this four-component dust model is the high degree of correlation between the \ion{H}{i} emission, a known dust tracer, and the cold dust, with a Pearson's correlation coefficient of 92\%, as well as the correlation between the new dust tracer \ion{C}{ii}, which traditionally maps the hot ionized medium but is found here to trace the hot dust component with a correlation of 95\%. \cref{fig:hotCiiCorr} shows per-pixel scatter plots between our dust maps and these tracers. The top panel shows the hot dust and FIRAS \ion{C}{ii}, downgraded to the $7^\circ$ resolution of the FIRAS maps, and the bottom pane shows the cold dust and HI4PI column density maps at $N_{\mathrm{side}}=1024$. Both show strong correlation, which is further evidence that they trace similar regions of the Galaxy. 
...
components to the \Planck\ data using GNILC, a needlet ILC code with
...
subtract CIB fluctuations at the input map level, the two $T$ maps
...
\section{Conclusions}
\label{sec:conc}
% We present the first four-component high resolution dust maps produced using the \planck\ HFI data. Two of the dust components are built from templates based on external data. First, the \cite{edenhofer:2024} nearby dust map, used to build our near dust map ($T_\mathrm{near}=18$, $\beta_{near}=1.7$), and secondly the WHAM \Ha map \citep{wham:2003,2016WHAM}, which informs our \Ha\ correlated dust map ($T_\mathrm{H\alpha}=17.8$, $\beta_{H\alpha}=2.62$). We find that the \planck\ HFI data has little constraining power on the \Ha\ correlated dust, so take the best fits from DIRBE for this analysis. In addition, we generate two novel dust maps, corresponding to hot and cold dust populations, which have global temperature and spectral indices of $T_\mathrm{h}=30$~K, $\beta_{h}=1.75$, and $T_\mathrm{c}=11$~K, $\beta_{c}=1.85$ respectively. These maps end up having a 92\% correlation between the cold dust and the HI4PI map \citep{HI4PI2016} and a striking 94\% correlation between the hot dust and the FIRAS \ion{C}{ii} map. This new dust model is able to successfully map physical tracers of hot and cold regions in the Milky Way, with fewer free parameters than previous dust models, and with more physical meaning to the parameters. 

% We believe that this model represents a strict improvement over the established 3-parameter MBB description of dust, and should be used preferentially for future simulations of Milky Way dust. Additionally, future analysis should look for similar improvements in the polarized dust populations should the data achieve the required signal-to-noise. 
% We note that the residuals are very small, and the new dust model successfully represents over 99\% of the dust signal in all the \planck\ HFI bands. 

% Future analyses of this data could attempt improved gain estimations for the 545~GHz and 857~GHz frequency channels, which could lead to further improvements. Additionally, the treatment of the CO line-ratios could be improved in particular in the 100~GHz bands, by sampling them or including additional data that could better constrain them. Finally, improvements in zodiacal modelling and CIB mitigation at HFI frequencies could clear up some of the remaining residual power that this model cannot describe.

In \cite{CG02_05} we show that the thermal dust in the microwave and infrared regimes can be described well by a multi-component model, in which each component is tracing a specific physical environment. Namely, we use templates to trace \ion{H}{i} correlated dust, \ion{C}{ii} correlated dust, stellar extinction dust, CO correlated dust and \Ha\ correlated dust, and preform a low-resolution analysis with these templates. We find that this fits a modified blackbody across most of the included bands and have little spatial SED variations. 

Two dominant components in \cite{CG02_05} were the \ion{C}{ii} correlated dust (hot dust) and the ion{H}{i} correlated dust (cold dust). The correlation between \ion{C}{ii} emission and dust had not been previously known to us, but represented an exciting new tracer. 

Here, we present the first high resolution four-component dust maps produced using the \planck\ HFI data. We generate two novel dust maps, corresponding to the aforementioned hot and cold dust populations, which have global temperature and spectral indices of $T_\mathrm{h}=30$~K, $\beta_{h}=1.75$, and $T_\mathrm{c}=11$~K, $\beta_{c}=1.85$ respectively. These maps end up having a 92\% correlation between the cold dust and the HI4PI map \citep{HI4PI2016} and a striking 95\% correlation between the hot dust and the FIRAS \ion{C}{ii} map. This new dust model is able to successfully map physical tracers of hot and cold regions in the Milky Way, with fewer free parameters than previous dust models, and with more physical meaning to the parameters. We believe that this model represents a strict improvement over the established 3-parameter MBB description of dust, and should be used preferentially for future simulations of Milky Way dust.

When compared to previous \planck\ dust maps by combining the four dust templates into a single dust component, we see similar results, without large spatial SED variations. This strongly suggests that previous analyses that have modelled dust as a one-component MBB with spatially varying spectral indices have actually been averaging over different effects that ideally shouldn't be averaged over. As such, by using multiple components with simpler spatial varations our model is a simpler, more physical, and more economical model. Furthermore, as it is computationally much easier to fit for linear (amplitude) parameters than for non-linear (spectral) parameters, so this will also provide speed-ups to future dust simulations and analysis. 

In the future, this work should be extended through joint analyses with DIRBE, AKARI, WISE, SPHEREx and others, to produce even higher resolutions dust maps with greater sensitivity and longer frequency coverage. 
...
% 1) Eirik's first dust paper showed that thermal dust in the microwave and infrared regimes could be described by a multi-component model in which each component trace a specific physical environment, with little spatial SED variations; 
% 2) the two dominant components in this decomposition was hot and cold dust, which correlate with CII and (HI+CO), respectively; 
% 3) while the previous paper performed a low-resolution analysis with existing external templates, this paper has derived new high-resolution maps of hot and cold dust derived from Planck HFI. And those are the main products from this paper.  
% 4) In agreement with Eirik's findings, these new templates also trace dust in Planck extremely efficiently, without spatial SED variations. This strongly suggests that previous analyses that have modelled dust as a one-component MBB with spatially varying spectral indices have in effect been averaging over different effects that ideally shouldn't be averaged over, and a simpler, more physical, and more economical model is available through multiple components with simpler spatial variations. Furthermore, it is computationally much easier to fit for linear (amplitude) parameters than for non-linear (spectral) parameters. 
% 5) In the future, this work should be extended through joint analyses with DIRBE, AKARI, WISE, SPHEREx etc. to produce even higher resolutions dust maps with greater sensitivity and longer frequency coverage. 
% 6) This result is also critically important for polarization B-mode experiments, as it may suggest that future polarization B-mode experiments should implement capabilities to disentangle hot and cold dust efficiently, which suggest a highest frequency around 800-1000 GHz. 
...
%\begin{figure*}[t]
%    \centering
%    \includegraphics[width=0.6\linewidth]{figures/residuals_plotres.pdf}
...
\section{RMS scaling}
\label{sec:RMSScale}
The published single horn RMS maps were found to have a known residual multiplicative offset from the production pipeline, as is evident when the single horn data maps and RMS maps are compared each-other and to the single frequency maps (averaged maps). We include in \cref{tab:RMSScales} the values used here to divide the single horn RMS maps. 
...
\halign{\tabskip 0pt \hbox to 0.1\linewidth{#\leaderfil}\tabskip 0pt&\hbox to 0.1\linewidth{\hfil#\hfil}\tabskip 0pt&\hbox to 
0.1\linewidth{\hfil#\hfil}\tabskip 0pt&\hbox to 
0.1\linewidth{\hfil#\hfil}\tabskip 0pt&\hbox to 
0.1\linewidth{\hfil#\hfil}\tabskip 0pt&\hbox to 
0.1\linewidth{\hfil#\hfil}\tabskip 0pt&\hbox to 
0.1\linewidth{\hfil#\hfil}\tabskip 0pt&\hbox to 
0.1\linewidth{\hfil#\hfil}\tabskip 0pt&\hbox to 0.1\linewidth{\hfil#}\tabskip 0pt\cr
\noalign{\doubleline\vskip 1pt}
\omit\hbox to 1in{Frequency\hfil}& Horn 1 Scale &
Horn 2 Scale &
Horn 3 Scale &
Horn 4 Scale &
Horn 5 Scale &
...
The templates used for the near dust and the \Ha\ dust are the dust extinction from \cite{edenhofer:2024} and the WHAM maps \citep{wham:2003,2016WHAM}. The fits from this analysis can be seen in \cref{fig:dust_template_figures}. Note that the \Ha\ is negative as it represents a dust extinction component, reducing the apparent dust in the blue areas. 
\begin{figure*}[h]
    \centering
\includegraphics[width=0.4\linewidth]{figures/dustComp_nearDust.pdf}
\includegraphics[width=0.38\linewidth]{figures/dustComp_HaDust_sb.pdf}
    \caption{Near dust and \Ha\ dust extinction template fits.}
...
\vspace*{1mm}
\centering
\hspace*{1.5cm}\includegraphics[width=\textwidth]{figures/residuals_plotres.pdf}
...
\centering
\hspace*{1.5cm}\includegraphics[width=0.75\textwidth]{figures/hotDust_cold_Grid_v2.pdf}
...
\centering
\hspace*{1.5cm}\includegraphics[width=0.75\textwidth]{figures/coldDust_cold_Grid_ratio_v2.pdf}
...
% \begin{figure*}[t]
%     \centering
%     \includegraphics[width=0.9\linewidth]{figures/hotDust_wham_Grid.pdf}
...
%     \centering
%     \includegraphics[width=0.9\linewidth]{figures/coldDust_wham_Grid.pdf}
...
% \begin{figure*}[t]
%     \centering
%     \includegraphics[width=0.9\linewidth]{figures/hotDust_wham_Grid_ratio.pdf}
%     \caption{Monopole subtracted hot dust amplitudes ratios compared to the fiducial map (central panel) for varying $T_\mathrm{H\alpha}$ and $\beta_\mathrm{H\alpha}$. The corresponding cold dust maps can be seen in \cref{fig:Ha_cold_dust_set_r}.}
%     \label{fig:Ha_hot_dust_set_r}

%     \centering
%     \includegraphics[width=0.9\linewidth]{figures/coldDust_wham_Grid_ratio.pdf}
%     \caption{Monopole subtracted cold dust amplitudes ratios compared to the fiducial map (central panel) for varying $T_\mathrm{H\alpha}$ and $\beta_\mathrm{H\alpha}$. The corresponding hot dust maps can be seen in \cref{fig:Ha_hot_dust_set_r}.}
...
The near dust was found to have unphysical regions banded both by the cold dust amplitudes and the hot dust amplitudes. We additionally aimed to minimize correlations between the cold and near, and hot and near, dust, such that the dust population described by the near dust was an independent dust component. For regions with lower $\beta_\mathrm{near}$ and lower $T_\mathrm{near}$ the cold dust starts to behave unphysically and this corresponds also to a higher correlation between the near dust and the hot dust (as can be seen by eye in the lower left panels of \cref{fig:near_hot_dust_set}). 
...
The parameter space explored for the \Ha\ correlated dust was shown to have a very flat $\chi^2$ profile, and minimal changes to the hot and cold dust amplitudes or correlations. Since there is no perceivable difference then in the cold and hot dust amplitudes during the grid search we do not include those plots here. As a reminder, we chose to take the DIRBE fit values from \cite{CG02_05} for our fiducial values since the HFI have so little control of this dust component and as we expect the DIRBE bands to have better control of these SED parameters. 
