...
For this analysis we use the single-bolometer HFI maps, chosen because HFI offers strong constraining power on the dust and the single-bolometer maps are able to better control the gain fluctuations and model components such as the CO-emission.\footnote{The 100~GHz and 143~GHz maps are at an \nside\ of 2048, the other frequencies are at an \nside\ of 4096.} 
...
\subsection{FIRAS}
\label{sec:firas}
FIRAS was a cosmology telescope, launched in 1989 as part of the COBE mission, with the goal of measuring the blackbody spectrum of the CMB to unprecedented precision \citep{fixsen1997}. The FIRAS maps have very low calibration uncertainties, and so we use them to help calibrate the map amplitudes. The FIRAS 857, 1251, 1809, 2081, 2135 and 2802 GHz maps are used for this analysis in conjunction with the HFI maps. 
...
$\vec{a}_{s}$ are the amplitudes sampled per-pixel for the CMB ($\vec{a}_\mathrm{cmb}$), cold dust ($\vec{a}_\mathrm{c}$), hot dust ($\vec{a}_\mathrm{h}$), and the CO lines (($\vec{a}^{\{100,217,353\}}_\mathrm{co}$)). We keep the CO line-ratios ($h_\nu^{\{100,217,353\}}$) fixed for this analysis to the \planck\ results \cite{planck2020-LVI}. We also keep the free-free component fixed due to the low constraining power of the HFI bands on the free-free emission. The template we use has electron temperature ($T_e$) of 7000 K and amplitudes ($\vec{t}_\mathrm{ff}$) and Gaunt factor ($g_\mathrm{ff}$) from the Beyond Planck analysis \cite{bp01}. $a_\mathrm{near}$ and $a_\mathrm{H\alpha}$ are amplitude scale value applied to templates ($\vec{t}_\mathrm{near}$ based on \cite{edenhofer:2024} and $\vec{t}_\mathrm{H\alpha}$ based on \cite{wham:2003,2016WHAM}) for the near dust ($\mathrm{near}$) and \Ha\ correlated (\Ha) dust respectively.   
...
\begin{figure*}[htbp]
    ...
    \caption{Contour plot of relevant statistical tests and $\chi^2$ for (clockwise from top left) cold dust, hot dust, \Ha\ dust and near dust. The black dot shows the fiducial point used for all parameters. Maps of a subset of the grid points can be found in \cref{app:GridSearchTests}. }
...
One of the great successes of this four-component dust model is the high degree of correlation between the \ion{H}{i} emission, a known dust tracer, and the cold dust, with a Pearson's correlation coefficient of 93\%, as well as the correlation between the new dust tracer \ion{C}{ii}, which traditionally maps the hot ionized medium but is found here to trace the hot dust component with a correlation of 94\% \cref{fig:hotCiiCorr} shows per-pixel scatter plots between our dust maps and these tracers. The top panel shows a the hot dust and Firas \ion{C}{ii}, downgraded to the $7^\circ$ resolution of the Firas maps, and the bottom pane shows the cold dust and HI4PI column density maps at $N_{\mathrm{side}}=1024$. Both show strong correlation, which is further evidence that they trace similar regions of the Galaxy. 
...
So far we have fitted four dust components to the Planck data, each
modelled as a modified black-body with free spectral parameters $T$
and $\beta$ that are non-varying on the sky. We now want to compare
this to the \textit{Planck} 2015 analysis where only one MBB
dust component was fitted to the Planck data, but allowing for spatial
variations of $T$ and $beta$. The Planck 2015 maps are plotted at the
top of  \cref{fig:beta_T}. Here both $T$, $beta$ and the dust
amplitude are fitted per Healpix nside=256 pixel using
\texttt{Commander 1} \cite{eriksen2008}.\footnote{\texttt{Commander 1}
is distinct from \texttt{Commander 3} in that it uses a per-pixel
Gibbs sampling routine, compared to the \texttt{Commander 3}, which
mixes harmonic and map space analysis.}.
While $beta$ is relaitvely flat on the sky, we see that $T$ varies
significantly. In this analysis (ref Planck X 2015) CIB was not fitted
separately, and the CIB fluctuations mostly went into the thermal dust
component. The hot areas on high latitudes corresponds to CIB mixing in the
dust component.

(ref Planck GNILC) fitted separate CIB and thermal dust components to
the Planck data using GNILC, a needlet ILC code with power spectrum
priors. Also here the thermal dust component was modelled as a
1-component MBB with variable $T$ and $beta$ as plotted in the middle
of \cref{fig:beta_T}. Compared to the Commander maps above, the CIB
pollution is now gone from the $T$ map, but the $beta$ maps is varying
more on the sky.

In the bottom of \cref{fig:beta_T} we have used Commander1 to fit 
a 1-component MBB with varing spectral indices to the 4-component dust
model with spatially flat spectral parameters developed in this
paper. We see that an underlying simple 4-component dust model appears
very similar as a 1-component model with a close to constant $beta$ and a varying
temperature $T$. Except for at high latitudes, where Commander 2015
$T$ is CIB dominated, while the Planck maps in this analysis are CIB
subtracted, the two $T$ maps agree very well. This shows that the
effective 1-component MBB from Planck 2015 can instead be replaced
with the new simpler 4-component model with fewer free parameters.  
