...
\end{itemize}
We build off the work in \cite{CG02_05}, by using several additional data sets, discussed in \cref{sec:data}, and using two techniques, Gibbs sampling, discussed in \cref{sec:sampling}, and a parameter grid search, discussed in \cref{sec:}. 
Additionally, we will discuss the sky model and the data model used for this analysis in \cref{sec:skymodel} and \cref{sec:model}.


\subsection{Data}
\label{sec:data}
In \cite{CG02_05} we compute a linear fit to the DIRBE data, finding COOL STUFF. \RS{Add some details from the DR2-05 paper here.}
To build on this work we will use the \planck\ PR4 data, several FIRAS maps, and additional ancillary data to best break degeneracies and constrain the new dust model. We discuss some of the masks used in \cref{sec:masks}. 
...
Specifically, the FIRAS maps are used when calculating the $\chi^2$ during one of the Gibbs loops to determine whether a step will be accepted or not, if the $\chi^2$ calculated using the FIRAS maps is larger than the update is rejected. 
...
Additionally, we use the Wisconsin \ion{H}{$\alpha$} mapper data as a template for the H$\alpha$ correlated dust, and the results from \citep{CG02_05} as the fiducial values for the H$\alpha$ dust emission, since it is relatively poorly constrained by the \planck\ bands (see \cref{sec:halpha}). The \ion{H}{$\alpha$} signature comes from the warm ionized medium (WIM) of the Milky Way, and maps a small extinction component in the \Planck\ bands. 
As mentioned above, we additionally subtract the GNILC CIB maps \citep{planck2016-XLVIII} and the \Planck\ PR2 zodiacal light model \cite{planck2014-a09, planck2013-pip88, maris2006c} from the \planck\ HFI maps. 
...
\begin{align}
    d_j=g_j\left[\mathsf{B}_{j}^\mathrm{symm}\sum_s \mathsf{M}_{sj}(\beta_p,\Delta_{j,\mathrm{bp}})a_{p,j}\right]+n_{j}
    \label{eq:dataModel}
\end{align}
where $g_j$ are the instrumental gain for detector $j$, $\mathsf{B}$ the beam convolution, $\mathsf{M}_{sj}(\beta_p,\delta_j)a_{p,j}$ the foregrounds mixing matrix with spectral parameters $\beta$, bandpass specifications $\Delta_{j,\mathrm{bp}}$ and amplitudes $a_{p,j}$ per pixel $p$ for the various sky components $s$ as discussed in \cref{sec:skymodel}. 
...
\end{itemize}
As mentioned in \cref{sec:planckhfi}, the \planck\ HFI bands include several relevant foregrounds in addition to dust, such as free-free (ff), carbon monoxide (CO), cosmic microwave background radiation (CMB) (see \cref{fig:SkyModel}). 
For this analysis we fix the free-free fixed to  due to the low constraining power of the HFI bands on the free-free component. The template we use has electron temperature ($T_e$) of 7000 K and amplitudes set from the Beyond Planck analysis \cite{bp01}.\RS{Is this correct?}%(init_ff_amp_BP8.1_v1.fits) 
Additionally, we keep the CO line-ratios constant for this analysis and set to the ratios from the \planck\ results \cite{planck2013-p03a}, but allow the amplitude to vary pixel-by-pixel for the three CO emission lines. \RS{Is this correct? Do we have more up to date CO line ratios from later data releases?}
For our four dust components, as mentioned before the hot and cold dust are each allowed to have free amplitudes for each pixel, and global\footnote{Here global refers to a single value for the entire map.} temperature and spectral index values. The nearby dust and dust extinction have fixed templates (as mentioned in \cref{sec:otherdata}), but are allowed to vary with a global amplitude, temperature, and spectral index. 
...
\begin{align}
    ...
    \label{equ:model}
\end{align}
where $u_\nu$ is a unit scaling conversion factor to go from brightness temperature to thermodynamic temperature, %$g_\nu$ is the gain (which in this case is only sampled for 857 GHz), $\mathsf{B}_\nu$ is the beam convolution operator,
$\vec{a}_{d}$ for $d\in (\mathrm{ff,c,h,near,H\alpha,co})$ are the amplitudes, which are sampled per-pixel for the CMB ($\mathrm{cmb}$), cold dust ($\mathrm{c}$), hot dust ($\mathrm{h}$), and the CO lines, and as a spatially constant value for the nearby dust ($\mathrm{near}$) and H$\alpha$ correlated dust ($\mathrm{H\alpha}$). $m_\nu$ are the monopoles and dipoles per frequency \RS{Should this be included here? Monopoles are not real signals, dipoles maybe should be just included as a CMB signal, so maybe monopoles are more of a thing just for the data model?}. The free-free ($\mathrm{ff}$) is held fixed, as well as the CO line ratios ($h_\nu^{\{100,217,353\}}$). 
Synchrotron is not included in this analysis since it is negligible in the HFI frequencies. 
\RS{ Should maybe move the sky model plots up here. }


\subsection{Gibbs sampling}
\label{sec:sampling}
\begin{itemize}
    \item Big Gibbs sampling equation
...
Gibbs sampling is a Monte-Carlo sampling technique useful for very high dimensional parameter spaces, however suffers in the case where there are highly correlated parameters (as do most Monte Carlo sampling techniques). As such, we will only employ Gibbs sampling for some of the parameters, and instead do a manual grid search technique to efficiently sample the parameter space.
 
For the Gibbs sampling, we employ the algorithm,
\begin{align}
{\bf a}_i &\leftarrow P({\bf a}_i |{\bf d}, \phantom{{\bf a}_i ,},g_\nu,m)\\
g_\nu &\leftarrow P(g_\nu |{\bf d}, {\bf a}_i,\phantom{g_\nu,}m)\\ 
m &\leftarrow P(m |{\bf d}, {\bf a}_i,g_\nu,\phantom{m}) 
\end{align}
where the amplitudes for the maps (as mentioned in \cref{fig:SkyModel}) are sampled while holding the 857~GHz gains and monopoles and dipoles fixed, and proceding similarly through the rest of the sampled parameters.
\RS{Should I also have Cls for the CMB? That is also fit but maybe fine to lump it in with the amplitudes from the sky model?}
\RS{Add references} 
...
We found that the four dust components were highly correlated, and so we performed an extensive grid search to explore the parameter space efficiently, while imposing a physicality prior requiring that the large scale amplitudes of the hot and cold dust be positive, and that the temperatures of the dust components did not exceed 35~K or go lower than 10~K. Additionally, the nearby dust and the H$\alpha$ dust should be uncorrelated with the hot and the cold dust, as much as possible. A highly correlated hot and nearby dust model would indicate that either the nearby dust is too low, leaving much of the nearby dust to leak into the hot dust for example. Lastly, we expect that the cold dust and the \ion{H}{I} signal as seen by HI4PI should be more highly correlated, so we keep that in mind while exploring the parameter space (though we do not force such a correlation). 

Each step in the grid search used Gibbs sampling fit the 857-{1-4} gains, the amplitudes per pixel of the hot and cold dust, the CMB and the CO amplitudes, as well as monopoles and dipoles, for a set temperature and spectral index for each dust component. These amplitudes are fit through the conjugate gradient sampling routine implemented in the Commander3 framework and as described above for a set $\beta$ and $T$ for each dust component and a set amplitude scale for each of the templates. 
...
The grid search results are summarized in \cref{fig:SEDGridContours} and \cref{fig:SEDGridAmplitude}. We choose fiducial values based on the criteria as described above, and show how when each dust component is set to its fiducial value and only one allowed to vary how the space explored changes (see also \cref{app:GridSearchTests}). The gray grids show areas that were unphysical (with large negative hot or cold dust fits). We additionally plot the correlation with the hot and the cold dust compared to the templates for the nearby and H$\alpha$ correlated dust, where the aim is to have the lowest overall correlation between those components. 
We found that varying the spectral parameters of the hot or cold dust had more effect on the final amplitudes of the other, for example, as the hot dust changed in temperature, the final amplitudes of the hot dust did not change drastically, however the cold dust amplitudes did change dramatically in response. As such, we have included in the Hot dust grid the correlation between the cold dust and the HI4PI maps, and interestingly the line tracing the 92\% correlation traces closely with the unphysical region, and a similar line can be drawn when we look at the correlation between the hot dust and the Firas CII map, with the 95\% correlation region tracing the unphysical areas. 
Broadly speaking we have very little constraining power on the H$\alpha$ correlated dust with the \planck\ HFI bands. Since it represents a very small contribution to the signal we choose to set the value for the $T_{\mathrm{H\alpha}}$ and $\beta_{\mathrm{H\alpha}}$ to those from the DIRBE results \citep{CG02_05}. Despite exploring a wide parameter space for those and the amplitude scale factor we found that the $\chi^2$ as well as the correlations with the hot and cold dust were relatively robust. 
...
\begin{figure}
    ...
    \caption{$\chi^2$ as a function of nearby (\emph{top}) and $H\alpha$-correlated (\emph{bottom}) dust amplitude scale.}
...
\noalign{\vskip 4pt\hrule\vskip 6pt}
\noalign{\vskip 3pt}
Cold dust& $ 11.3 $ \RS{$11\pm2$} & $ 1.85 $ \RS{$1.85\pm0.15$}\cr
\noalign{\vskip 3pt}
Hot dust& $ 30.0 $ \RS{$30\pm5$} & $ 1.75 $\RS{$1.75\pm0.2$or+0.2-0.05}\cr
\noalign{\vskip 3pt}
H$_\alpha$ dust& $ 17.80 $\RS{???} & $ 2.62 $ \RS{???}\cr
\noalign{\vskip 3pt}
Nearby dust& $ 18.0 $ \RS{$18+2-1$}& $ 1.7 $ \RS{$1.7\pm0.1$}\cr
\noalign{\vskip 3pt}
\noalign{\vskip 3pt\hrule\vskip 4pt}}}
\endPlancktable
\endgroup
...
\cref{fig:dustSED} the spectral energy distribution of the dust components. \RS{Need to choose final values and error bars!!!}}
\label{tab:SEDs}
\end{table}
...
% \begin{figure*}[t]
%     \centering
% \includegraphics{figures/dust_Ha_map.pdf}
% \includegraphics{figures/dust_near.pdf}
% \includegraphics[width=0.65\linewidth]{figures/dustComp_coldDust.pdf}
% \includegraphics[width=0.65\linewidth]{figures/dustComp_hotDust.pdf}
% \includegraphics{figures/dustComp_Himap.pdf}
% \includegraphics{figures/dustComp_firasCII.pdf}
%     \caption{}
%     \label{fig:dust_figures}
...
One of the great successes of the 4-component dust model is the high degree of correlation between the \ion{H}{I} emission, a known dust tracer, and the cold dust, at 93\% and the correlation between the new dust tracer \ion{C}{II}, which traditionally maps the hot ionized medium and traces well the hot dust component, with a correlation of 95\% (see also \cref{fig:hotCiiCorr}). 
\Cref{fig:correlationSquare} shows the spatial correlations between the different dust components and the tracers, and we can see that we have a low degree of correlation between the near dust and the hot and cold dust. We have additionally included Dame in this check \cite{dame2001}, which maps the galactic carbon dioxide emission lines, however we do not find a high degree of correlation with other dust tracers. 
...
\begin{figure*}
    \centering
    \includegraphics{figures/commander1_dustComp.pdf}
    \caption{Beta and T fits. \RS{Confirm map v1 vs v2 with Ingunn}}
    \label{fig:beta_T}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics{figures/coldDust_cold_Grid_ratio.pdf}
    \caption{ratio}
...
\begin{figure*}
    \centering
    \includegraphics{figures/hotDust_hot_Grid_ratio.pdf}
    \caption{ratio}
    \label{fig:cold_cold_dust_set2}
\end{figure*}
...
\begin{figure*}
    ...
    \caption{Cold dust amplitudes for varying $T_\mathrm{H\alpha}$ and $\beta_\mathrm{H\alpha}$ values with all other parameters set to the fiducial values. The central panel is the fiducial map. The corresponding hot dust maps can be seen in \cref{fig:Ha_hot_dust_set}. This explores only $a_\mathrm{H\alpha}=1$.}
...
\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{figures/dustFrac_plcmap.pdf}
