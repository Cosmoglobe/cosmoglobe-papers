...
\subsection{Grid search and optimization strategy}
\label{sec:MethodGridSearch}
To address the high level of correlation between the various dust models we find that a grid search was optimal. Allowing the amplitudes to vary freely within the Gibbs sampling algorithm tended to favor unphysical results and so with the grid search we can apply a so-called physicality prior to the data, while also attempting to minimize the $\chi^2$. We chose our physicality prior from several reasonable assumptions. Firstly, we expect that the hot and cold dust should have positive large scale modes, or in other words parameters that lead to the hot or cold dust being largely negative are deemed unphysical. Secondly, in addition to a low $\chi^2$, the nearby dust and the H$\alpha$ dust should be uncorrelated with the hot and the cold dust, as much as possible. A highly correlated hot and nearby dust model would indicate that either the nearby dust is too low, leaving much of the nearby dust to leak into the hot dust for example. Lastly, we expect that the cold dust and the \ion{H}{I} signal as seen by HI4PI should be more highly correlated, so we keep that in mind while exploring the parameter space (though we do not force such a correlation). 
