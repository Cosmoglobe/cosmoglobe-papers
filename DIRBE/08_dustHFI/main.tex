...
The rest of the paper is organized as follows. In \cref{sec:method} we discuss broadly the methods used to produce the new sky models, including in \cref{sec:data} the data use, in \cref{sec:model} the data model, in \cref{sec:skymodel} the sky model, in \cref{sec:sampling} Gibbs sampling and in \cref{sec:statistics} the summary statistics analyzed to determine the best fits. Next, in \cref{sec:results} the best fit results for our new thermal dust model. In \cref{sec:gof} we discuss the goodness of fit of the model, and in \cref{sec:correlations} the correlations with external line emission maps. In \cref{sec:mbb} we compare to the previous \planck\ results and show the MBB fit to the multi-component dust model. Finally, in \cref{sec:conc} we will present our conclusions. In \cref{app:residuals} and \cref{app:GridSearchTests} we include supplementary details regarding the residuals and additional tests and figures for clarity.\RS{UPDATE WHEN DONE} 
...
Additionally, we will discuss the sky model and the data model used for this analysis in \cref{sec:model} and \cref{sec:skymodel}.
...
\label{sec:planckhfi}
The \planck\ PR4 (also known as \npipe) maps are the final \planck\ data release, improving on previous data releases by jointly analyzing the low-frequency instrument (LFI) and the  high-frequency instrument (HFI) data in a single pipeline, in addition to several other improvements (for details see \cite{planck2020-LVII}).\RS{add PLA link and healpix}
For this analysis we use specifically the HFI data because of its strong constraining power on the dust. We use the single-bolometer maps to better control the gain fluctuations and better model components such as the CO-emission. Thus, we have 4 channels for both the 857~GHz and 100~GHz frequencies, 3 for 545~GHz, 8 for 353 and 217~GHz and 7 for 143~GHz.\footnote{The 100~GHz and 143~GHz maps are at an \nside\ of 2048, the other frequencies are at an \nside\ of 4096.} 
We first correct all maps for the zodiacal light emission, using the \Planck\ PR2 zodiacal light model \citep{planck2014-a09, planck2013-pip88, maris2006c}, then apply a correction for the cosmic infrared background (CIB) to the 857, 545 and 353 GHz maps from the GNILC CIB results \cite{planck2016-XLVIII}.
We apply a scaling correction to the root-mean squared (RMS) maps, such that the tail of the spectra matches the white-noise level from the RMS maps since the published individual bolometer frequency RMS maps have residual offsets from the analysis pipeline, evident when comparing the map and RMS power-spectra. 
...
We sample for the 857~GHz gain but all other gains and bandpass' are fixed to the \NPIPE\ values. 
...
\label{sec:otherdata}
To model the near dust component we use the template produced by \citet{edenhofer:2024}, which uses the star reddening from the \GAIA\ and \WISE\ star catalogues to map the dust out to 1.25~kpc from the Sun. This provides a spatial template for the nearby dust, to which we can fit an amplitude, temperature, and spectral index matching a blackbody.
Additionally, we use the Wisconsin H-$\alpha$ mapper data as a template for the \Ha\ correlated dust, and the results from \citep{CG02_05} as the fiducial values for the \Ha\ dust emission, since it is relatively poorly constrained by the \planck\ bands (see \cref{sec:halpha}). The \Ha\ signature comes from the warm ionized medium (WIM) of the Milky Way, and maps a small extinction component in the \Planck\ bands. 
...
When considering our summary statistics in \cref{sec:correlations} we compare the new dust templates with the \ion{C}{ii} line emission as measured by the FIRAS \ion{C}{ii} map, and to the HI4PI data \citep{HI4PI2016}. 
...
$\vec{a}_{s}$ for $s\in (\mathrm{ff,c,h,near,H\alpha,co})$ are the amplitudes, which are sampled per-pixel for the CMB ($\mathrm{cmb}$), cold dust ($\mathrm{c}$), hot dust ($\mathrm{h}$), and the CO lines, and as a spatially constant amplitude scale value applied to templates for the nearby dust ($\mathrm{near}$) and \Ha\ correlated (\Ha) dust. We keep the free-free ($\vec{a}_\mathrm{ff}$) fixed due to the low constraining power of the HFI bands on the free-free component. The template we use has electron temperature ($T_e$) of 7000 K and amplitudes set from the Beyond Planck analysis \cite{bp01}. 
Additionally, we keep the CO line-ratios ($h_\nu^{\{100,217,353\}}$) constant for this analysis from the \planck\ results \cite{planck2013-p03a} \RS{Is this correct? Do we have more up to date CO line ratios from later data releases?}, but allow the amplitudes ($\vec{a}^{\{100,217,353\}}_\mathrm{co}$) to vary pixel-by-pixel.  
Finally, the monopoles and dipoles ($m_\nu$) are sampled for each frequency band. \RS{Should this be included here? Monopoles are not real signals, dipoles maybe should be just included as a CMB signal, so maybe monopoles are more of a thing just for the data model?}. As mentioned in \cref{sec:planckhfi} we have corrected the maps for the CIB and zodiacal light, so these are not included here. 

\begin{figure}[tbp]
    \centering
    \includegraphics{figures/cmb_map.pdf}
    \includegraphics{figures/ff_map.pdf}
    \includegraphics{figures/co100_map_nc.pdf}
    \includegraphics{figures/co217_map_nc.pdf}
    \includegraphics{figures/co353_map.pdf}
...
Gibbs sampling is a Monte-Carlo sampling technique useful for very high dimensional parameter spaces, however suffers in the case where there are highly correlated parameters (as do most Monte Carlo sampling techniques). As such, we will only employ Gibbs sampling for per-pixel parameters and instead do a grid search for the global spectral parameters.
...
\end{align}
where the amplitudes for the maps (as mentioned in \cref{fig:SkyModel}) are sampled while holding the 857~GHz gains and monopoles and dipoles fixed, and proceeding similarly through the rest of the sampled parameters.
...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/AvgDust_Grid_plotres.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/efficiency_avg.pdf}
...
One of the great successes of the 4-component dust model is the high degree of correlation between the \ion{H}{I} emission, a known dust tracer, and the cold dust, at 93\% and the correlation between the new dust tracer \ion{C}{II}, which traditionally maps the hot ionized medium and traces well the hot dust component, with a correlation of 94\% (see also \cref{fig:hotCiiCorr}). 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/dust_corr_sm.pdf}
    \caption{(\emph{Left:} Scatter plot between the hot dust and FIRAS \ion{C}{ii} maps; both maps are smoothed to a common resolution of 420' FWHM, and pixelated at \nside=16. \emph{Right:} Scatter plot between the cold dust and HI4PI maps; both maps are smoothed to a common resolution of 16.2' FWHM, and pixelated at \nside=1024.%\RS{full size of each is also available, but not sure it adds much info compared to this smaller version - if full size preferred uncomment below}
    }
...
\begin{figure}[htbp]
    \centering
    \includegraphics{figures/correlation_plot.pdf}
    \caption{Correlation between dust components and tracers. The correlation between the hot dust and the \ion{C}{ii} emission seen in FIRAS, as well as the cold dust and the HI4PI HI emission are very strong.
...
\label{sec:mbb}
We fit a spatially varying 3-parameter MBB to our new four-component model and find very similar spatial distributions of $T$ and $\beta$ as in the official \textit{Planck} analysis, but with a smoother $\beta$ map, as well as less correlation between the dust $\beta$ maps to previous \planck\ thermal dust maps (see \cref{fig:beta_T}). This analysis was done using Commander 1 \cite{eriksen2008}.\footnote{Commander 1 is distinct from Commander 3 in that it uses a per-pixel Gibbs sampling routine, compared to the Commander 3, which mixes harmonic and map space analysis to.}
...
\begin{figure*}[htbp]
    ...
    \caption{Ratio of the cold dust amplitudes compared to the fiducial value (central panel) for varying $T_\mathrm{c}$ and $\beta_\mathrm{c}$ values with all other grid parameters set to the fiducial values as recorded in \cref{tab:SEDs}. The corresponding hot dust maps can be seen in \cref{fig:cold_hot_dust_set}.}
    \label{fig:cold_cold_dust_set2}
\end{figure*}

\subsection{Hot dust}
Similar to with the cold dust, in \cref{fig:SEDGridContours}, there is an unphysical space mapped by the coldest and lowest spectral index in the hot dust grid. This corresponds to large over-subtraction and negative areas in the cold dust amplitudes as can be seen in \cref{fig:hot_cold_dust_set}. Also similar to the cold dust, when shifting the $T_h$ and $\beta_h$ we find the shape of the amplitude maps are visually similar to the fiducial map (see \cref{fig:dust_figures}), so we show the monopole subtracted ratio of the grid position to the fiducial value in \cref{fig:hot_hot_dust_ratio}.
% \begin{figure*}
%     \centering
...
\begin{figure*}[htbp]
    ...
    \caption{Ratio of the hot dust amplitudes compared to the fiducial value (central panel) for varying $T_\mathrm{h}$ and $\beta_\mathrm{h}$ values with all other grid parameters set to the fiducial values as recorded in \cref{tab:SEDs}. The corresponding cold dust maps can be seen in \cref{fig:hot_cold_dust_set}}
