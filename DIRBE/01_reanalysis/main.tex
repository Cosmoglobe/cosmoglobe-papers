...
The sky model is described in detail by \citet{CG02_04,CG02_04}, and
...
is defined by \citet{CG02_04,CG02_05}, and reads as follows in units
...
\begin{alignat}{4}
  ...
  \textrm{(Bright stars)} \nonumber\\
  + &U_{\mathrm{mJy}} \t_{\mathrm{GAIA,fs}}\, a_{\mathrm{fs},\nu}, &\quad&
  \textrm{(Faint stars)} \nonumber\\  
...
\citep{edenhofer:2024}.  As such, this dust model has in fact only two
...
The dense frequency spacing of the FIRAS data makes it ideal for determining the continuum behavior of sky emission and allows for identification of emission lines \citep{bennett:1994}. For the purposes of this analysis, a subset of the FIRAS bands, 14 FIRAS bands, listed in Table \ref{tab:firas_bands}.
...
We ran six independent Gibbs chains, for (a,b,c,d,e,f) samples each. Inspecting the best-fit parameters, as shown in Fig.~\ref{fig:trace}, we determined the chains to be approximately burned in after 25 samples. We performed zodiacal dust searches every other sample, so the chains are thinned by a factor of two. In total, this gave (g) samples in total.

We can approximately map out the correlations between parameters by computing a correlation matrix, displayed in Fig.~\ref{correlations}. This shows that there are correlations between zodiacal emission and the total monopole, as well as with certain white noise estimates.
...
K98 ZL model, namely the so-called circum-solar ring and the
...
circum-solar ring and Earth-trailing feature. Indeed,
...
on a visualization that still included the circum-solar ring and
...
Fig.~\ref{fig:solarmaps}.

With the current pixelized maps in hand, it is possible to construct
more targeted masks that exclude only the contaminated data. The masks
adopted for the current production run are shown as gray boundaries in
Fig.~\ref{fig:solarmaps}; any pixels outside these boundaries are
excluded from the final ZSMA maps.\footnote{The current masks were
made by thresholding similar maps from a previous unconverged test
run. A future version should start from these converged runs.}

While this eliminates the worst affected data, there are still many
significant excess 
