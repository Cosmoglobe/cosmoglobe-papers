...
\begin{align}
	...
	\dv &=\G\P\B\sum_{c=1}^{n_{\mathrm{comp}}}\M_c\a_c+\s_{\mathrm{zodi}} +
        ...
        &\equiv \s_{\mathrm{tot}} + \n_{\mathrm{w}}.
\end{align}
Here $\dv$ denotes a stacked vector of all DIRBE TOD for all frequency
bands, $\G$ is an $n_{\mathrm{tod}}\times n_{\mathrm{tod}}$ diagonal
gain calibration matrix, $\P$ denotes a satellite
pointing matrix, which we define in Galactic coordinates, and $\B$
denotes an instrumental beam (or point spread function) convolution
operator.  The sum runs over $n_{\mathrm{comp}}$ astrophysical
...
component, taking into account the bandpass of each detector. The term
$\s_{\mathrm{zodi}}$ is a model of zodiacal light emission from
components that appear time-variable as seen from Earth (e.g., the
zodiacal cloud and asteroidal bands), $\s_{\mathrm{static}}$ is an
excess signal that appears stationary with respect to the Earth-Sun
system, discussed further in Sect.~\ref{sec:excess},
$\n_{\mathrm{corr}}$ is correlated instrumental noise (which for now
is only fitted for the lowest DIRBE frequency channel), and
...
As we work with calibrated TOD, we set $\G=\tens{I}$ for all channels except 12 and $25\,\mu$m for now, but
...
overlap frequency range between FIRAS and DIRBE. For the 12 and $25\,\mu$m channels, we sample one overall constant value for each operation day under the constraint that the mean averages to unity, using the algorithm described by \citet{bp07}. Similarly, both the
...
The sky model is described in four companion papers \citep{CG02_04,CG02_05,CG02_06,CG02_07}, and
...
there are {36} free parameters in this model, and these are collectively
denoted $\zeta_{\mathrm{z}}$. {Some parameters, including the circumsolar ring and Earth-trailing feature, are fixed to the K98 model. Such choices are further described in \citet{CG02_02}.}
...
this length. For the {140 and} 240\,$\mu$m \textbf{channels} we additionally include a
...
is defined by \citet{CG02_04}, \citet{CG02_05}, \citet{CG02_06}, and \citet{CG02_07}, and reads as follows in units
...
\begin{alignat}{4}
 ...
 && \textrm{(Nearby dust)} \nonumber \\
  ...
  + &m_{\nu} && \textrm{(Monopole)}. \nonumber
\end{alignat}
In this expression, $\t$ denotes an external fixed spatial template, and $q_i$ (where relevant) represents its amplitude. The four first terms indicate four physically distinct thermal dust tracers, as discussed by \citet{CG02_05}: The first and second components trace cold \ion{H}{i} correlated dust and hot \ion{C}{ii} correlated dust with temperatures around $T\sim16\,\mathrm{K}$ and 26\,K, respectively, and we adopt the \Planck\ HFI-based templates presented by \citet{CG02_06} for these. The third component corresponds to nearby dust (located closer to the Sun than 1.25\,kpc), and we adopt the \GAIA-based dust extinction template presented by \citet{edenhofer:2024} for this. Finally, the fourth component corresponds to dust extinction as traced by the WHAM H$\alpha$ sky map \citep{wham:2003}; for a full discussion of this effect, see \citet{CG02_07}. It is important to note that this dust model has, in fact, no degrees of freedom per pixel as far as the current DIRBE-based analysis is concerned. 

%HKE: Removed, since it duplicates the above paragraph.
%As indicated by the above sky model, we fit a novel {four-component}
%generalized modified blackbody model to account for thermal dust
%across the combined DIRBE and \Planck\ HFI frequency range;
%for full details, see \citet{CG02_05}, \citet{CG02_06}, and \citet{CG02_07}. The {four} components correspond
%to cold dust, hot dust, and nearby dust emission, respectively, {as
%well as dust extinction traced by the WHAM H$\alpha$ sky map \citep{wham:2003}}. All
%four are modeled with spatially constant spectral parameters, and
...
%constant spectral energy distribution (SED) parameters. This is an extremely economical model of
...
$g_{\mathrm{ff}}(\nu; T_\mathrm{e})$ \citep{dickinson:2003,draine:2011}, which
...
electron temperature, $T_\mathrm{e}$. For now, however, we adopt the $T_\mathrm{e}$
distribution presented by \citet{planck2014-a11} as given, and will
rather attempt to actually fit $T_\mathrm{e}$ in future work.


The {bright stars} component corresponds to \textbf{bright} starlight emission, which
is relevant in the frequency range between 1 and 25\,$\mu$m; for full
details regarding this model, see \citet{CG02_04}. For {this component}, we first \textbf{crossmatch the AllWISE pointsource catalog \citep{CatWISE} against \Gaia\ DR2, obtaining corresponding entries for almost all stars brighter than magnitude 7 in the W1 band. Then, we construct a unique set of the brightest stars that are no closer together than 5', which gives us a set of 424\,829 bright point sources that includes most of the brightest sources on the sky, but which will not be degenerate with one another when sampled.} For each of these, we store the best-fit
...
bands are not used for the actual fit. {These individually fitted sources are denoted as ``bright
...
The AllWISE catalog itself contains a total of about 711 million
...
extinction correction is applied to the faint stars.} Together, these {two} source components comprise an
unprecedented deep model for compact objects in DIRBE that has only
become possible due to WISE and \GAIA.

The final component is simply a monopole per frequency. For
...
the current sky model, which is negligible in the DIRBE bands.
...
diverging into pathological solutions. Second, for a few select astrophysical components, for
...
component, we refer the interested reader to \citet{CG02_04} and \citet{CG02_07}.
...
form of a template amplitude (such as the cold dust amplitude
$q^{\mathrm{cold}}$) or a catalog (such as the bright star amplitude
...
DIRBE TOD already exists, several of the steps and models discussed
...
sampling for DIRBE is described by \citet{CG02_07} and the novel
...
The DIRBE scanning beam has a rather unusual squared shape, caused by Field Stop no.~2 in Fig.~\ref{fig:optics_model}. Fully accounting for this shape in each sample is computationally very expensive. Instead, following experiments such as \WMAP\ \citep{bennett2012} and \Planck\ \citep{planck2016-l01}, we operate with the azimuthally symmetrized beams shown in the top panel of Fig.~\ref{fig:beams}. These are obtained by radially averaging the original 2D DIRBE beams around their center-of-mass. In general, these radial beam profiles resemble a tophat with a slow falloff, and the corresponding harmonic-space representations, $b_{\ell}$, are therefore reminiscent of a sinc function, with oscillations about zero above $\ell\gtrsim500$; these are obtained through a Legendre transform of the radial beams, and the results shown in the bottom panel. For a band-limited signal to be characterized fully in map space, the 42\arcm\ DIRBE beams must be represented with a 21\arcm\ or smaller pixelization. However, in order to be fully characterized in harmonic space, a requirement for the \commanderthree\ multi-resolution component separation, the pixelization scheme must have support up to $\ell\lesssim1500$. The original DIRBE maps have pixel size of 21\arcm, insufficient for harmonic space analysis.
...
pixelization (resolution 9, pixel size 21\arcm) nor the CADE\footnote{Centre d'Analyse de Données Etendues, \url{http://cade.irap.omp.eu/dokuwiki/doku.php?id=dirbe}} reprojection into HEALPix\footnote{\url{http://healpix.sourceforge.net/}} ($N_\mathrm{side}=256$, pixel size 13\farcm7) have the required support over the full multipole range. 
...
{However, the same does not hold true in general, as there are
...
With the model in hand, we run \commanderthree\ on the data. We find that an average Gibbs iteration takes approximately 250\,CPU-hrs, excluding initialization time. In order to ensure proper sampling of the full distribution, we run {five} independent \commanderthree\ runs initialized with their own random seeds. \textbf{The chains have 244, 246, 246, 242, and 246} samples, giving a total of \textbf{1224} samples. A total of one month walltime on 1920 computing cores was required to produce these samples.

In order to assess burn-in and convergence, we plot a range of parameters from the Gibbs chain in Fig.~\ref{fig:trace}. Included in this figure are the sampled monopoles, zodiacal dust albedos and emissivities, and instrumental parameters, including the estimated white noise in bands 1 and 10, and the correlated noise parameters $f_\mathrm{knee}$ and $\alpha$ for band 10. From this figure, some burn-in can be seen in the monopoles, concluding at approximately sample 20.  At the same time, there is still drift within the emissivity and albedos, especially for parameters less strongly expressed in the model, such as $A_{4,\mathrm C}$ and $E_{9/10,\mathrm C}$. However, since the monopoles are stable beyond this point, this is mainly due to degeneracies within the zodiacal model itself. This is explored in full detail in \citet{CG02_02}.

{Depending on the parameter of interest, 30--50 samples are considered burned in. This gives us a total of 974--1074 total samples, depending on the parameter.}
...
in the form of posterior mean and root-mean square (RMS) estimates for each sampled
...
\begin{figure}
	...
	\caption{Instrumental noise $\sigma_0$ for each band, averaged over all Gibbs chains. Data not used in the Gibbs chains are not displayed. Bands $12$--$240\,\mathrm{\mu m}$ are plotted in units of $\mathrm{MJy\,sr^{-1}}$, while all others are in $\mathrm{kJy\,sr^{-1}}$.}
	\label{fig:sigma0}
\end{figure}
...
\Planck\ LFI. 
{A preliminary analysis of available housekeeping data (J.~Weiland, priv.~comm) does not
...
channels is their much higher overall noise level, which is almost
...
noise power spectral density as well as noise realizations to be fit and removed in the
...
RMS as a function of position on the sky including the
...
strategy as such, but rather by the DIRBE quality flags which remove
...
defined in Sect.~\ref{sec:excess}, which remove a significant
fraction of the overall data, and affect some parts of the sky more
...
under-subtracted. {The relatively larger degree of oversubtraction at 1.25$\,\mathrm{\mu m}$ is due the emissivity, which is applied uniformly for all stars. In practice, this overestimates the effect of extinction in the highest density portions of the Galactic plane.} In this respect it is important to note that the
starlight model presented by \citet{CG02_04} is based on the {AllWISE}
...
{AllWISE} catalog is, however, much higher at high Galactic latitudes than
...
independently from that of the diffuse cloud component; however, the S/N  of
...
{Recall that each of the dust components are fit to a template, with a global amplitude per frequency channel. It is therefore expected that more high-resolution data in the infrared, such as from \AKARI, \IRAS, and SPHEREx, will allow
}
for refining the current
model without compromising the overall S/N and
...
component into more local clouds \citep{CG02_05,CG02_06,CG02_07}.
...
noise level, which indicates that the unmodeled features are about one
...
the DIRBE beam. {Indeed, except for the 1.25 and 2.2\,$\mu$m
...
monopole and clear ZL band residuals. Other minor exceptionis include the
...
than K98, and in bands $2.2$ and $3.5\,\mathrm{\mu m}$,} K98 has a brighter Ecliptic plane than DR2.
It is very
...
visual ambiguity. Here, the K98 map exhibits
clear ZL residuals while the DR2 model is visually clean. The
same holds even more so at 12$\,\mu$m.
...
\begin{acknowledgements}
  We thank the anonoymous referee for insightful and useful comments that signifiantly improved this analysis.
