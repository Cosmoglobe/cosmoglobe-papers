...
With the model in hand, we run \commanderthree\ on the data. We find that an average Gibbs iteration takes approximatly 500\,CPU-hrs, excluding initialization time. In order to ensure proper sampling of the full distribution, we run \textbf{two} independent \commanderthree\ runs initialized with their own random seeds. \textbf{The chains have 624 and 627} samples, giving a total of \textbf{1251} samples. A total of one month walltime on 416 computing cores was required to produce these samples.
