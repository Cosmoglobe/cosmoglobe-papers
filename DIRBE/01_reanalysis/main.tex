...
blackbody spectrum. \textbf{In the current analysis, we use the smooth modified blackbody form for \Planck\ HFI, and the discrete binned form for DIRBE.} Another point to note in the above equation is
...
external template. \textbf{Finally, $\epsilon_j(\nu)$ denotes a frequency-dependent dust extinction factor. In the current analysis, this is based on the \Planck\ 2013 $E(\mathrm{B}-\mathrm{V})$ model (see \citealp{CG02_04} for details), and is only applied to the bright discrete stars. Generalizing this to a full Galactic 3D dust model lies beyond the scope of the current paper, but we note that very preliminary work in this direction has already started.}
...
algorithm, \textbf{which forms the seventh component of the data model.} Together, these \textbf{two} source components comprise an
