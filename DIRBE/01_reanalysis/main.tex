...
(CIOs). Ideally, the optimal approach would in principle be to start from raw uncalibrated TOD, but those are unfortunately not
...
parameters.\footnote{The dependence of $\N_\mathrm w$ on free parameters technically breaks this assumption, but is in practice constant relative to all other parameter variations.}
...
\begin{figure}
  ...
  \caption{Processing masks used in the analysis. Green regions correspond to the general TOD processing masks, and the blue regions correspond to the zodiacal emission masks.}
  \label{fig:masks}
\end{figure}
...
\begin{figure*}
	...
	\caption{Trace plots of MCMC sampling chains. Each panel shows a series of sampled components for each Gibbs sample, including the ten DIRBE monopoles, the zodiacal dust cloud albedos $A$ and emissivities $E$, the ambient dust temperature at 1 AU $T_0$, and the instrumental parameters for band 10.}
    \label{fig:trace}
\end{figure*}
...
\begin{figure*}
	...
	\caption{Correlations between a selection of all sampled parameters. $m$ are the band monopoles, $A$ and $E$ are the albedos and emisivities of the zodiacal light cloud per band, $T_0$ is the dust temperature, $\sigma$ are the white noise levels and $f_{\mathrm{knee}, 10}$ and $\alpha_{10}$ are the correlated noise parameters in band 10 where these are fit.}
...
In order to assess burnin and convergence, we plot a range of parameters from the Gibbs chain in Fig.~\ref{fig:trace}. Included in this figure are the sampled monopoles, zodiacal dust albedos and emissivities, and instrumental parameters, including the estimated white noise in bands 1 and 10, and the correlated noise parameters $f_\mathrm{knee}$ and $\alpha$ for band 10. From this figure, it is clear that there is a large amount of burnin in bands 1--3, concluding at approximately sample 50.  At the same time, there is still drift within the emissivity and albedos. However, since the monopoles are stable beyond this point, this is mainly due to degeneracies within the zodiacal model itself. This is explored in full detail in \citet{CG02_02}.

In practice, only every other Gibbs sample is independent, due to the dedicated solving for best-fit zodiacal dust parameters within each independent chain. Discarding the burnin and every other sample, we obtain a total of 233 independent Gibbs samples. Due to the extended burn-in of the zodiacal parameters, this chain cannot yet be considered fully converged, but because of the stability of the monopoles and frequency maps, we are able to present a preliminary analysis here. 
...
\end{figure*}
       ...
       \begin{figure*}
        ...
        \includegraphics[width=0.35\linewidth]{figs/dirbe_04_hmhd_v1.pdf}\\         
       	...
       	\caption{Zodi-subtracted half-mission half-difference maps for each channel. The 140 and 240\,$\mu$m channels have been smoothed to an angular resolution of $3^{\circ}$, while all others are shown at their native resolution. }
       	\label{fig:hmhd}
       \end{figure*}
...
slope of the $T$--$T$ plot between the two maps \textcolor{red}{(where is this plot?)}, evaluated either
outside the DR2 processing mask (Figure \ref{fig:masks}) or the full sky. The results from
...
appear brighter, due to the additional contamination of the ZL
residuals in the original analysis.
