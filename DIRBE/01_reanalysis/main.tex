...
\begin{align}
	...
	\dv &=\G\P\B\sum_{c=1}^{n_{\mathrm{comp}}}\M_c\a_c+\s_{\mathrm{zodi}} +
        ...
        &\equiv \s_{\mathrm{tot}} + \n_{\mathrm{w}}.
\end{align}
Here $\dv$ denotes a stacked vector of all DIRBE TOD for all
...
pointing matrix, which we define in Galactic coordinates, and $\B$ denotes
...
bandpass of each detector. The term $\s_{\mathrm{zodi}}$ is a model of zodiacal
...
denoted $\zeta_{\mathrm{z}}$. \textbf{Some parameters, including the circumsolar ring and Earth-trailing feature, are fixed to the K98 model. Such choices are further described in \citet{CG02_02}.}
...
all channels except \textbf{140 and} 240\,$\mu$m. The stationarity period is assumed to
be 24~hours, and the data are correspondingly processed in segments of
this length. For the \textbf{140 and} 240\,$\mu$m \textbf{channels} we additionally include a
...
As indicated by the above sky model, we fit a novel \textbf{four-component}
generalized modified blackbody model to account for thermal dust
across the combined DIRBE and \Planck\ HFI frequency range;
for full details, see \citet{CG02_05}, \citet{CG02_06}, and \citet{CG02_07}. The \textbf{four} components correspond
to cold dust, hot dust, and nearby dust emission, respectively, \textbf{as
...
bands are not used for the actual fit. \textbf{These individually fitted sources are denoted as ``bright
...
algorithm, \textbf{which forms the seventh component of the data model. No
extinction correction is applied to the faint stars.} Together, these \textbf{two} source components comprise an
...
is also sensitive to zodiacal light and Galactic residuals. \textbf{In particular}, DIRBE was designed to have negligible straylight contamination, while internal thermal sources in FIRAS were explicitly measured by onboard thermistors and subtracted before mapmaking.
%For
...
Conversely, at very long and very short wavelengths, a more aggressive Galactic mask is required. These are shown in green in Fig.~\ref{fig:masks}, and are defined again by regions in which the foregrounds are bright and not modeled sufficiently well. \textbf{The specific values for masks are chosen based on smoothed residual masks from preliminary runs, with brightnesses determined by the overall signal-to-noise level within each band.}
...
posterior mean maps for \textbf{each half-mission and channel}. Blue pixels correspond to
...
strong testament to the efficacy of the \textbf{current compact template based
...
physical origin of the excess \textbf{signals seen in
...
limits correspond to \textbf{the two thin} concentric circles centered on the Sun in
...
With the model in hand, we run \commanderthree\ on the data. We find that an average Gibbs iteration takes approximately 250\,CPU-hrs, excluding initialization time. In order to ensure proper sampling of the full distribution, we run \textbf{five} independent \commanderthree\ runs initialized with their own random seeds. \textbf{The chains have 244, 246, 246, 242, and 246} samples, giving a total of \textbf{1224} samples. A total of one month walltime on 1920 computing cores was required to produce these samples.
...
\textbf{Depending on the parameter of interest, 30--50 samples are considered burned in. This gives us a total of 974--1074 total samples, depending on the parameter.}
...
\begin{figure}
	...
	\caption{Realization of correlated noise for first half-mission of $240\,\mathrm{\mu m}$ band, from the \textbf{250th} sample of the first chain.}
  \label{fig:ncorr_map}
\end{figure}
...
\end{figure*}
       ...
       \begin{figure*}
        ...
        \includegraphics[width=0.35\linewidth]{figs/todres_04.pdf}\\         
	       ...
	       \caption{Data-minus-model residual maps \textbf{averaged over all Gibbs samples}. The 140 and 240\,$\mu$m channels have been smoothed to an angular resolution of $3^{\circ}$, while all others are shown at their native resolution. }
       	\label{fig:res}
       \end{figure*}
       ...
       \begin{figure*}
        ...
        \includegraphics[width=0.35\linewidth]{figs/hmhd_04.pdf}\\         
	       ...
	       \caption{Zodi-subtracted half-mission half-difference maps \textbf{averaged over} all Gibbs samples. The 140 and 240\,$\mu$m channels have been smoothed to an angular resolution of $3^{\circ}$, while all others are shown at their native resolution. }
       	\label{fig:hmhd}
       \end{figure*}
...
\textbf{These bands also have residuals northwest of the Galactic plane, corresponding to the relatively low depth of coverage in the AllWISE coverage.}
...
\textbf{It is likely that these are primarily due to unmodeled sky components, including zodiacal dust and Galactic emission.}
Above
...
12 and 60$\,\mu$m channels, which also \textbf{show} slight evidence of residual ZL
...
that \textbf{the $1.25\,\mathrm{\mu m}$ DR2 channel has a brighter Ecliptic plane
...
\textbf{The largest discrepancies are the higher noise levels in channels 5 and 6. This is due to the more aggressive masking based on the Solar-centered masking, as indicated in Table~\ref{tab:data_selection}.}
