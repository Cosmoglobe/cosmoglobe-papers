...
In the Gaia database, 25\% of entries stronger than 12 mag were found to be binary stars. Whenever stars flagged as binaries in the Gaia database also had estimated physical parameters, we first calculated the spectrum of each binary according to Eq. \ref{eq:starspec}. Since the flux values are given per unit of area, we weighted the two spectra according to the estimated area of the corresponding star, so that the total flux estimated from the binary system
\begin{equation}
