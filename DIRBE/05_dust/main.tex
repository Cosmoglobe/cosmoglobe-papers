...
In order to perform such a fit, we first downgrade all datasets and components
to the lowest common resolution, which in our case is an \nside of 16.
Furthermore, we smooth all maps with the FIRAS beam, shown in Fig.
\ref{fig:firasbeam}, as this is the lowest resolution beam present in the maps
used.

\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{figures/firas_beam.pdf}
  \caption{The FIRAS beam, with which all maps in this analysis were smoothed.}
  \label{fig:firasbeam}
\end{figure}
...
upwards, as well as DIRBE data from the 8 longest wavelength bands (from 240
$\mu m$  to $3.5 \mu m$).
