...
(as well as the nearby dust) SEDs are only allowed to be nonzero for bins 1-6 (DIRBE bands 10 through 5).


%There are thus two features which distinguishes the three dust components from
%each other; first, whether the component has a freely varying amplitude per
%pixel; and second, whether the component is allowed a nonzero SED contribution
%at bins 7 and 8 in Table \ref{tab:bins}.
%
%The first component, here called ``cold dust'', is defined by a freely varying
%amplitude at each pixel, but its SED is set to zero above bin 6. This
%component, as the name suggest, is meant to trace dust in low-energy regions,
%and since the PAH features, which are  at high frequencies are only expected to
%contribute in DIRBE bands 3 and 4.
%\begin{equation}
%\end{equation}
...
measured the sky from $1\,\mathrm{\mu m}$ to $240\,\mathrm{\mu m}$. In this
analysis, we have converted the original DIRBE CIO (Calibrated Individual
Observations), whose pointing information is given in terms of Quadcube pixels
with a resolution of 20\arcs\ into a HEALPix \nside=512 pixelation, giving an
approximate resolution of 7\arcm. Following the nomenclature of \cite{CG02_01},
we refer to the DIRBE CIOs as ``time-ordered data'' (TOD). Including DIRBE TOD
in the analysis in this way allows us to target the zodiacal light
\citep{CG02_02}, which, as mentioned above, must be analysed as a time-varying
component on the sky, in contrast to other astrophysical components.

The use of DIRBE data also allows a full exploration of the scales relevant for
interstellar dust modelling: As mentioned above, 
