...
current paper. In \cref{sec:results} we present the results from
...
%function.
%
%The number of parameters involved in our model (\cref{eq:model}) is of the
...
%distribution. Applied to \cref{eq:model}, we get a sequence of operations as
%follows:
%
%%\begin{equation}
%%\begin{alignat}{11}
...
%Hence, in this paper, we mainly focus on the three first components of 
%(\cref{eq:skymodel}), and leave the other components for other papers. (TODO
...
The template fitting procedure described in \cref{sec:tempfit} requires
...
convolve all maps with the \COBE-FIRAS beam, shown in \cref{fig:firasbeam},
