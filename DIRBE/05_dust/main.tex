...
\title{\bfseries{\Cosmoglobe\ DR2. V. Spatial correlations between thermal dust and ionized carbon emission in \Planck\ HFI and COBE-DIRBE}}
   ...
   \abstract{We regress five tracers of thermal dust emission on ten \Planck\ HFI and COBE-DIRBE frequency maps between 353\,GHz and 25\,THz, aiming to map the relative importance of each physical host environment as a function of both position on the sky and frequency. Four of these correspond to well known thermal dust tracers, namely HI (HI4PI), CO (Dame et al.), H$\alpha$ (WHAM), and dust extinction (GAIA), while the fifth is ionized carbon (CII) emission as observed by \COBE-FIRAS. To our knowledge, this has typically not been counted among the classical dust tracers until now. After smoothing all data to a common resolution set by the FIRAS experiment, and subtracting subdominant astrophysical components as appropriate for each channel (CMB, CIB, and zodiacal light), we jointly fit all five templates to each frequency channel through linear regression. At frequencies higher than 1\,THz, we find that the dominant tracer is CII, and above 10\,THz this component accounts for almost the entire fitted signal; at frequencies below 1\,THz, its strength is second only to HI. We also find that all five components can be well described by a modified blackbody spectral energy density (SED) up to some maximum frequency between 1 and 5\,THz, depending on component, and in this interpretation the CII-correlated component has an effective temperature of about 25\,K. For comparison, the HI and CO components have effective temperatures of 16\,K and 12\,K, respectively. The H$\alpha$ component has a temperature of 18\,K, and, unlike the other four, this is observed in absorption rather than emission. The spectral indices of the five components range between $\beta = 1.4$ (for HI) and 2.6 (for H$\alpha$). Despite the extreme simplicity of the model, which relies only external templates coupled to spatially isotropic SEDs, we find that 98\,\% of the signal rms may be modelled in terms of this simple five-component model at frequencies below 1\,THz; at higher frequencies, the model captures more than 80\,\% of the signal rms. This high efficiency suggests that the spatial variations in the thermal dust SED previously reported by \Planck\ and other experiments on large angular scales may be more effectively modelled in terms of a spatial mixing of individually nearly isotropic physical components, than by a single uniform well-mixed interstellar medium coupled to a spatially varying temperature field, as has been the norm until now. Indeed, the results found in this paper motivates the thermal dust model adopted for the \Cosmoglobe\ DR2 re-analysis of the \COBE-DIRBE data, and we believe that they may also provide inspiration for refining both current theoretical ISM models and component separation algorithms in general.}
