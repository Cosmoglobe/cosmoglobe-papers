...
\begin{figure}
  ...
  \caption{\COBE-FIRAS beam response as a function of anglular distance, with which all maps in this analysis were smoothed. The beam diameter is around $7^\circ$ on the sky. The maps were published in the QuadCube pixelization scheme, but they have now been converted to HEALPix with \nside=16, which then limits this analysis to that \nside.}
  \label{fig:firasbeam}
\end{figure}
...
\begin{figure*}
  ...
  \caption{Comparison between original maps (left) and the template subtracted maps, weighted by the fitted amplitudes (right), for the three dust-dominated \Planck\ HFI channels. All maps are convolved with the FIRAS beam and dowgraded to \nside=16. The grey pixels are masked by the common analysis mask. }
...
\begin{figure}
  ...
  \caption{Efficiency of the dust model at describing the data at at each frequency. We subtract the squared residuals from the squared map, and normalize by the squared amplitude of the map. The five component template-based dust model is able to recover more than 80\% of the total map power at all frequencies, including those with known residual zodiacal contamination, such as 25$\mu$m.}
  \label{fig:temprms}
\end{figure}
...
account the drift along the Ecliptic \citep{odegard:2019},
...
with temperatures in the typical thermal dust temperature range. Below, we estimate the temperatures for each of these spectra.
...
comparison between the original data (left columns) and the template subtracted
maps, weighted by the fitted amplitudes (right columns). The template fitting performs well on all
...
(evident in the two highest frequency maps). Overall, a first inspection of the maps seems to
...
unexplained RMS left in the map ranges from as low as 2\% (for the lower
frequencies) to around 15\%, and we note that the only two maps that are left with more
than 5\% of unexplained RMS are the two maps where the imprint of remaining
zodiacal light is the strongest.

Additionally, in \cref{fig:tempfract}, we show the relative contribution of each
template to the total dust model amplitude at each frequency. Showing, once again, that the
two most dominant components are the \CII\ and \HI\ templates, with the former
dominating at higher frequencies, and the latter at lower frequencies.
...
\begin{equation}
    s(\nu) = A \frac{\nu^{\beta + 3}}{\exp{(\frac{h\nu}{k_b T})} - 1}.
\end{equation}
Here, A is some amplitude scaling factor, T is the blackbody temperature, and
...
We fit these to data from \Planck\ HFI and
DIRBE, from 353\,GHz to 12\,$\mu$m, downgraded to the FIRAS resolution, where thermal dust emission is one of the
