...
\begin{document} 
   ...
   \title{\bfseries{\Cosmoglobe\ DR2. V. Global modelling of diffuse Galactic dust emission between 100\,GHz and $12\,\mu$m with DIRBE and HFI}}
   ...
   \abstract{We present a compact global model of thermal dust emission derived from the \Planck\ HFI and \COBE-DIRBE frequency channels between 100\,GHz and 12\,$\mu$m, as part of the global Bayesian \Cosmoglobe\ DR2 analysis. This model is based on four morphologically distinct astrophysical components, each defined by a spatially varying amplitude coupled with a spatially constant spectral energy distribution (SED). The four components correspond to nearby, cold, hot, and infrared dust emission, respectively. The amplitude map of the nearby dust component is defined by integrating the GAIA extinction-based 3D dust model of Edenhofer et al. (2022) along each line of sight, and this component therefore does not introduce any new parameters per pixel. The amplitude maps for the cold and hot components are derived from the \Planck\ 100, 143, 217, and 545\,GHz maps alone, while the amplitude of the infrared component is primarily derived from the DIRBE 60 and 100$\,\mu$m channels. The SEDs of each component are modelled with standard modified blackbody (MBB) emission laws with a free temperature, $T$, and spectral index, $\beta$, up to some maximum frequency, after which is defined by a piecewise binned function. Fitting this model to the HFI and DIRBE data, we make the following observations: 1) With only three free parameters per pixel, this model is able to reproduce more than 98\,\% of the observed diffuse Galactic signal at all \Planck\ and DIRBE frequency channels between 100\,GHz and 12\,$\mu$m; 2) the cold component correlates strongly with HI emission, and has an effective MBB temperature of $T_{\mathrm{cold}} = 11\pm3\,\mathrm{K}$; 3) the hot component correlates strongly with \CII\ emission, and has an effective MBB temperature of $T_{\mathrm{hot}} = 40\pm5\,\mathrm{K}$; 4) the infrared component is morphologically similar to the hot component, but more concentrated around the Galactic center; it is also overall fainter, and it has an effective temperature of $T_\mathrm{ir} = 30\pm3\,\mathrm{K}$; 5) the Galactic signal observed in the DIRBE 25 and 12\,$\mu$m channels is extremely well traced by the hot component alone, which is derived exclusively from \Planck\ data; 6) there is some evidence of unmodelled spatial variations in the SED parameters of the nearby component, and this suggests that this component in a future analysis should be segmented in 3D into physically distinct templates; 6) the best-fit spectral indices for the hot, cold and nearby dust component are all consistent with $\beta = 1.65$, while the best-fit value for the nearby component is $\beta=1.45$. However, the uncertainties are large, and the fact that there is evidence of unmodelled spatial variations associated with the nearby component suggests that a single spectral index cannot yet be ruled out. Not only does this model provide a very compact numerical representation of thermal dust emission in the Milky Way across the CMB and infrared regimes, but the extremely strong --- and hitherto underappreciated --- \CII\ correlation should provide inspiration for future theoretical studies. Finally, we note that that these findings may important implications for future CMB $B$-mode polarization studies, both in terms of limiting potential line-of-sight dust SED variations at larger distances than those covered by GAIA, and as a worked example of how to integrate 3D information from starlight experiments like GAIA and PASIPHAE into classical CMB analysis methods. }
...
\clearpage
\section{Low-resolution template fitting}
...
\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{figures/template_ratio_v1.pdf}
  \caption{Overview of low-resolution template fitting results derived from \Planck\ HFI and DIRBE data. }
  \label{fig:tempamp_comparison}
\end{figure}
