...
dust emission with a temperature of $\sim$25-30\,K. This observation may
...
which we wish to perform the fit are given as HEALPix\footnote{\url{http://healpix.sourceforge.net/}} \citep{healpix,Zonca2019} maps in the same
...
\begin{equation}
    ...
    \label{eq:datamodel}
\end{equation}
where the sum over $c$ runs over $n_{\mathrm{comp}}$ dust components,
each with an amplitude $a_{i, c}$ and a spatial template $\t_c$. In
addition, we include a monopole term, $m_i$, and a white noise
contribution, $\n_{w,i}$. We assume that the data do \emph{not}
include CMB, CIB, or zodiacal light contributions, as these are removed through preprocessing; see \cref{sec:data} for details. 
...
this case, is an $n_{\mathrm{comp}}+1$-dimensional vector, the extra dimension being due to the
monopole contribution), and we can then write
\begin{equation}
    \boldsymbol{a}_i = (\mathsf{T}^T \mathsf{T})^{-1} (\mathsf{T}^T\boldsymbol{d}) ,
    \label{eq:lss}
\end{equation}
where we define $\mathsf{T}$ as a $n_\mathrm{pix}$ by $n_{\mathrm{comp}}+1$ matrix such that matrix
...
its $n_{\mathrm{comp}}+1$ elements the best-fit amplitudes for each component (and best-fit
monopole).
...
measured the sky in ten frequency bands, from 1\,$\mu$m to
240\,$\mu$m. 
...
and is typically associated with cold neutral medium regions of the galaxy (as it
...
account the drift along the Ecliptic \citep[e.g.,][]{odegard:2019},
