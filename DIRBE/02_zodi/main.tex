...
(\zeta_{\mathrm{z}})$ also by construction entirely blind to the
...
in the original DIRBE analysis of. For full details on the
...
\begin{figure*}
    ...
    \caption{Trace plot of all geometrical interplanetary dust parameters. The different colors indicate six independent Markov chains, and the gray regions indicate burn-in. The bottom right section shows the average full-sky intensity.}
    \label{fig:trace-ipd}
\end{figure*}
...
\begin{figure*}
    ...
    \caption{Trace plot of all amplitude parameters. The different colors indicate six independent Markov chains, and the gray regions indicate burn-in.}
...
\begin{figure}
    ...
    using the K98 model and our best-fit ZL model. A best-fit monopole has been removed from the difference map prior to dividing by the  DIRBE signal.}
    \label{fig:reldiff}
\end{figure}
...
\begin{figure*}[t]
    ...
    \caption{Half-mission split residual maps for each channel. All bands between 1.25 and 100$\,\mu$m have been smoothed with a $15\arcm$ Gaussian kernel, and the 140 and 240\,$\mu$m channels have been smoothed with a $30\arcm$ FWHM kernel.}
...
important, the K98 and \cosmoglobe\ DR2 models agree to better than
...
residuals reported by \citet{Kelsall1998} in the original DIRBE
analysis, corresponding to $\sim$\,2\,\% at 12\,$\mu$m.
...
At the same time, we do in fact obtain reduced $\chi^2$'s that are
...
simultaneously, whether they are of astrophysical or instrumental
