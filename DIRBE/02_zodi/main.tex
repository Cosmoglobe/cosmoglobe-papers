...
\begin{document} 
%\linenumbers
   \title{\bfseries{\Cosmoglobe\ DR2. III. Revised modeling of
...
% write an abstract 
   ...
   \abstract{We present a \textbf{revised} zodiacal light (ZL) model for \textit{COBE}-DIRBE, 
...
Sect.~\ref{sect:zodi-model} we introduce our ZL model, \textbf{then in Sect.~\ref{sect:data} 
...
but we use a linear combination of Henyey-Greenstein functions to describe the phase function, 
as defined by \citet{Hong} to model the scattering properties of the grains. 
The reason for this is that \citet{kelsall1998} used a version of the same scattering 
function that was adapted to fit the DIRBE data specifically, while this work attempts 
to find a more physically motivated and flexible ZL model so that it will be possible to 
apply it to a wider range of dataset in the future.}

\textbf{The total ZL signal may then be evaluated by summing up the intensity
...
to \citet{SanPhD} for maps showing the 
...
Fig.~54. \textbf{Such radiation can in principle be created through several
...
\textbf{For comparison, another example of ZL subtraction carried out 
without using a differential method is \citet{wright2001}. Here the full 
total signal is taken into account, however some assumptions about the 
infrared sky are introduced and compared against each other to distinguish 
ZL from other sky components without using an end-to-end approach. 
...
\begin{figure}[t]
    ...
    in the wavelength channels 1.25 -- 3.5\,$\mu$m, \textbf{while other colors are used to 
...
with those reported by \citet{kelsall1998}\textbf{, \citet{Hong} and \citet{skysurf}}. All the models are 
...
in our function lies between the Hong et al.\ and K98 minima, \textbf{very close to the one of 
...
Figure~\ref{fig:reldiff} \textbf{shows the relative difference between the K98 model 
...
detailed model of ZL in the Ecliptic \textbf{plane.}
%; again, joint analysis with
...
into pixelized sky maps. \textbf{Figures~\ref{fig:dr2-zsma-compare1} and \ref{fig:dr2-zsma-compare2}
