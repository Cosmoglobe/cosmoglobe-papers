...
% write an abstract 
   ...
   decades, appears cleaner in the current processing. Still, obvious ZL residuals 
...
\citealp{partridge1967}). The main scientific motivation of the \textit{COBE}-DIRBE 
experiment \citep{hauser1998,hauser:2001} was to detect
...
three-dimensional model of the IPD distribution. We adopt a modified version of the 
...
underlying parametric model. For full
details of this model, we refer the interested reader to
\citet{kelsall1998} and \citet{Hong}.\footnote{We have also spent large 
...
As described above, our model has only $\mathcal{O}(10^2)$ free
...
factors at each of these channels should be beneficial in many ways; both in
...
post-burn-in samples. For each parameter the number of burn-in 
...
As noted in Sect.~\ref{sect:data}, we remove the first week of
...
very close to unity, namely 140 and 240$\,\mu$m.
...
Furthermore, future analyses will also include SPHEREx data, 
