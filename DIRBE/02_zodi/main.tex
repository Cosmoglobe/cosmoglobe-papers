...
\begin{document} 
   ...
   \title{\bfseries{\Cosmoglobe\ DR2. III. Improved modelling of zodiacal light in \textit{COBE}-DIRBE through Bayesian global analysis}}
...
1.4^\circ$, $10^\circ$, and $15^\circ$, and are associated with
...
noise. We denote the set of all free parameters in Eq.~\eqref{eq:model}
...
\begin{figure*}
    ...
    fitting algorithms adopted by the DIRBE (\emph{right column}) 
    ...
    ratio, both to ZL parameters and zero-levels.}
...
Since we are not dealing with the excess static emission at this moment, we have chosen to keep the
near-earth circumsolar-ring and Earth-trailing components fixed to the K98 values. Attempting to fit these could 
prove to be difficult due to the channel-to-channel varying excess stationary emission.
...
\begin{figure*}
    ...
    \caption{Trace plots of all geometrical and shape IPD parameters in addition to the IPD temperature at 1 AU $T_0$ and the full-sky averaged ZL intensity traces for six selected DIRBE channels. for all six independent Markov chains.}
    \label{fig:trace-ipd}
\end{figure*}

\begin{figure*}
    \centering
    \includegraphics[width=0.975\textwidth]{figs/emissivity_and_albedo_trace.pdf}
    \caption{Trace plots of the wavelength dependent emissivity and albedo parameters for all six independent Markov chains.}
...
applied to the two longest wavelengths.

\input{ipd_param_table.tex}


Additional data is removed from the analysis in order to account for
the excess static emission in solar-centric coordinates discussed by
\citet{CG02_01}, and briefly reviewed in this paper in
Sect.~\ref{sec:gibbs}. In short, \citet{CG02_01} defines a set of
masks in solar-centric coordinates for each frequency channel between
1.25 and 100\,$\mu$m, and removes any time sample that is excluded by
these masks from further analysis. This approach plays exactly the
same role as the solar elongation cut used by the DIRBE team to
produce their final ZSMA maps, but provides much better precision in
terms of removing specific systematic features.
...
rotation of the spacecraft about its boresight (0.8\,RPM). Within this
rotation, both the Galactic and Ecliptic planes are crossed twice. The
two outer-most bulges in the 1.25, 2.2, 3.5, and 4.9 $\mu$m bands
and the right bulge in the remaining bands represent the Galactic
plane crossings. The sine-like waves in bands 4.9, 12, and 25\,$\mu$m
...
the corresponding residuals therefore provide information about
seasonal ZL variations not captured by the model.
...
evaluated at 1.25\,$\mu$m at 20\,kJy/sr. In addition, to remove
diffuse thermal dust emission, we remove all observations with an
absolute Galactic latitude $|b|<45^{\circ}$. The resulting mask is
shown in blue in Fig.~\ref{fig:zodi-procmask}, and leaves 18\,\% of
the sky available for analysis.
...
We are now finally ready to present our new ZL model that is obtained
...
produced a total of 378 full Gibbs samples, distributed over six
Markov chain, and we remove the first 25 samples from each chain as
burn-in, leaving a total of 228 samples for final analysis. In total,
...
Many interesting features may be identified in this plots. Starting
...
samples or so, but after that, they appear mostly stationary.  we also
...
Some parameters appear pathological, however, for instance
$p_{\mathrm{B1}}$, which only explores a very narrow parameter region
near $p_{\mathrm{B1}}\lesssim 4$. In this case, it is worth noting
that Band~1 has a relatively low S/N overall, and this particular
parameter, which quantifies the slope of the band, has an even lower
S/N. It has therefore drifted to its upper hard prior of
$p_{\mathrm{B1}}< 4$, apparently attempting to compensate for some
other effect in the data. In retrospect, this parameter should perhaps
have been fixed at an IRAS-derived value, similar to what was done in
the K98 analysis. For this first \cosmoglobe\ DR2 analysis, however,
an important aim was to explore the full data model as freely as
possible, to understand what the DIRBE data themselves prefer. A
future joint analysis between DIRBE and IRAS should hopefully lead to
better-determined parameters for the asteriodal bands without the need
for fixed priors. It is also worth recalling that most of the
parameters shown in Fig.~\ref{fig:trace-ipd} are constrained only by
the 12 and 25$\,\mu$m channels, and including more channels in the
shape parameter ZL sampling group may reduce some of these
degeneracies further.


Moving on the emissivity and albedo parameters shown in
...
as IRAS and AKARI should be highly beneficial in constraining these.
...
\begin{figure}
    ...
    \caption{Relative fractional difference in the mission-averaged ZL simulations at 25 $\mu$m 
    using the K98 model and our best-fit ZL model.}
    \label{fig:reldiff}
\end{figure}
...
Table~\ref{table:zodi-params-geo}, while the amplitude
parameters are provided in
Table~\ref{table:zodi-params-source}. For comparison, the
corresponding values from the K98 model are listed in the second
column in each table. The fourth and final column lists the uniform
priors imposed on each parameter.

In general, we see that the differences between the new DR2 and the
old K98 models for many parameters vary significantly more predicted
by the reported statistical uncertainties. For instance, the density
of the dominant cloud parameter is about 25\,\% lower in the DR2 model
than in K98, and measured in units of the K98 uncertainty, this
correspond to more than a $40\,\sigma$ discrepancy. However, we also
see that the DR2 uncertainty for this parameter is six times larger
than the K98 uncertainty, which reduces the tension to $7\,\sigma$. At
the same time, we also notice that the amplitude of the first band is
13 times higher in the DR2 model than in the K98 model, and this
accounts for a significant fraction of the lower cloud intensity.
...
is particularly interesting to note structure of the first asteroidal
band, which in the K98 model is very faint indeed. In the DR2 model,
...
point. It is then interesting to compare this new model to the alternative
model presented by \citet{Robinson2013}, where they replace the K98
parametrization with a fan-plus-comet composite. At a purely visual
level, our $B_1$ component appears to play a similar role as their
so-called ``broad band'' (bb) component.
...
as a function of position on the sky for the 25\,$\mu$m channel
only. In this channel, for which the ZL emission is the brightest and
therefore degeneracies are the least important, the K98 and
\cosmoglobe\ DR2 models agree to better than 5\,\% over the full sky,
and better than 2\,\% in most of the Ecliptic plane. These differences
are very similar in magnitude to the residuals reported by
\citet{Kelsall1998} in the original DIRBE analysis, corresponding to
$\sim$\,2\,\% at 12\,$\mu$m.
...
\caption{Reduced goodness-of-fit ($\chi_\mathrm{red}^2$) values along with the number of TODs used to fit the ZL parameters and the instrument white noise levels for each DIRBE channel.}
\label{tab:chisq}
\vskip -4mm
\footnotesize
\setbox\tablebox=\vbox{
...
instance, in the 1.25\,$\mu$m HM1 map, corresponding to the first five
...
all channels below 100$\,\mu$m, and future work should obviously aim
...
free of sharp edges, and the main features appear either at the
end or beginning of the survey suggests at least that an instrumental
origin cannot be excluded a-priori. For the 4.9--100$\,\mu$m channels,
a whole month is removed at the end of the survey, and a corresponding
...
of the white noise level, as reported by \citet{CG02_01},
...
reduced $\chi^2$ of 1.30 with their extended model. In contrast, we
...
establish a set of zodiacal light subtract mission averaged (ZSMA)
...
by subtracting the ZL component only from the TOD, and bin the rest
into pixelized sky maps.

Figures~\ref{fig:dr2-zsma-compare1} and \ref{fig:dr2-zsma-compare2}
compares the legacy DIRBE ZSMA maps (second column) with the new
...
\begin{figure}
    ...
    \caption{RMS ratio between \cosmoglobe\ DR2 and DIRBE ZSMA,
      $\sigma_{\mathrm{DR2}}/\sigma_\mathrm{DIRBE}$, as evaluated
...
zero-levels between the DIRBE and DR2 maps, and this is shown in
...
the difference is defined as DIRBE-minus-DR2, and this is always
positive. In other words, the new analysis appears to have removed
more ZL than the legacy DIRBE analysis. In fact, the measured monopole
difference is actually also higher than the CIB monopole spectrum
itself, suggesting that the modifications are in fact $\mathcal{O}(1)$
or larger as measured relatively to the main science target of DIRBE;
at 12$\,\mu$m, the difference is almost two orders of magnitude. For
further discussion regarding the scientific implications of these
differences in terms of the CIB monopole, we refer the interested
reader to \citet{CG02_03}.
...
have $\sim$\,30\,\% lower rms than the legacy DIRBE maps, which once
again indicates that more ZL emission has been removed from these
the new maps.
...
ideas from simulated annealing and/or Hamiltonian sampler.

In addition, it is of course by no means obvious that a
low-dimensional 3D model such as K98 with only $\mathcal{O}(10^2)$
free parameters can describe such a rich and dynamical system as ZL to
the required precision for next-generation cosmology. Future work
should therefore also consider conceptual generalizations of the
entire framework. One step in this direction was taken by
\citet{Robinson2013}, who introduced a different set of basis
components than K98; indeed, the new fit presented in this paper
appears to have moved toward that model within the K98 framework. More
drastic approaches would be to consider perturbative Taylor expansion
models around the existing components, or even fully voxelized 3D
models. Obviously, these would be highly degenerate when using only
near-Earth observations, but perhaps combination with data from solar
system probes such as Juno or New Horizons, new insights may be
gained.
