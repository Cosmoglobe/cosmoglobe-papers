...
% write an abstract 
   ...
   framework. The parametric form of the ZL model is \textbf{inspired by that introduced by 
   \citet{kelsall1998} (K98 model)}, but the specific best-fit parameter values are re-derived 
   using the combination of DIRBE Calibrated Individual Observations, \Planck\ HFI 
...
accurate ZL model to remove the time-varying ZL. In this paper, we therefore implement support for \textbf{the K98 model, combined with the scattering phase function described 
by \citet{Hong} in \commanderthree\,} and we apply this to the time-domain DIRBE data. 
...
line-of-sight $s$ from the observer and toward $p$\footnote{\textbf{Should 
we add p and t subscripts also to right hand side? \textit{Take out this note!!!}}}.

\textbf{Due to the limited signal-to-noise ratios, we follow
...
\begin{align}
	\label{eq:model}
	\dv &=\G\B\left[\P\sum_{c=1}^{n_{\mathrm{comp}}}\M_c\a_c+\s_{\mathrm{zodi}} +
...
\r = \dv - (\G\P\B\sum_{c=1}^{n_{\mathrm{comp}}}\M_c\a_c +
          \s_{\mathrm{static}}),
\end{equation}
and this should ideally only contain ZL and \textbf{unmodeled noise\footnote{\textbf{My point is this noise 
...
As such, the assumption that this noise is also Gaussian\footnote{\textbf{This is tricky since before we 
specified the covariance of the Gaussian noise and that created problems, but I'm not sure how to 
fix this. \textit{Take out this note!!!}}} 
defines the conditional distribution in Eq.~\eqref{eq:gibbs_zodi}, and we may write}
\begin{equation}
...
\begin{figure*}[h!]
    \centering
    \includegraphics[scale=0.9]{figs/traceplot_tot_shape_new.pdf}
...
but only \textbf{their} number density. \textbf{In particular,} throughout our many attempts, 
...
\textit{Something here about how we chose to fit only some emissivities (I guess is a S/N thing but I'm not sure)?}}
...
Fig.~\ref{fig:correlations} shows the Pearson correlation
...
\begin{figure}[h!]
    ...
    The red lines represent $\Phi(\Theta)$ as defined by the original K98 model \citep{kelsall1998} 
...
%revise this{
Fig.~\ref{fig:mission-averaged-comp-maps} in
...
\textbf{As it is the main difference between the K98 and the DR2 ZL models, 
Fig.~\ref{fig:phase_function} shows the phase function $\Phi(\Theta)$ as defined 
in \citet{kelsall1998} (red lines), in \citet{Hong} (black dashed line) and in this 
paper (black solid line). All the models are normalized over the interval 
$\Theta \in [65^\circ,\,125^\circ]$, since this is the interval of 
solar elongation angle that DIRBE is able to observe. The DR2 phase function was obtained 
by computing the average and the standard deviation of all phase function values 
resulting from post-burn-in parameters.}

Fig.~\ref{fig:zodi-intensity} compares the predicted ZL intensity
...
Fig.~\ref{fig:reldiff} shows a similar relative difference, but now 
...
\textbf{\textit{This whole subsection needs to be revised}}
...
observed residuals are due to real ZL variations or instrumental
...
\citet{CG02_01}, and shown to correlate directly with a bright
...
circumsolar ring and Earth-trailing feature needs to be modified.
...
improvements in the thermal dust modeling.
