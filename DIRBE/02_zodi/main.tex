...
post-burn-in samples. For each parameter the number of burn-in samples
was tuned individually, as shown in Figs.~\ref{fig:trace-ipd} and
\ref{fig:trace-emissivity-albedo}. The resulting values for the shape
...
\begin{figure*}[t]
    ...
    %}
      \\
    \caption{\textit{(Left column):} Mission-averaged frequency maps containing ZL after data 
...
The intensity of the third asteroidal band is particularly weak in our model, and we only conclude with an upper limit. 
...
Each map shown in Figs.~\ref{fig:atlas1} and ~\ref{fig:atlas2} 
