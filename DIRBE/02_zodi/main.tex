...
Following \citet{kelsall1998}, and due to their low signal-to-noise
ratios, we fit only one overall albedo for each high-frequency
channel, as well as only one common emissivity for all three
asteroidal bands. However, unlike the original K98 model, we do allow
a non-zero albedo for the 4.9\,$\mu$m.
...
\begin{figure}
    ...
    \caption{(\textit{Top panel}:) Instantaneous ZL intensity
      predicted by the DR2 (solid lines) and DIRBE (dashed lines)
      ...
      between the DR2 and DIRBE models, evaluated from the results
...
\begin{figure}
    ...
    \caption{Relative fractional difference in the mission-averaged ZL
      simulations at 25\,$\mu$m using the DIRBE K98 model and our best-fit ZL
      model. A best-fit monopole has been removed from the difference
      map prior to dividing by the DIRBE signal.}
    \label{fig:reldiff}
\end{figure}
...
\begin{figure}
    ...
    \caption{RMS ratio between the \cosmoglobe\ DR2 and DIRBE ZSMA maps,
