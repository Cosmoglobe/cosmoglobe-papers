...
\begin{figure*}
    ...
    \caption{Illustration of the basic sky maps involved in the zodiacal light fitting algorithms adopted by the \Cosmoglobe\ and DIRBE pipelines for one week of $25\,\mu\mathrm{m}$ observations and using the K98 model. For \Cosmoglobe, the basic data element is the full sky signal, $I_{\nu}$ (\emph{top left}), which is fitted with the full zodiacal light model, $Z_{\nu}$ (\emph{middle left}), in time-domain. The ultimate goal is to minimize the total signal-minus-model residual, $I_{\nu}-Z_{\nu}$ (\emph{bottom left}). In contrast, the DIRBE pipeline works exclusively with weekly difference maps on the form $\Delta I_{\nu} \equiv I_{\nu}-\left<I_{\nu}\right>$ (\emph{top right}), where brackets indicate full-survey averages. Correspondingly, the zodiacal light model variations are only constrained through $\Delta Z_{\nu} = Z_{\nu}-\left<Z_{\nu}\right>$ (\emph{middle right}), and the final $\chi^2$ is defined through $\Delta I_{\nu} - \Delta Z_{\nu}$. For a comparison of the relative strengths and weaknesses of these two approaches, see the discussion in the main text.}
