...
\begin{figure}
    ...
    \label{fig:tod_zodi}
\end{figure}
We use the publicly available DIRBE Calibrated Individual Observations (CIO)
in our analysis. These are a user-friendly pre-calibrated version of the raw TOD observed by DIRBE.
The CIO are pixelized according to the COBE Quadrilateral cube projection. As part of the data proprocessing,
we convert these to corresponding HEALPix pixel indicies and re-order them into a time-ordered format.
For more details on the preprocessing of the DIRBE CIO we refer to \cite{CG02_01}. In the following, we will 
refer to the pre-processed CIO as TODs.
A subsample of the TOD for each DIRBE band as used in our analysis can be seen in Fig.~\ref{fig:tod_zodi}, 
where we show data for one rotation of the spacecraft about its boresight (0.8 RPM). Within this rotation, 
both the Galactic and Ecliptic planes are crossed twice. The the two outer-most bulges in the 1.25, 2.2, 3.5, 
and 4.9 $\mu$m bands and the right bulge in the remaiining bands represent the Galactic plane crossings. The 
sine-like wave in bands 4.9, 12, and 25 $\mu$m are the zodiacal light which peaks in the ecliptic plane. 
...
\end{figure}
\subsection{Masks}
We use channel specific ZL processing masks. This is necessary because we are fitting ZL parameters 
directly to the TOD residual which may contain unmodeled Galactic emission. Three of the ZL processing 
masks used in this analysis is shown in Fig.~\ref{fig:zodi-procmask}.
