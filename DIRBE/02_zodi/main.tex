...
% write an abstract 
   ...
   differences between the predicted signals from K98 and our new model are \textbf{$\lesssim\,15\,\%$} in 
   ...
   decades, appears significantly cleaner in the current processing. Still, obvious ZL residuals 
   ...
   the state-of-the-art for DIRBE modelling, it also only represents the first among many steps 
...
exponent which is expected to be $\sim 0.5$ for grey dust. In addition to emitting thermally, IPD grains also scatter
...
line-of-sight $s$ from the observer and towards $p$.
...
what is common for most Monte Carlo methods discussed in the
literature, and this indicates rather long correlation lengths. A similar behaviour is observed for the 
...
%which contains oscillatory features in almost every parameter. This behaviour is
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{figs/normdif_05_new.pdf}\\
    \includegraphics[width=\columnwidth]{figs/normdif_06_new.pdf}
    \includegraphics[width=0.6\columnwidth]{figs/normdif_cbar_new.pdf}
    \caption{Relative fractional difference \textbf{between our best-fit ZL 
    model and K98} at 12 and 25\,$\mu$m.}
    \label{fig:reldiff}
\end{figure}
...
\textbf{the comparaison between our best-fit ZL model the K98 model is shown} 
...
$\lesssim$\,15\,\% over most of the sky. These differences are thus
higher, but still comparable in magnitude,} to the relative residuals measured by
...
significant advance in zodiacal light modelling for DIRBE, particularly in the mid-infrared.}
...
\begin{figure*}[h!]
    \centering
    \resizebox{\textwidth}{!}{
    \includegraphics[height=7cm]{figs/density_Cloud_new.pdf}
    \includegraphics[height=7cm]{figs/density_Band1_new.pdf}
    }\\
    \resizebox{\textwidth}{!}{
    \includegraphics[height=7cm]{figs/density_Band2_new.pdf}
    \includegraphics[height=7cm]{figs/density_Band3_new.pdf}
    }\\
    \caption{Visualization of the IPD number density of the four fitted zodiacal 
    components in our model. The number densities are shown as a cross-section of the 
    Solar system in the xz-plane. \textit{(top left):} The smooth cloud. \textit{(top right):} 
