...
% write an abstract 
   ...
   \citet{kelsall1998}, but the specific best-fit parameter values are re-derived 
   ...
   analysis with existing and future high-resolution full-sky surveys such as AKARI, \IRAS, \Planck\ 
...
in \commanderthree\ support for \textbf{the K98 model combined with the scattering pahse function described 
...
Python package for ZL simulations. As an early application of this framework, \cite{San2022} 
...
details of this model, we refer the interested reader \textbf{to
...
\begin{figure}
  \includegraphics[width=0.88\columnwidth]{figs/figure1_1_new.pdf}\\
  \includegraphics[width=0.95\columnwidth]{figs/figure1_2_new.pdf}\\
  \includegraphics[width=0.95\columnwidth]{figs/figure1_3_new.pdf}\\
  \includegraphics[width=0.95\columnwidth]{figs/figure1_4_new.pdf}
...
\textbf{\begin{align}\label{eq:I_tot}
...
relative to \textbf{a suitable set of parameters for our ZL model}. Intuitively, this corresponds to rotating
...
\begin{figure*}[h!]
    ...
    \caption{\textbf{Trace plots of all geometrical and shape IPD parameters, along with the power-law exponent of the IPD
     ...
     Different colors indicate two independent Markov chains, the gray regions indicate the burn-in set for each 
...
produced a total of 1251 full Gibbs samples, distributed over two
Markov chains. A number of samples between 100 and 300, tuned on each parameter 
individually, was removed from each chain as burn-in, 
leaving a total of 1051--651 samples for final analysis. In total,
256 computing cores were used to produce this sample set, for a
total cost of about 96k CPU-hrs.}
...
with the shape parameters for the dominant cloud component shown in
the leftmost column in Fig.~\ref{fig:trace-ipd}, we see that many
parameters appear to drift slightly during the first 100 samples or so, 
but after that, they appear more stationary. We also
...
and cannot robustly measured. Therefore, we aim here to constrain an upper limit for it, 
rather than attempting to provide a point estimate.}

\textbf{Regarding the emissivity and albedo parameters shown in
Fig.~\ref{fig:trace-emissivity-albedo}, we see much of the same
quantitative behavior. A slight drift is visible during burn-in, 
which here was set to 100 samples for all parameters a part from the emissivities 
at 100\,$\mu$m that required more time to converge.  
All the albedoes are characterized by extremely long correlation lengths, 
and each emissivity chain for the 100--240\,$\mu$m channels appears to fall down into a local
$\chi^2$ minima after initialization and then explore a very small region
...
300 samples to match the longest convergence period among all parameters, ensuring that an 
equal number of post–burn-in samples were used for all correlations.}
This plot is highly structured, reflecting the structure of the complex ZL likelihood. 
...
\begin{figure}[h!]
    ...
    \caption{\textbf{Comparison of different phase function $\Phi(\Theta)$ normalized 
    ...
    follows the definition of Equation~\ref{eq:phase_function} and uses the values listed in 
    Table~\ref{table:zodi-params-phase} for the free parameters. By propagating the errors given in the 
    same table, it was possible to show a confidence interval of $1\,\sigma$ for $\Phi$, shown as a grey 
    area in the figure.}
    }
...
\begin{figure}[h!]
    ...
    differences, an estimate value of the CIB monopole (as evaluated in Fig.~\ref{fig:zsma_mean}) 
...
samples was tuned individually, as shown in Fig.~\ref{fig:trace-ipd} and 
...
\begin{table}
 ...
 \label{table:zodi-params-phase}
  ...
  \hline
   ...
   \hline
   $g_1$\dotfill& 0.70  & 0.07 & \pm & 0.03 & [-1, 1]\\
   $g_2$\dotfill& -0.20  & -0.51 & \pm & 0.05 & [-1, 1]\\
   $g_3$\dotfill& -0.81  & -0.84 & \pm & 0.09 & [-1, 1]\\
   \hline
   \multicolumn{6}{c}{Weights}\\
   \hline
   $w_2$\dotfill& 0.330  & 0.15 & \pm & 0.03 & [0, 1]\\
   $w_3$\dotfill& 0.005  & 0.005 & \pm & $9 \times 10^{-6}$ & [0, 1]\\
   \hline
   &&&&&\\
\end{tabular}
\end{table}
...
approximately 66\,\% and 21\,\% lower in the DR2 model than in K98. 
...
by making use of the parameters listed in Table~\ref{table:zodi-params-phase}.}
...
as already noted, we think we don't have a proper detection of it with the DIRBE data alone.}
...
\textbf{(Still needs to be changed)}
...
*1.25   & *670 & *0.022 & 1.11 \cr 
*2.2    & \cdots & *0.024 & \cdots \cr 
*3.5    & \cdots & *0.023 & \cdots \cr 
*4.9    & *151 & *0.028 & 1.48 \cr 
*12     & 5300 & *0.109 & 3.90 \cr
*25     & \cdots & *0.210 & \cdots \cr 
*60     & \cdots & *0.369 & \cdots \cr 
100     & *182 & *0.418 & 2.29 \cr 
140     & **31 & 32.033  & 1.02 \cr 
240     & **34 & 18.040  & 1.03 \cr
...
the reduced $\chi^2$ \textbf{is 3.90}, which indicates that the white noise rms
...
\begin{figure}[h!]
    ...
    and third columns of Figs.~\ref{fig:dr2-zsma-compare1}, outside the DR2 analysis masks.}}
    \label{fig:zsma_mean}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{figs/zodi_rms_ratio_new.pdf}
...
\begin{acknowledgements}
  ...
  \textsc{CMBInflate}). \textbf{This research is also funded by the Research
...
\begin{figure*}
    ...
    \caption{ZL parameter atlas showing the difference between increasing 
    and lowering each ZL model parameter by $5\%$ in the form of %normalized 
    mission-averaged ZL maps. Columns list the circum-solar ring and Earth-trailing feature components.}
    \label{fig:atlas2}
\end{figure*}
