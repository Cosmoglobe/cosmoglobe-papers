...
\begin{document} 
%\linenumbers
   \title{\bfseries{\Cosmoglobe\ DR2. III. Improved modeling of
...
% write an abstract 
   ...
   \abstract{We present a new zodiacal light (ZL) model for \textit{COBE}-DIRBE, 
   derived through global Bayesian analysis within the \cosmoglobe\ Data Release~2 (DR2) 
   framework. The parametric form of the ZL model is inspired by the original DIRBE model by Kelsall et al.\ (K98), but the specific best-fit parameter values are re-derived 
   ...
   starlight emission, and the new model takes into account excess radiation that appears 
   stationary in solar-centric coordinates, as reported in a companion paper. The relative 
   differences between the predicted signals from K98 and our new model are $\lesssim\,3\,\%$ in 
   the 12 and 25$\,\mu\mathrm{m}$ channels over the full sky.
   %The zero-levels of the cleaned DR2 
   %maps are lower than those of the K98 Zodiacal light Subtracted Mission Average (ZSMA) maps 
   %by $\sim$\,30\,kJy/sr at 1.25--3.5\,$\mu\mathrm{m}$, which is larger than the entire predicted 
   %contribution from high-redshift galaxies to the Cosmic Infrared Background (CIB) at the same 
   %wavelengths. At high Galactic latitudes, the total root-mean square of each DR2 map is lower than the corresponding 
   %DIRBE ZSMA map of $\sim$\,30\,\% at wavelengths up to and including $3.5\,\mu\mathrm{m}$, and
   %$\sim$\,80\,\% at wavelengths 4.9--25\,$\mu\mathrm{m}$.
   ...
   \textbf{Thus, in addition to better modeling of the ZL in the DIRBE data, we provide cleaner intensity maps for future large-scale Galactic science studies.
   }
   %Still, obvious ZL residuals 
   %can be seen in several of the DR2 maps, and further work is required to mitigate these. Joint 
   %analysis with existing and future high-resolution full-sky surveys such as \textit{AKARI}, \textit{IRAS}, \textit{Planck} 
   %HFI, and SPHEREx will be essential both to break key degeneracies in the current model and to 
   %determine whether the reported solar-centric excess radiation has a ZL or instrumental origin. 
   %On the algorithmic side, more efficient methods for probing massively multi-peaked likelihoods 
   %should be explored and implemented. Thus, while the current work
   %builds a novel foundation for DIRBE modeling, it also only
   %represents a first among many steps   
   %toward a future optimal Bayesian ZL model.
   }
...
the \textit{COBE}-DIRBE instrument \citep{hauser1998}, \textbf{whose 
...
Sect.~\ref{sect:zodi-model}, we introduce our ZL model and discuss
implementation and optimization aspects. \textbf{Then, in Sect.~\ref{sect:data} 
...
\textbf{ZL is commonly modeled sample-by-sample in time-domain by performing
...
\textbf{For the current \commanderthree\ implementation we adopt the K98 parametrization, 
but we use a Henyey-Greenstein phase function as defined by \citet{Hong} to model 
the scattering properties of the grains. 
...
circumsolar ring and an Earth-trailing figure, following \citet{kelsall1998} 
nomenclature); and $ds$ is a small distance along the
...
for full details of this model we refer the interested reader to
\citet{kelsall1998} and \citet{Hong}.}

%\footnote{We have also spent large 
%resources on implementing and exploring the parametric ZL model presented in 
%Appendix~1 of \citet{wright:1998}, but we have unfortunately been unsuccessful 
%with achieving goodness-of-fit statistics that are competitive with the current 
%K98-based analysis. We are uncertain whether this is due to the intrinsic lower 
%flexibility of that model or an undiscovered bug in our implementation, and we 
%therefore do not discuss this alternative model further at this time.} 
%We also consider a few numerical
%approximations that reduce the overall computational cost.
...
%resonance and disturbance in the orbit of the planets \citep{Low1984, Dermott1984, Dermott1994, Reach1997}. 
%There are also structures found in the orbits of other Solar System planets, \citep{kennedy, jones_venus, Stenborg}, 
%but these are potential extensions to the Kelsall model.  
%
...
%%p and v are different for each band so I added _i, is that okay?
%
%\subsubsection{Circumsolar ring and Earth-trailing feature}
%\label{sec:ring}
%Finally, a circumsolar ring (denoted ``SR'') component is included in the
...
%nW$\,\mathrm{m}^{-2}\,\mathrm{Hz}^{-1}\,\mathrm{sr}^{-1}$. The connection between the
...
%wavelength channel, and may refer interchangeably to both the physical
...
%temperature $T$, which is assumed to fall off with radial distance $r$ (in AU)
%from the Sun as
%\begin{equation}
%    T(r) = T_0 r^{-\delta},
%\end{equation}
%where $T_0$ is the temperature of IPD at 1\,AU and $\delta$ is a power-law 
%exponent which is expected to be $\sim 0.5$ for gray dust. 
%
%In addition to emitting thermally, IPD grains also scatter
...
%    I^\mathrm{Scattering}_{c, \lambda} = A_{c, \lambda} F_\lambda^\odot(r) \Phi(\Theta),
%\end{equation}
...
%and $\Phi(\Theta)$ is the so-called phase function for scattering angles
%$\Theta$, which describes the angular distribution of the scattered
%light. In particular, 
%our ZL model features a Henyey-Greenstein phase function as defined by 
%\citet{Hong}:
%\begin{equation}
...
%
%\begin{figure}
%  \includegraphics[width=0.85\columnwidth]{figs/figure1_1_new.pdf}\\
%  \includegraphics[width=0.9\columnwidth]{figs/figure1_2_new.pdf}\\
%  \includegraphics[width=0.9\columnwidth]{figs/figure1_3_new.pdf}\\
%  \includegraphics[width=0.9\columnwidth]{figs/figure1_4_new.pdf}
%  \caption{Geometry of the first asteroidal dust band. (\textit{First row}:) 
...
%  row}:) Observed instantaneous intensity plotted in Ecliptic 
...
%    &= A_{c, \lambda} F_\lambda^\odot \Phi + (1 - A_{c, \lambda})E_{c,\lambda} B_\lambda,
%\end{align}
%where we have also taken into account that reflective material have lower thermal emission
%by introducing a factor $(1 - A_{c, \lambda})$ to the thermal term.
%The total ZL signal may then be evaluated by summing up the intensity
%from all dust grains, which in practice means evaluating a line-of-sight 
%integral for each observation,
%\begin{equation}\label{eq:los}
...
\textbf{In order to interpret such residuals, and potentially define better
...
free parameter. Appendices~\ref{sec:zodi-comps} and \ref{sec:param-atlas} 
display various projections of the three-dimensional IPD number density 
distribution for different copmonents, as well as maps showing the 
impact of each parameter included in the model on the total observed 
signal. Quickly scanning through the individual panels in
Figs.~\ref{fig:atlas1} and \ref{fig:atlas2}, we can already now
identify strong degeneracies that are likely to turn out to be problematic
...
%
%The second panel in Fig.~\ref{fig:band1} shows the corresponding
...
%seen outward from the observer. In this projected 2D space, the
...
%the second panel; they appear narrower when looking toward the Sun,
...
%Fig.~\ref{fig:band1} shows the difference between the total signal
%obtained when changing the ascending node $\Omega$ for Band~1 by 5\,\%
%relative to a suitable set of parameters for our ZL model. 
%Intuitively, this corresponds to rotating
...
%are very useful for building up visual intuition regarding the ZL 
%model in use. Quickly scanning through the individual panels in
%Figs.~\ref{fig:atlas1} and \ref{fig:atlas2}, we can already now
%identify strong degeneracies that are likely to turn out to be problematic
%later. For instance, we see that $n_{0,\mathrm{C}}$,
%$\alpha_\mathrm{C}$, $n_{0,\mathrm{SR}}$, $T_0$, and $\delta$ are all
...
%A subsample of the TOD for each DIRBE band as used in our analysis can
...
parameters. The \textbf{DIRBE TOD} are clearly not able to constrain this
...
%As summarized by Eq.~\eqref{eq:model}, the \cosmoglobe\ DR2 data model
%includes a wide range of both astrophysical and instrumental
%parameters. The DIRBE CIO are clearly not able to constrain this
...
%
%The third ancillary data set is \GAIA\ DR2 \citep{gaia:2018}. We attempt to match each
...
%25\,$\mu$m, and normalized to unity at 1.25\,$\mu$m; only one overall
...
The main operational goal of this paper is to \textbf{fit} the free ZL
...
the interested reader to \citet{CG02_01}. However, we
...
%In order for this to work, we have to define $\s_{\mathrm{tot}}$ as the sum 
%of all components but $\n_\mathrm{w}$, so that $\n_\mathrm{w}=\dv-\s_{\mathrm{tot}}$ 
%is a Gaussian distribution with 
%zero mean and covariance matrix $\N_{\mathrm{w}}$ that also has free parameters. 
%For most instruments this is an excellent approximation. With this assumption, we can
...
%  &\equiv \chi^2(\omega) +\ln|\N_{\mathrm w}|,
%\end{align}
...
In order to map out this full joint posterior, \textbf{we use Gibbs 
...
%Sampling from $N$ conditional distributions, each
...
%does not have a closed form analytical expression. 
%In practice, for
...
%          \s_{\mathrm{static}} + \n_\mathrm{corr}),
%\end{equation}
%and this should ideally only contain ZL and white instrumental noise.
%
%  \footnote{\textbf{My point is this noise 
%is the residual we will get in the end, stuff that we can't resolve (right?) so I would add "unmodeled" to not 
%confuse this with instrumental noise or other stuff. One thing I don't know where to write is that it 
%is my understanding that we also sample $N_\mathrm{w}$ at the beginning of a Gibbs step, so that 
%then we can say -log likelihood = chisquare (because at that point it's like chisquare given covariance). 
%Since previously we said that this has free parameters and 
%therefore cannot just be skipped, I think we should say something to justify what follows. \textit{Take out this note!!!}}
% HKE: No, note that there is the word "ideally" there. When defining the posterior, we actually do assume that the model is correct. Unmodelled noise appears in the goodness-of-fit tests. The fact that the white noise also has free parameters is irrelevant for the ZL Gibbs sampling step; it only affects the noise sampling step itself. Gibbs sampling is great :-)
%As such, the assumption that this noise is also Gaussian defines the
%conditional distribution in Eq.~\eqref{eq:gibbs_zodi}, and we may
%write
%\begin{equation}
...
%For instance, a yet unknown zodiacal
%component could create a signal that appears stationary in
%solar-centric coordinates, just like the circumsolar ring, or it could also be caused by stray-light
%contamination in the DIRBE optics. 
Even though this radiation
...
model. 
%However, before that is done, it is imperative to rule out the
%stray-light hypothesis, and that will require both detailed modeling
%of the DIRBE instrument and joint analysis with other experiments,
%such as \AKARI, \IRAS, \Planck\ HFI, and SPHEREx. Doing that is beyond the scope
...
sampling from \textbf{the posterior distribution of zodiacal light 
...
statistic. That is, rather than optimizing the full \textbf{data-minus-ZL model} 
...
full-mission mean, \textbf{corresponding to the signal used during 
...
%samples that are close to known bright Galactic sources; this is fully
%equivalent to what is done in the CMB field when estimating the CMB
%power spectrum.

\textbf{The bottom panel in particular is useful to visualize 
...
\textbf{the posterior distribution of zodiacal light 
...
%    \caption{Reduced $\chi^2$ as a function of the temperature at 1\,AU, $T_0$, for the same run shown 
%    in Fig.~\ref{fig:powell_chisq_iter}. Each curve shows the full set of parameter trials within one 
%    single main Gibbs iteration (or Powell call), and different colors indicate different Gibbs iterations. 
%    Redder colors are earlier in the chain.}
%    \label{fig:powell_T0}
%\end{figure}
...
%\begin{figure*}[t]
%    \centering
%    \includegraphics[scale=0.8]{figs/traceplot_tot_shape_new.pdf}
%    \caption{Traceplots of all the geometrical and shape IPD parameters kept free in 
%    our model (\textit{Cloud} and \textit{Bands}), the power-law exponent of the IPD
%    temperature and the phase function parameters (\textit{General}), along with the full-sky averaged 
%    ZL intensity traces for six selected DIRBE channels (\textit{Mean sky $Z_\nu$}). 
%    Different colors indicate five independent Markov chains, the gray regions indicate the burn-in set for each 
%    parameter, and vertical axes were cropped to emphasize convergence.}
%    \label{fig:trace-ipd}
%\end{figure*}
%
%\begin{figure*}
%    \centering
%    \includegraphics[width=0.985\textwidth]{figs/traceplot_tot_wl_new.pdf}
%    \caption{Traceplots of the wavelength dependent emissivity and albedo parameters. 
%    Different colors indicate five independent Markov chains, the gray regions indicate the burn-in set for each 
%    parameter, and vertical axes were cropped to emphasize convergence.}
...
8\,Hz \textbf{DIRBE TOD}. In principle, we could have averaged over this time
...
full-mission maps as \citet{kelsall1998} did. \textbf{Since with 
...
%Markov chains for each one of the 36 free ZL parameters in $\zeta_{\mathrm{z}}$. 
%These are shown for the general ZL shape parameters in
%Fig.~\ref{fig:trace-ipd} and for the amplitude (emissivity and albedo)
%parameters in Fig.~\ref{fig:trace-emissivity-albedo}. The gray regions indicate the discarded burn-in, 
%which, at this stage, was tuned for each parameter individually. 

%\footnote{Due to a parameter file error introduced before the final production run, the emissivity
% for asteroidal bands for the 25$\,\mu$m band, $E_{\mathrm{6,B}}$, was accidentally fixed at the best-fit value
%  derived in the penultimate run. This parameter is thus not marginalized over in the current results, but rather
%   fixed at a reasonable value, similar to the parameters that are fixed at their K98 values. 
%   The signal-to-noise ratio of the asteroidal bands is, however, very low, accounting only for a few percent of 
%   the total ZL signal, and neglecting the uncertainties in this particular parameter has therefore a very small impact 
%   on other results.}  



\textbf{As in any MCMC-based analysis, as part of this work traceplots of the 
...
we found that we were unable to constrain the signal in dust band 3 with the DIRBE data alone, 
as the band would effectively disperse. This phenomenon was previously noted by \citet{Spiesman}. 
...
%parameters. It should be noted that the intial value for these parameters are the best-fit 
%parameters of previous trial runs and not the final K98 parameters.
%
%From the component-wise offsets ($x_0$, $y_0$, $z_0$), we see that the $x_0$ 
%parameters all appear to have hit the prior limits of $\pm 0.02$ AU, and the $y_0$ parameters 
%for the dust bands also hit the floor set by the prior. 

%Examining the traceplots helps to evaluate the degree of convergence for individual parameters. Starting
%with the shape parameters for the dominant cloud component shown in
%the leftmost column in Fig.~\ref{fig:trace-ipd}, we see that many
%parameters appear to drift slightly during the first 30--50 samples or so, 
%but after that, they become more stationary. We also
%see that the mixing between different chains for $\gamma$ and $\mu$ is notably slower than
%what is common for most Monte Carlo methods discussed in the
%literature, and this indicates rather long correlation lengths. A similar behavior is observed for the 
%phase function parameters, as shown in the bottom plots in the central column of Fig.~\ref{fig:trace-ipd}. Future
%algorithm development work will aim at decreasing
%these correlation lengths through better sampling algorithms, as well
%as reducing the cost per Monte Carlo sample, such that longer Markov
%chains can be produced. Still, even with the currently available
%limited sample set, it does appear possible to derive sensible
%estimates of both the posterior mean and standard deviation for most
%parameters. It should be noted that the intial values for these parameters are the best-fit 
%parameters of previous trial runs and not the final K98 parameters.

%Looking at the dust bands' parameters (uppermost plots in the central column of Fig.~\ref{fig:trace-ipd}), 
%it is worth noting that the number density of dust band 3 
%seems to be reaching a prior bound. As already pointed out, with the DIRBE data this band is very faint 
%and cannot be robustly measured. Therefore, we aim here to constrain an upper limit for it, 
%rather than attempting to provide a point estimate. A similar effect characterizes the $g_3$ phase function 
%parameter, which consequently appears to be prior-determinated.
%%\textbf{Something to say about the Mean zodi column?}
%
%Regarding the emissivity and albedo parameters shown in
%Fig.~\ref{fig:trace-emissivity-albedo}, we see much of the same
%quantitative behavior. A slight drift is always visible during burn-in, 
%which was set to 30 samples for all parameters apart from the emissivity 
%at 240\,$\mu$m and the albedoes that required more time to converge, 
%but then all the traceplots seem quite stable. Long correlation lenghts are noticeable 
%in the albedoes plots, which could benefit from the algorithmic improvements already noted.

%For these channels we also observe
%a number of apparent phase transitions, in which the Markov chains
%suddenly move from one stationary state to another, most notably in the curve denoted by the red line, 
%which contains oscillatory features in almost every parameter. This behavior is
%another manifestation of the complex multi-peaked ZL likelihood
%surface discussed in Sect.~\ref{sec:nonlin}. 
...
%wavelengths is simply very faint, and therefore has limited significance. 
...
%Particularly strong are the correlations between the reference temperature, 
%$T_0$, and the emissivity and albedo parameters, $E_i$ and $A_i$. 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figs/phase_function_norm.pdf}
    \caption{Comparison of different parametrizations of the phase function $\Phi(\Theta)$, normalized 
    ...
    in the frequency channels 1.25 -- 3.5\,$\mu$m; the black dashed line is the phase function 
    defined in \citet{Hong}; while the solid black line is the DR2 function. This final one 
...
\begin{figure}[t]
    \centering
    \vspace{1pt}
    \includegraphics[width=\columnwidth]{figs/zodi_intensity_new.pdf}
    \caption{(\textit{Top panel}:) ZL intensity as a function of wavelength on the 2024-01-01
      ...
      models. Colors show three different positions on the sky with Galactic coordinates 
      ($l=39.91^\circ,\,b=-58.70^\circ$), ($l=96.38^\circ,\,b=29.81^\circ$), and ($l=191.41^\circ,\,b=8.68^\circ$) 
      corresponding to the solar elongation at $90^\circ$, the North Ecliptic Pole, 
      and the antisolar point, respectively. The black curves instead show the full-sky average. 
      (\textit{Bottom}:) Fractional difference between the DR2 and 98 models, 
      evaluated from the results shown in the top panel.
    }
...
\begin{figure*}[t]
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_01.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_01.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_01.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_01.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_01.pdf}%
    %}
    \\

    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_02.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_02.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_02.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_02.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_02.pdf}%
    %}
    \\

    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_03.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_03.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_03.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_03.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_03.pdf}%
    %}
      \\

    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_04.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_04.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_04.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_04.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_04.pdf}%
    %}
      \\

    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_05.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_05.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_05.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_05.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_05.pdf}%
    %}
      \\

    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_06.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_06.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_06.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_06.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_06.pdf}%
    %}
      \\

    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_07.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_07.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_07.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_07.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_07.pdf}%
    %}
      \\
    \caption{\textbf{\textit{(Left column):} Official DIRBE zodiacal-light-subtracted mission-averaged frequency 
...
\begin{figure*}[t]
    \centering
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_08.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_08.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_08.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_08.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_08.pdf}%
    %}
    \\

    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_09.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_09.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_09.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_09.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_09.pdf}%
    %}
      \\

    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/K98/k98_zsma_10.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/DR2/DR2_zsma_10.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/cbar/cbar_10.pdf}%
    \includegraphics[height=2.90cm]{figs/zsma_new/diff/zsma_diff_10.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/diff/zsma_diff_cb_10.pdf}%
    %}
      \\
...
%At the same time, we also notice that the amplitude of the first band is 10 times
...
%as already noted, we cannot conclude that we have a detection of it with the DIRBE data alone.
...
with those reported by \citet{kelsall1998} and \citet{Hong}. All the models are 
...
in our function lies between the Hong et al.\ and K98 minima. The biggest discrepancy is 
...
predictions, respectively. The bottom panel shows the relative
difference. In general, the two models agree to better than 10\,\% for
most wavelengths, except at the very longest, 
for which the overall amplitudes are very uncertain. 
%, and near the $4.9\,\mu$m channel where we have fit a free albedo. 
...
detailed model of ZL in the Ecliptic plane; again, joint analysis with
high-resolution measurements from \IRAS\ and \AKARI\ should prove
...
\textbf{Figures~\ref{fig:dr2-zsma-compare1} and \ref{fig:dr2-zsma-compare2}
...
%\begin{figure}[t]
%    \centering
%    \includegraphics[width=\linewidth]{figs/zodi_mean_diff_new.pdf}
%    \caption{Monopole difference between the official K98 and 
%    \cosmoglobe\ DR2 ZSMA maps, evaluated as the average of the 
%    difference between the maps at 1.25--25$\,\mu$m, as shown in the second 
%    and third columns of Fig.~\ref{fig:dr2-zsma-compare1}, outside the DR2 analysis masks.}
%    \label{fig:zsma_mean}
%\end{figure}

%\begin{figure}[t]
%    \centering
%    \includegraphics[width=0.98\linewidth]{figs/zodi_rms_ratio_new.pdf}
...
%One of the primary scientific goals of DIRBE was
...
%calculation, but this time in the form of the RMS ratio of the ZSMA
%maps evaluated outside the same masks. Here we see that the DR2 maps
%have lower RMS than the K98 maps in all the channels between 1.25 and 25\,$\mu$m, which once
%again indicates that more ZL emission has been removed from the new maps.
%Here we see that the DR2 maps
...
zodiacal light model (e.g., \citet{skysurf11}).
...
%The IPD number density visualization for the K98 model, 
...
In this Appendix, we present an atlas of mission-averaged ZL parameter maps \textbf{for the $12\,\mu$m band}.
Each map shown in Figs.~\ref{fig:atlas2} and ~\ref{fig:atlas1} 
represents the effect of changing one ZL model parameter by $\pm 5\%$ while holding 
\textbf{the other fixed. In the top-left corner of each map the negative and positive 
limits of the colorbar are given expressed in units of MJy/sr, while in the top-right corner 
the parameter names are displayed. Apart from the general parameters $T_0$ and $\delta$, 
the maps are organized in columns according to the components they refer to: 
circumsolar ring (SR) and Earth-trailing feature (TF) in Fig.~\ref{fig:atlas2}, 
smooth cloud (C), dust band 1 (B1), dust band 2 (B2), and dust band 3 (B3) in Fig.~\ref{fig:atlas1}.}

\begin{figure*}

  \centering
  \resizebox{0.44\textwidth}{!}{%
  \includegraphics[height=4.5cm]{figs/atlas_new/atlas_61.pdf}%
  \hspace{0.3cm}
  \includegraphics[height=4.5cm]{figs/atlas_new/atlas_62.pdf}%
  }

  \resizebox{0.95\textwidth}{!}{%
  \begin{tabular}{cccc}
      {\Large SR} & {\Large SR} & {\Large TF} & {\Large TF} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_41.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_46.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_50.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_55.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_42.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_47.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_51.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_56.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_43.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_48.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_52.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_57.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_44.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_49.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_53.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_58.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_45.pdf} &
      \rule{0pt}{4.5cm} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_54.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_59.pdf} \\

      \rule{0pt}{4.5cm} &
      \rule{0pt}{4.5cm} &
      \rule{0pt}{4.5cm} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_60.pdf} \\
  ...
  \end{tabular}%
    ...
    mission-averaged ZL maps. \textbf{The top row show the general parameters, 
    then columns list the parameters defining the circumsolar ring (SR) and Earth-trailing feature (TF) components. 
    In each map parameter names are indicated in the top-right corner, while the 
    limits of the colorbar expressed in units of MJy/sr are shown in the top-left corner.}    
    }
    \label{fig:atlas2}
\end{figure*}
...
\begin{figure*}[hbt]
  ...
  \resizebox{0.95\textwidth}{!}{%
  \begin{tabular}{cccc}
      {\Large C} & {\Large B1} & {\Large B2} & {\Large B3} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_01.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_11.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_21.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_31.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_02.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_12.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_22.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_32.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_03.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_13.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_23.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_33.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_04.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_14.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_24.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_34.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_05.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_15.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_25.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_35.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_06.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_16.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_26.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_36.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_07.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_17.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_27.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_37.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_08.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_18.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_28.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_38.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_09.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_19.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_29.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_39.pdf} \\

      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_10.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_20.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_30.pdf} &
      \includegraphics[height=4.5cm]{figs/atlas_new/atlas_40.pdf}
  \end{tabular}%
  }

  \caption{ZL parameter atlas showing the difference between increasing
  and lowering each ZL model parameter by $5\%$ in the form of
  normalized mission-averaged ZL maps. \textbf{Columns list parameters of the smooth cloud (C) 
  and the three dust bands ((B1), (B2), and (B3) respectively). In each map parameter 
  names are indicated in the top-right corner, while the 
  limits of the colorbar expressed in units of MJy/sr are shown in the top-left corner.}}
  \label{fig:atlas1}
\end{figure*}
