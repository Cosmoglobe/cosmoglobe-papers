...
\caption{Reduced goodness-of-fit ($\chi_\mathrm{red}^2$) values along with the number of TODs used to fit the ZL parameters and the instrument white noise levels for each DIRBE channel. Note that the 12 and 25$\,\mu$m channels are analyzed jointly, and $N_{\mathrm{samp}}$ and $\chi^2_{\mathrm{red}}$ correspond to the union of the two.}
\label{tab:chisq}
\vskip -4mm
\footnotesize
\setbox\tablebox=\vbox{
