...
zero-level of the ZL model. On the one hand, such blindness may
certainly be considered as a strength of the algorithm, as indeed
argued by \citet{Kelsall1998}, given that one of the main goals of the
entire DIRBE experiment was to precisely measure the CIB monopole
spectrum \citep{hauser1998}. At the same time, the final derived CIB
constraints do of course still depend directly on monopole of the
reconstructed monopole. Therefore, if the ZL monopoles have a large
uncertainty, then so do the CIB monopoles. Furthermore, differential
measurements are in general far much more susceptible to bias from
systematic modelling errors from sources other than Galactic confusion
than absolute intensity measurements, including optical
non-idealities. Intuitively speaking, the differential method aims to
measure the ZL monopole using only derivatives, and whether that task
is easier or harder than to establish a sufficiently accurate model of
