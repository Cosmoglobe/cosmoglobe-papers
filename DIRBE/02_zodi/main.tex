...
% write an abstract 
   ...
   Kelsall et al. (1998; K98), but the specific best-fit parameter values are re-derived 
...
law exponent which is expected to be $\sim 0.5$ for grey dust. In addition to emitting thermally, IPD grains also scatter
...
\end{align}}
where we have additionally taken into account that reflective material have lower thermal emission
by adding a factor $(1 - A_{c, \lambda})$ to the thermal term.
...
\begin{figure*}
    ...
    \caption{\textbf{Trace plots of all geometrical and shape IPD parameters, along with the power-law index of the IPD
...
Markov chain. We remove the first \textbf{100} samples from each chain as
...
and cannot robustly measured. Therefore, we aim here to constraining an upper limit for it, 
...
provided in Table~\ref{table:zodi-params-source}, \textbf{and the phase function parameters 
can be found in Table~\ref{table:zodi-params-phase}}. For comparison, the
corresponding values from \citet{kelsall1998} \textbf{and \citet{Hong}} are listed in the second
...
\begin{table}
  ...
  \caption{\textbf{Best-fit phase function parameter estimates and uncertainties in the DR2 analysis,
    comparing values with the \citet{Hong}.}}
