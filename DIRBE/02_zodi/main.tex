...
exponent which is expected to be $\sim 0.5$ for gray dust. In addition to emitting thermally, IPD grains also scatter
...
\begin{figure*}[h!]
    \centering
    \includegraphics[scale=0.75]{figs/traceplot_tot_shape_new.pdf}
...
identifies region with a high \textbf{signal-to-noise ratio (S/N)} for ZE, 
...
leaving a total of 1060--960 samples for final analysis. In total,
...
Markov chains for \textbf{each one of the 36 free ZL parameter in $\zeta_{\mathrm{z}}$}. 
...
seems to be reaching a prior. As already pointed out, with the DIRBE data this band is very faint 
...
which was set to 30 samples for all parameters a part from the emissivity 
