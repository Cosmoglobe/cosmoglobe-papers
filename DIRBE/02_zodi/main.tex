...
Zodiacal light (ZL, sometimes zodiacal emission or interplanetary dust emission) is the primary source of diffuse radiation observed in the infrared sky between 1-100 $\mu$m \cite{leinert1998}. The emission comes from scattering and re-emission of sunlight from interplanetary dust grains. This results in a spectra that looks like two blackbodies, with one peak near ~15$\mu$m corresponding to the thermal emission and another near ~0.5 $\mu$m corresponding to the scattered sunlight.

Foregrounds, such as the CMB and galactic thermal dust emission, are assumed to be stationary in the sky and are usually modeled by a single template describing the structure of the component at some reference frequency. The template can then be scaled to arbitrary frequencies given a description of the component's spectral energy density (SED). However, this is not the case for the zodiacal light, which highly depends on the position of the observer within the Solar system and the scanning strategy of the telescope. The ZL is a local foreground originating from the near vicinity of the observer. As such, it is impossible to describe and model the zodiacal light foreground through a single template, applicable to all experiments. Instead, the zodiacal light must be dynamically modeled on a per-experiment basis. In this paper, we will detail the modeling of zodiacal light in the Commander framework, both in terms of the interplanetary dust model used but also from a Bayesian perspective, utilizing all DIRBE bands jointly along with HFI and FIRAS to produce the best zodiacal light model of the infrared sky.
