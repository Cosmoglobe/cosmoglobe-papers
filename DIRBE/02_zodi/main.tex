...
% write an abstract 
   ...
   \abstract{We present an improved zodiacal light (ZL) model for \textit{COBE}-DIRBE, 
   derived through global Bayesian analysis within the \cosmoglobe\ Data Release~2 (DR2) 
   framework. The parametric form of the ZL model is \textbf{inspired by the original DIRBE model by Kelsall et al.\ (K98)}, but the specific best-fit parameter values are re-derived 
   ...
   differences between the predicted signals from K98 and our new model \textbf{are $\lesssim\,3\,\%$ in} 
   the 12 and 25$\,\mu\mathrm{m}$ channels over the full sky. The zero-levels of the cleaned DR2 
   maps are lower than those of the K98 Zodiacal light Subtracted Mission Average (ZSMA) maps 
   by \textbf{$\sim$\,30\,kJy/sr at 1.25--3.5\,$\mu\mathrm{m}$}, which is larger than the entire predicted 
   contribution from high-redshift galaxies to the Cosmic Infrared Background (CIB) at the same 
   wavelengths. \textbf{At high Galactic latitudes, the total root-mean square of each DR2 map is lower than the corresponding 
   ...
   decades, \textbf{appears cleaner} in the current processing. Still, obvious ZL residuals 
   ...
   should be explored and implemented. Thus, while the results presented in this paper do redefine 
   the state of the art for DIRBE modeling, it also only represents the first among many steps 
...
\citealp{partridge1967}). {\textbf{The main
    scientific motivation of the \textit{COBE}-DIRBE experiment \citep{hauser1998,hauser:2001} was to detect
    and characterize the CIB spectrum.}
...
the \textit{COBE}-DIRBE instrument \citep{hauser1998}. There have been 
...
papers collectively denoted \cosmoglobe\ \textbf{Data Release 2 (DR2)}, is to make
...
accurate ZL model to remove the time-varying ZL. In this paper, we therefore implement support in \texttt{Commander3} for \textbf{the K98 model, combined with the scattering phase function described 
...
Sect.~\ref{sect:zodi-model}, \textbf{we introduce our ZL model} and discuss
...
three-dimensional model of the IPD distribution. \textbf{We adopt a modified version of the 
...
underlying parametric model. \textbf{For full
details of this model, we refer the interested reader to
\citet{kelsall1998} and \citet{Hong}.\footnote{\textbf{We have also spent large resources on implementing and exploring the parametric ZL model presented in Appendix~1 of of \citet{wright:1998}, but we have unfortunately been unsuccessful with achieving goodness-of-fit statistics that are competitive with the current K98-based analysis. We are uncertain whether this is due to the intrinsic lower flexibility of that model or an undiscovered bug in our implementation, and we therefore do not discuss this alternative model further at this time.}} We also consider a few numerical
...
and \textbf{$\Phi(\Theta)$} is the so-called phase function for scattering angles
$\Theta$, which describes the angular distribution of the scattered
light. \textbf{In particular, 
...
\begin{figure}
  ...
  \caption{Geometry of the \textbf{first} asteroidal dust band. (\textit{First row}:) 
  ...
  (\textit{Fourth row}:) \textbf{Difference} between observed
...
\textbf{\begin{equation}\label{eq:los}
...
line-of-sight $s$ from the observer and toward $p$. \textbf{Due to the limited signal-to-noise ratios, we follow
...
As described above, \textbf{our model} has only $\mathcal{O}(10^2)$ free
...
free parameter. Fig.~\ref{fig:band1} shows one specific (and
arbitrary) example of this. First, the top panel shows a
$x$--$z$-plane slice through the three-dimensional IPD number density
distribution for the \textbf{first dust band, $B_1$.} In this figure, the
...
Fig.~\ref{fig:band1} shows the \textbf{difference} between the total signal
obtained when changing the ascending node $\Omega$ for \textbf{Band~1} by 5\,\%
relative to \textbf{a suitable set of parameters for our ZL model}. 
...
are very useful for building up visual intuition regarding \textbf{the ZL 
...
8\,Hz DIRBE CIO. In principle, we could have averaged over this time
...
is vastly dominated by the \textbf{12, 25, and 60\,$\mu$m channels}. At the same
...
corresponding emissivities, albedos and monopoles) using only the 
\textbf{12, 25, and 60\,$\mu$m channels}, and then we estimate the emissivity, albedo,
and monopole for each of the remaining channels separately --- but now
conditionally on the shape parameters derived \textbf{before}. 
...
but the \textbf{advantage} is a computational speed-up of roughly one
...
unknown SED parameters, $\beta$; \textbf{and $\n=\n_\mathrm{corr} + \n_\mathrm{w}$ denotes instrumental
...
\citet{CG02_01}. \textbf{The foreground model is treated in more 
...
In order for this to work, \textbf{we have to define $\s_{\mathrm{tot}}$ as the sum 
of all components but $\n_\mathrm{w}$, so that $\n_\mathrm{w}=\dv-\s_{\mathrm{tot}}$, 
is Gaussian distribution with 
zero mean and a covariance matrix $\N_{\mathrm{w}}$ that also has free parameters. 
...
\begin{figure*}
    ...
    \caption{\textbf{Illustration of the basic sky maps involved in the ZL 
...
Related to this, we also note that the circumsolar ring and trailing
...
Fig.~\ref{fig:week_vs_full} illustrates this difference. The top
...
visually. The middle row shows the same for the \textbf{ZL signal as predicted by the K98 model},
...
samples that are \textbf{close to known} bright Galactic sources; this is fully
equivalent to what is done in the CMB field when estimating the CMB
power spectrum.
%are we sure about the last sentence? samples close to bright sources?
...
\textbf{DIRBE mission} was to precisely measure the CIB monopole spectrum
...
The algorithm is defined as follows: \textbf{we start by initializing all free
...
\begin{figure}[t]
    ...
    single main Gibbs iteration (or Powell call), and different colors indicate different Gibbs iteration. 
...
in \textbf{a preliminary analysis run} for the 12+25\,$\mu$m ZL sampling
group; we show \textbf{the preliminary} chain here rather than the final
...
\begin{figure*}[t]
    ...
    \caption{\textbf{Trace plots of all the geometrical and shape IPD parameters kept free in 
...
\begin{figure*}
    ...
    \caption{\textbf{Trace plots of the wavelength dependent emissivity and albedo parameters. 
    Different colors indicate five independent Markov chains, the gray regions indicate the burn-in set for each 
    parameter, and vertical axes were cropped to emphasize convergence.}}
...
\begin{figure*}
    \centering
    \includegraphics[scale=0.9]{figs/correlation_matrix_new.pdf}
...
In the following, we will refer to the pre-processed CIO as TOD.  A
...
\textbf{As summarized by Eq.~\eqref{eq:model}, the \cosmoglobe\ DR2 data model
...
identifies regions with a high \textbf{signal-to-noise ratio (S/N)} for ZE, 
...
algorithm outlined in Sect.~\ref{sect:param-estimation}. \textbf{We have
produced a total of 1210 full Gibbs samples, distributed over five
...
Markov chains for \textbf{each one of the 36 free ZL parameters in $\zeta_{\mathrm{z}}$}. 
These are shown for the general ZL shape parameters in
Fig.~\ref{fig:trace-ipd} and for the amplitude (emissivity and albedo)
parameters in Fig.~\ref{fig:trace-emissivity-albedo}. \textbf{The gray regions indicate the discarded burn-in, 
...
Note that we are not attempting to fit the geometrical parameters of \textbf{the three dust bands}, 
but only \textbf{their} number densities. \textbf{In particular,} throughout our many attempts, 
we found that we were unable to constrain the signal in \textbf{dust band 3} with the DIRBE data alone, 
as the band would effectively disperse. \textbf{This phenomenon was previously noted by \citet{Spiesman}. 
...
kept fixed at 286\,K. The same approach was already adopted by \citet{kelsall1998}. Finally, the 25\,$\mu$m emissivities for the ring and feature were fixed at the K98 value, as these are strongly degenerate with the solar-centric component discussed by \citet{CG02_01}. }
...
\textbf{Examining the trace plots helps to evaluate the degree of convergence for individual parameters. Starting
with the shape parameters for the dominant cloud component shown in
the leftmost column in Fig.~\ref{fig:trace-ipd}, we see that many
...
\textbf{Looking at the dust bands' parameters (uppermost plots in the central column of Fig.~\ref{fig:trace-ipd}), 
...
\textbf{Regarding the emissivity and albedo parameters shown in
...
evaluated directly from the post-burn-in Gibbs sample set. \textbf{Here, the burn-in was set to 
...
\textbf{Particularly strong are the correlations between the emissivity parameters in the 60--240\,$\mu$m channels.} 
As already noted above, fitting a smooth parametric ZL SED function rather than individual amplitude
factors at each \textbf{of these} channels should be beneficial in many ways; both in
...
\begin{figure}[t]
    ...
    \caption{\textbf{Comparison of different parametrizations of the phase function $\Phi(\Theta)$, normalized 
    ...
    follows the definition of Eq.~\ref{eq:phase_function} and uses all the post-burn-in parameters 
...
\begin{figure}[t]
    ...
    \caption{(\textit{Top panel}:) ZL intensity as a function of wavelength on the 2024-01-01
      ...
      models. \textbf{Colors show three different positions on the sky with Galactic coordinates 
...
\begin{figure}[t]
    ...
    \caption{\textbf{Relative fractional difference between mission averaged ZL simulations 
    of our best-fit model and K98 at 12 and 25\,$\mu$m. To highlight structural zodiacal light 
    differences, an estimate value of the ZL monopole (as evaluated in Fig.~\ref{fig:zsma_mean}) 
...
post-burn-in samples. \textbf{For each parameter the number of burn-in 
samples was tuned individually, as shown in Figs.~\ref{fig:trace-ipd} and 
\ref{fig:trace-emissivity-albedo}. The resulting values for the shape parameters
are tabulated in the fourth column of
Table~\ref{table:zodi-params-geo}, the amplitude parameters are
provided in Table~\ref{table:zodi-params-source}, and the phase function parameters 
can be found in Table~\ref{table:zodi-params-phase}. For comparison, the
corresponding values from \citet{kelsall1998} and \citet{Hong} are listed in the third
...
\begin{table}
  \small
  \centering
  \caption{\textbf{Best-fit phase function parameter estimates and uncertainties in the DR2 analysis,
...
\begin{figure*}[t]
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_01.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_01.pdf}%
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_02.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_02.pdf}%
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_03.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_03.pdf}%
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_04.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_04.pdf}%
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_05.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_05.pdf}%
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_06.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_06.pdf}%
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_07.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_07.pdf}%
    ...
    %}
      \\
    \caption{\textbf{\textit{(Left column):} Mission-averaged frequency maps containing ZL after data 
    selection. \textit{(Middle column):} Official DIRBE ZSMA maps. \textit{(Right column):} Our ZL subtracted mission-average 
    maps. All the maps are at our native HEALPix resolution of $N_\mathrm{side} = 512$. Rows show, from top to 
...
\begin{figure*}[t]
    \centering
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_08.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_08.pdf}%
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_09.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_09.pdf}%
    ...
    %\resizebox{\textwidth}{!}{%
    \includegraphics[height=2.90cm]{figs/zsma_new/tot/Inu_zsma_10.pdf}%
    \includegraphics[width=2.90cm,angle=90]{figs/zsma_new/tot/Inu_cbar_10.pdf}%
...
uncertainties. For instance, \textbf{the density parameters of dust bands 1 and 2 are 
...
for both models per component. Here we see visually that \textbf{in the DR2 maps both the 
smooth cloud and the three dust bands are fainter than in the K98 maps. 
The intensity of the third asteroidal band is particularly weak in our model but, 
...
\textbf{Figure~\ref{fig:phase_function} compares our new phase function, $\Phi(\Theta)$, with those reported by \citet{kelsall1998} and \citet{Hong}. All the models are normalized over the interval 
$\Theta \in [65^\circ,\,125^\circ]$, since this is the interval of 
solar elongation angle that DIRBE is able to observe. However, when comparing these it is worth noting that most of the DIRBE constraining power lies between 80 and $110^{\circ}$, and the visual impression in this figure does depend on the specific range used for normalization. With this caveat in mind, we see that the minimum (which is independent of normalization) in our function lies between the Hong et al.\ and K98 minima. The biggest discrepancy is seen at very low scattering angles, where our phase function lies about $4\,\sigma$ lower than Hong et al. In that respect, it is worth noting that the Hong et al.\ analysis included datasets at very low solar elongations. A future \Cosmoglobe\ re-analysis of the current model may want to consider including those as well.  }
...
most wavelengths, \textbf{except at the very longest, 
...
brightest, the K98 and \cosmoglobe\ DR2 models agree to \textbf{within
...
\caption{\textbf{Reduced goodness-of-fit ($\chi_\mathrm{red}^2$) values for one arbitrarily 
...
months of the mission, \textbf{we see a faint band that follows the Ecliptic
...
\textbf{As noted in Sect.~\ref{sect:data}, we remove the first week of
observations to remove instabilities at both the beginning and end of
the DIRBE survey. For the 4.9--100$\,\mu$m channels, a whole month is
...
are seen as large gray regions in the two half-mission data. After
...
residuals. Any improvement in the ZL model for these channels should
therefore be accompanied by corresponding improvements in the thermal
dust modeling.
...
Table~\ref{tab:chisq}; \textbf{note that the 1.25--3.5$\,\mu$m and the 12--60$\,\mu$m channels are
...
very close to unity, namely \textbf{140 and 240$\,\mu$m}.

The biggest outliers are  the \textbf{12--60$\,\mu$m} channels, which
have the highest signal-to-noise ratio to ZL emission. In this case,
the reduced $\chi^2$ \textbf{is 2.17}, which indicates that the white noise RMS
...
compare the K98 ZSMA maps (second column) with the new
\cosmoglobe\ DR2 ZSMA maps (third column), as well as with the
corresponding non-ZL-subtracted maps (first column). 
\textbf{The 12--60\,$\mu$m DR2 ZSMA maps present negative 
...
\begin{figure}[t]
    ...
    \caption{\textbf{Monopole difference between the official K98 and 
...
Fig.~\ref{fig:zsma_rms} shows the results from a similar
calculation, but this time in the form of the RMS ratio of the ZSMA
maps evaluated outside the same masks. \textbf{Here we see that the DR2 maps
...
\textbf{Furthermore, future analyses will also include SPHEREx data, 
...
\begin{acknowledgements}
  ...
  772253 (ERC; \textsc{bits2cosmology}), \textbf{101165647 (ERC;
  \textsc{Origins}), 101141621 (ERC; \textsc{Commander})}, and 101007633 (MSCA;
  \textsc{CMBInflate}). \textbf{This article reflects the views of the authors only. The funding
...
\textbf{First, Fig.~\ref{fig:ipd-number-density} shows cross-sections of the 
...
Each map \textbf{shown in Figs.~\ref{fig:atlas1} and ~\ref{fig:atlas2}} 
represents the effect of changing one ZL model parameter by $\pm 5\%$ while holding the other fixed. %The maps are normalized.
...
\begin{figure*}
    ...
    \caption{ZL parameter atlas showing the difference between increasing 
    and lowering each ZL model parameter by $5\%$ in the form of %normalized 
    mission-averaged ZL maps. Columns list the circumsolar ring and Earth-trailing feature components.}
    \label{fig:atlas2}
\end{figure*}
