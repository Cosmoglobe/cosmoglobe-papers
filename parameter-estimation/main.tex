...
\title{\bfseries{\Cosmoglobe: A first Commander implementation of likelihood-free\\ cosmological parameter estimation with CMB data}}
\input{authors.tex}
...
\end{equation}
This is a multivariate Gaussian distribution with mean $\boldsymbol{\hat{s}}$ and a covariance matrix $\left[\boldsymbol{S}^{-1} + \boldsymbol{A}^t \boldsymbol{N}^{-1}\boldsymbol{A} \right]^{-1}$ which can be sampled from solving the equation
\begin{equation}
...
\end{equation}
where $\boldsymbol{w}_0$ and $\boldsymbol{w}_1$ are randomly drawn Gaussian maps with unit variance and zero mean. The signal covariance matrix is $\boldsymbol{S} = \boldsymbol{S}(\theta^i) = C_{\ell}\left(\theta^i\right)\delta_{\ell \ell'}\delta_{mm'}$.
...
Analyzing only the temperature fluctuations in a setup with uniform noise on fullsky data, this can be solved exactly in spherical harmonics space:
\begin{align}
...
\end{equation}
Here, $P(\theta)$ is the prior on $\boldsymbol{\theta}$ and
\begin{align}
    \nonumber
