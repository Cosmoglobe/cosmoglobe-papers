...
\begin{figure}
	...
	\caption{\label{fig:traceplot}Traceplot of the cosmological parameters for the $70\,$GHz case. The dashed line gives the input value of the CMB simulation.}
\end{figure}
...
Before showing the results of the run, it is constructive to visualize the power spectra of a randomly picked sky signal sample from Commander3 in Fig.~\ref{fig:sigma_ell} for the different sky components, $s_{\ell} = \hat{s}_{\ell m} + \hat{f}_{\ell m}$, compared to the theoretical $C_\ell$ used to create the simulation. At lower multipoles there is a high signal-to-noise (except for $BB$) and the mean field map $\hat{s}_{\ell m}$ dominates. But as we go to higher multipoles, noise starts dominating and all power comes from the fluctuation field $\hat{f}_{\ell m}$.
...
Hence, work is needed to lower the computational cost of generating one sample as there is no way to shorten the correlation length of the algortihm. The computational time is dominated by solving Eq.~\eqref{eq:mapmakingeq} by the method of Conjugant Gradient (CG). For the realistic case, we currently require 5000 CG iterations. Lowering this number can be done by better preconditioning and Hamiltonian sampling and blablabla \textcolor{red}{This part is for HK}
