...
$$
where $H$ is the Heavyside function, meaning that we mask every pixel $p$ where $|\theta(p) -\pi/2| < b$ for some latitude $b$ in radians. For a masked pixel $\left(N^{-1} \right)_{pp}=0$ meaning that the noise at that pixel is infinite, $N_{pp} = \infty$. We have the relation that
$$
