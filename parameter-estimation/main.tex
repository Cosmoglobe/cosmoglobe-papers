...
\begin{align}
    ...
    &= \frac{e^{-\frac12 \left(\r-\B\a \right)^T \N^{-1}\left(\r-\B\a \right)}}{\sqrt{\left|\N\right|}}
...
\begin{equation}
  f_{\ell m}^{\textrm{scaled},\, i+1} = \left(\C^{i+1}_{\ell}\right)^{1/2}\left(\C^{i}_{\ell}\right)^{-1/2} \hat{f}_{\ell m}^{i}.
\end{equation}
...
\begin{enumerate}
    ...
    \item We then draw a new cosmological parameter sample from $w(\theta|\theta^i)$, and re-evaluate $\S^{i+1}$, $\hat{\a}^{i+1}$, and the scaled fluctuation term $\f^{\textrm{scaled},\, i+1}$.
...
In the following, we will consider three different simulations, corresponding to different complexity and realism. The absolute noise level of all three cases are selected to match that of the \Planck\ LFI 70\,GHz channel, noting that a natural early target for this algorithm is a reanalysis of \cosmoglobe\ DR1. For the two first cases, the noise level is isotropic with a value matching twice the full-sky 70\,GHz mean, while it for the third case is given by one realization of the actual non-isotropic \BP\ 70\,GHz noise rms distribution.
...
$$
where $H$ is the Heavyside function, meaning that we mask every pixel $p$ where $|\theta(p) -\pi/2| < b$ for some latitude $b$ in radians. For a masked pixel $\left(N^{-1} \right)_{pp}=0$ meaning that the noise at that pixel is infinite, $N_{pp} = \infty$.
