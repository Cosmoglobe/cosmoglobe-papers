...
Since $N^{-1}_{\ell m, \ell' m'} \propto \delta_{m m'}$, we can create $\ell_{\textrm{max}}+1$ number of matrices, one for each $0 \leq m \leq \ell_{\mathrm{max}}$. And then solve the map-making equation for each matrix equation $m$.
