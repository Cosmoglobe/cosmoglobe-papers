...
\abstract{
  We implement a likelihood-free cosmological parameter estimation algorithm originally proposed by Racine et al.\ (2016) in \commander, a well-established Gibbs sampler for end-to-end CMB analysis, and assess the computational costs of this method. For validation, we compare with two ideal cases with known exact analytical likelihoods, namely uniform white noise and either full-sky coverage or an azimuthally symmetric Galactic mask. We find good agreement in both cases up to the precision supported by the number of available samples. However, we also find that the computational cost of producing one single \commander\ sample with acceptable accuracy is prohibitive: For a case with a maximum multipole of $\ell_{\mathrm{max}}=1300$, an accepted sky fraction of $f_{\mathrm{sky}}=0.85$, and noise-levels similar to \Planck\ LFI 70\,GHz, more than 5000 Conjugate Gradient iterations are required to reach sufficient accuracy, for a total computational cost of $\sim$\,50\,CPU-hrs. At the same time, the typical Markov chain correlation length is $\sim$\,100 samples, and producing two indendent samples therefore costs $\sim$$\,5000$\,CPU-hrs. For comparison, end-to-end processing of all \Planck\ LFI and \WMAP\ channels costs 812\,CPU-hrs per sample within the recently developed \cosmoglobe\ framework, including calibration, mapmaking and component separation. Thus, for this method to be practical, future work must aim to reduce the effective cost per independent sample by at least one order of magnitude, for instance through multi-grid preconditioners and/or derivative-based Markov chain sampling schemes.
}
