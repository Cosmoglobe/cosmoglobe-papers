...
\begin{figure}[t]
  ...
  %\includegraphics[width=0.5\columnwidth]{figs/cbar_pol.pdf}\\
  \caption{Polarization terms from the four 30\,GHz LFI detectors (rows 1--4) and the combined term used by the traditional binned mapmaker (bottom row). The left column shows the Stokes $Q$ term ($\cos2\phi/\sigma^2$), and the right column shows the Stokes $U$ term ($\sin2\phi/\sigma^2$).}
  \label{fig:polangles}
\end{figure}
