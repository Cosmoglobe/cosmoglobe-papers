...
\todo{- we reconstruct these simulations in order to generate realistic data, but this is something that would not be able to be derived from the FIRAS data, since the GRT data rate is too low}

\todo{- We simulate just the sky in interferograms, but this is easily used for any differential FTS by first subtracting the internal calibrator (“calibrating the interferograms”)}
...
\todo{- Mapmaking with interferograms and only then Fourier transforming to sky}
