...
To quantify the uncertainty, we take the cross spectrum for each pair of Gibbs samples from the adjacent chain, and report the 68\,\% confidence intervals on this posterior. The power spectra displayed in Fig.~\ref{fig:spectra}, and the standard deviation is computed using the within-bin variance of each bin, with the posterior standard deviation of the Gibbs chain added in quadrature for the \cosmoglobe\ spectra.
