...
\noalign{\vskip 3pt\hrule\vskip 5pt}
	$A_{\mathrm{s}}^{EE}$ [$\mathrm{\mu K^2}$]                & \phantom{-}2.39 \pm 0.07 & \phantom{-}2.37 \pm 0.05 \cr
	$A_{\mathrm{s}}^{BB}$ [$\mathrm{\mu K^2}$]                & \phantom{-}1.13 \pm 0.06 & \phantom{-}0.92 \pm 0.04 \cr
	$A_{\mathrm{s}}^{BB}/A_{\mathrm{s}}^{EE}$ & \phantom{-}0.47 \pm 0.03 & \phantom{-}0.39 \pm 0.02 \cr
	$\alpha_s^{EE}$            & -0.81 \pm 0.02 & -0.85 \pm 0.02 \cr
	$\alpha_s^{BB}$            & -0.74 \pm 0.04 & -0.87 \pm 0.04 \cr
%
...
\includegraphics[width=\linewidth]{figures/ut_big_multiscatterplot_converted.pdf}
	\captionof{figure}{$T$--$T$ plots for Stokes $Q$ and $U$ maps of the \Cosmoglobe\ DR1 \K-band versus the \Cosmoglobe\ DR1 30\,GHz pixels (black) and the \WMAPnine\ \K-band versus \Planck\ PR3 30\,GHz pixels  (red) for all regions. The horizontal (solid and dotted) lines indicate the corresponding inverse variance weighted values of the spectral index, averaged over rotation angle, and in the \Cosmoglobe\ case also weighted over samples. The corresponding $\beta_\mathrm s$ values can be found in Fig.~\ref{fig:TT_beta_maps}.}
...
\begin{figure*}
        ...
        \caption{Synchrotron spectral index as a function of rotation angle, computed using a $T$--$T$ plots between the \Cosmoglobe\ DR1 \K-band and the \Cosmoglobe\ DR1 30\,GHz pixels (black) compared to the spectral index using the \WMAPnine\ \K-band and \Planck\ PR3 30\,GHz pixels (red) for all regions. The horizontal (solid and dotted) lines indicates the corresponding inverse variance weighted values of the spectral index, averaged over rotation angle, and in the \Cosmoglobe\ case also samples.}
