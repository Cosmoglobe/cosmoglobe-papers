...
This paper is the first time that the LFI polarization data have successfully reproduced an estimate of the sky as measured by the \WMAP\ data. Each row of Fig.~\ref{fig:diff_history} can be considered a different attempt for \Planck\ LFI to reproduce \WMAP, by comparing the rescaled \K-band of \WMAP\ to the \Planck\ 30\,GHz map. The large-scale residuals have been reduced by at least an order of magnitude, with the largest residual along the Galactic plane, where the simple $\beta_\mathrm s=-3.1$ synchrotron scaling is inadequate.
