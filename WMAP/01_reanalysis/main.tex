...
\abstract{
  We present the first joint analysis of \textit{WMAP} and \textit{Planck} LFI time-ordered data, processed within the Bayesian end-to-end \cosmoglobe\ framework. This framework approaches the CMB analysis challenge through Gibbs sampling of a global posterior distribution. We find that the computational cost of producing one complete \WMAP+LFI sample is 581\,CPU-hrs, including calibration, map-making, and component separation, and 389\,CPU-hrs are spent on \WMAP\ low-level processing. Two key advantages of this framework are lower instrumental systematic residuals and more complete error propagation. As measured in terms of posterior mean maps, we find that our \WMAP\ temperature sky maps are largely consistent with the official maps. The most notable differences appears to be associated with the gain model, and our $K$-band calibration is on average {\color{red} 0.6\,\%} lower and it exhibits weaker bi-annual variations. In polarization, our maps contains much weaker residuals from poorly constrained transmission imbalance modes; angular power spectra computed from our radiometer half-difference map are about a factor of five lower for $\ell\lesssim 10$ than corresponding spectra computed from the official maps. Particularly noteworthy is it that we find that the $W$-band polarization sky maps for the first time appear statistically consistent with the $V$-band measurements. With this joint processing, differences between \WMAP\ and LFI frequency maps appear for the first time visually consistent with white noise over most of the sky. While these new products represents an important step forward in understanding the \WMAP\ data, they are not the final word, as we identify three notable outstanding issues that warrants further work. These include at least 1) noise mismodelling at high temporal frequencies, 2) a quadrupolar residual at the 2\muK\ level in the $V$-band temperature map; and 3) a strong degeneracy between the absolute $K$-band calibration and the dipole of the Anomalous Microwave Emission component. At the same time, we argue that already the current maps are significantly cleaner in terms of systematic uncertainties than the official \WMAP\ maps, and we believe that they are preferrable for most scientific analyses that rely on \WMAP\ observations. Both sky maps and the associated code are publicly available through the \cosmoglobe\ web page. 
}
