...
\subsection{Impact of transmission imbalance modes}


In polarization, we note large scale differences in both Stokes $Q$ and $U$. These large scale differences do not match known Galactic component morphologies. All of these differences are well-described by linear combinations of the  poorly measured modes, though the map-space morphologies are not identical. These large mode differences are due to three main effects: 1) poor polarization angle coverage for a few large-scale modes; 2) errors in transmission imbalance coupled with the Solar dipole; and 3) interplay between the transmission imbalance, the far sidelobe, and the Solar dipole, as briefly described in Sect.~\ref{sec:wmap_instmodel}. The scale of these effect is most pronounced in the \W-band polarization results, where we see the largest differences between the two processing pipelines.
...
Instead, in the joint analysis, the full sky model allows for large scale modes,
...
In this paper, we have produced the first external analysis of the \WMAP\ data from TOD to power spectra, as well as the first joint end-to-end experiment analysis as part of the \cosmoglobe\ project. In addition to reproducing the temperature analysis, the polarization maps produced by \cosmoglobe\ maps have no detectable poorly measured modes. Based on properties of the data in TOD space, map space, and power spectrum space, we recommend that scientists studying the microwave sky below 90\,GHz use \cosmoglobe's \WMAP\ maps rather than the \WMAPnine\ maps.


The case of \WMAP\ and \Planck\ LFI data analysis has been an important object lesson of repeatability, replicability, and reproducibility in science, displayed pictorially in Fig.~\ref{fig:diff_history}. In particular, the \WMAPnine\ maps of the sky are often considered the gold standard by which other polarized experiments are compared with. When the LFI maps were first published, they in essence failed to reproduce the \WMAPnine\ results using a different experimental setup. \Planck\ DR4 attempted to replicate the 2018 results and improve upon them, but was still unable to reproduce the \WMAPnine\ polarization maps. However, the failure to reproduce these results was more due to modeling errors in the \Planck\ side. It was only when \bp\ published its LFI maps when the the question of reproducing the \WMAPnine\ maps could be attributed to the \WMAP\ observing strategy.

This analysis began as a pure replicability study -- could we reproduce the \WMAPnine\ analysis results using an entirely different treatment of the uncalibrated data? It was only during the \Q-band analysis by \citet{bp17} when it was determined that replicating the \WMAPnine\ results was not desirable. The maps produced by \citet{bp17} lacked the \WMAPnine\ poorly measured modes without any special treatment, indicating that the low-level Gibbs sampling of instrumental parameters via \commanderthree\ was more effective than the official \WMAPnine\ pipeline.
...
%The \WMAP\ dataset is a cornerstone in our modern understanding of the CMB. There is a danger that it will not be used in future analyses, because (a) the signal-to-noise is too low to be relevant, (b) the existence of poorly measured modes will be used to dismiss the prior results, or (c) it is considered too high of an effort for too small of a reward. 
