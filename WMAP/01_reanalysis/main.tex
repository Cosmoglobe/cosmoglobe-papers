...
\abstract{
  ...
  \textit{Something to emphasize -- we have good W4 maps}
}
...
\begin{figure}
	...
	\caption{Difference between the \cosmoglobe\ $\boldsymbol d_\mathrm{cal}=\boldsymbol d/g-\boldsymbol b - \boldsymbol s_\mathrm{sl}$ and the delivered calibrated TOD from \WMAP. \textbf{Zoom in, maybe 0--100 seconds or less}}
...
\begin{figure*}
  \center	
   \includegraphics[width=0.7\linewidth]{figures/components_power_spectrum_std_masked_WMAP_new_rms.pdf}
  \caption{Pseudo-spectrum standard deviation for each instrumental
    ...
    show the power spectra of the mas themselves. Columns show results for \K,
...
The differences are overall quite small, as the \WMAP\ processing is nearly identical, with subtle differences as described in Sec.~\ref{sec:data}. Starting with total intensity, we see good agreement with the full \WMAPnine\ results, with deviations at the few $\mu\mathrm{K}$ level. The largest difference between the two analyses is shown in the \K-band difference map, which demonstrates the difference between the band calibration as described in Secs.~\ref{sec:gain} and \ref{sec:ame_Kband}.
%The \WMAPnine\ 0.2\,\% absolute calibration uncertainty can induce a $\sim6\,\mathrm{\mu K}$ Solar dipole in the final maps, as well as the absolute calibration width of the \cosmoglobe\ solution
The \cosmoglobe\ absolute calibration prior width of 0.002 can induce a $6\,\mathrm{\mu K}$ Solar dipole residual, comparable in amplitude to the 0.2\,\% absolute calibration uncertainty in the \WMAPnine\ solution. Therefore, a dipole difference such as this is not unexpected.
Smaller amplitude dipoles are also visible in the \Ka\ and \Q-band residuals. In \V\ and \W-band temperature maps, there is an additional quadrupole signal that increases in amplitude with increasing frequency. As noted in \citet{larson2014}, the \WMAPnine\ maps retain the kinematic quadrupole, whereas \commanderthree\ removes this component.
%For the other bands, we see \textcolor{red}{some other differences which Duncan can elaborate on here}.
...
%	\includegraphics[width=0.8\textwidth]{figures/cbar_10uK_symlog2.pdf}
	...
	\caption{Difference maps between the \cosmoglobe\ and 9-year \WMAP\ frequency maps. Columns show Stokes $T$, $Q$, and $U$ parameter maps, while rows show \K-, \Ka-, \Q-, \V-, and \W-band maps. The temperature maps are smoothed to an angular resolution of $1^{\circ}$ FWHM, while the polarization maps are smoothed to $52{\circ}$ FWHM.}
...
An important test for how well the instrumental systematics are being modeled is by checking the agreement within each of the \WMAP\ channels. As described in Sec.~\ref{sec:official_pipeline}, the $Q$-, $V$-bands each had two DAs, and the $W$-band had four. Checking for discrepancies between each of the individual DA maps within the same frequency channel can highlight mismodeled systematics.
...
\begin{figure*}
	...
	\caption{Internal \WMAP\ difference maps, smoothed by $10^\circ$.}
\end{figure*}
