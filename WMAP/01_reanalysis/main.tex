...
Starting with the \cosmoglobe maps, we see in the first and third columns of Fig.~\ref{fig:wmap_lfi_compare} that the magnitude of the differences are small in both Stokes $Q$ and $U$. As expected, there are differences tracing Galactic structure, though there is little to no structure at high Galactic latitudes. When comparing the official \WMAP\ maps, particularly for $K$-band, we see structures sweeping across large angular scales across the sky, likely due to \textcolor{red}{some effect Duncan totally knows how to describe here in detail}. These structures were noted within the \BP\ project, particularly in the polarized component separation and large-scale polarized AME studies \citep{bp14,bp15}.
...
\begin{figure*}
	...
	\caption{Difference maps between similar \WMAP\ and LFI frequency maps. The comparison plots go, by column: Stokes $Q$ for the \cosmoglobe sky maps, Stokes $Q$ for official \WMAP\ and \Planck\ data products, Stokes $U$ for the \cosmoglobe sky maps, and Stokes $U$ for the official data products. (Top row) difference between \WMAP\ $K$-band and LFI 30 GHz, scaled by the synchrotron power-law. (Top middle row) difference between \WMAP\ $Ka$-band and LFI 30 GHz, also scaled by the synchrotron power-law. (Bottom middle row) \WMAP\ $Q$-band compared to the LFI 44 GHz sky maps, scaled by the synchrotron power-law. (Bottom row) difference between the \WMAP\ $V$-band and the LFI 70 GHz, with unit scalings for each band.}
