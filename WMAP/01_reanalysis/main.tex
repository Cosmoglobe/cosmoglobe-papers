...
In order to encapsulate the true power processing both the full \WMAP\ and LFI TODs together within the \commanderthree\ framework, we present difference maps between the official 9-year \WMAP\ maps and the maps produced in this work in Fig.~\ref{fig:megadiff_wmap}. This figure shows a total of fifteen difference maps, one for each of the main \WMAP\ bands (K, Ka, Q, V, and W), in the three Stokes' parameters. 

The differences overall are quite small, as the \WMAP\ processing is nearly identical, with subtle differences as described in Sec.~\ref{sec:data}. Starting with total intensity, we see good agreemeent with the full 9-year \WMAP\ results, with deviations at the few $\mu\mathrm{K}$ level. Perhaps the largest difference between the two analyses is show in the K-band difference map, which demostrates the difference between the band calibration as described at length in Secs.~\ref{sec:gain} and \ref{sec:quadres}. For the other bands, we see \textcolor{red}{some other differences which Duncan can elaborate on here}.
