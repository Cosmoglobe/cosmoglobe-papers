...
for freqband, band, Nobs in zip(freqs, bands, Nobs_list):
    ...
    for pid in x:
        ...
        for i in range(4):
            ...
            bin_freqs_low = (
                np.histogram(freq, bins=bins_low, weights=freq)[0] / nmodes_low
            )
            ps_low = np.histogram(freq, bins=bins_low, weights=p)[0] / nmodes_low
            nmodes_high = np.histogram(freq, bins=bins_high)[0]

            bin_freqs_high = (
                np.histogram(freq, bins=bins_high, weights=freq)[0] / nmodes_high
            )
            ...
            popt, pcov = curve_fit(
                ...
                ps_high,
                p0=p0,
                sigma=ps_high / np.sqrt(nmodes_high),
            )
            ...
            chi2_binned = (
                ps_low / one_over_f(bin_freqs_low, sigma0, fknee) * nmodes_low
                - nmodes_low
            ) / (np.sqrt(2 * nmodes_low))
            chi2_binned_diff = (ps_low / sigma0_diff**2 * nmodes_low - nmodes_low) / (
                np.sqrt(2 * nmodes_low)
            )
            ...
            n_f = len(freq)
            chi2_fourier = (np.sum(p / one_over_f(freq, sigma0, fknee)) - n_f) / (
                np.sqrt(2.0 * n_f)
            )
            chi2_fourier_diff = (
                np.sum(p / one_over_f(freq, sigma0_diff, 1e8)) - n_f
            ) / (np.sqrt(2.0 * n_f))

            tod_corrected = fft.irfft(
                fft.rfft(tod) * sigma0 / np.sqrt(one_over_f(freq, sigma0, fknee))
            )
