...
for i in range(40):
    ...
    samples1 = chain1.get(f'tod/{chain_names[i//4]}/baseline')
    samples2 = chain2.get(f'tod/{chain_names[i//4]}/baseline')
    tot_samples = np.concatenate((samples1[50:,0],  samples2[50:,0]), axis=0)
    ...
    f0 = interp1d(curr_wmap_data[accept, 0],abs(mean[i%4, :][accept]) - abs(mean[i%4,:][accept].mean()))
    f0_err = interp1d(curr_wmap_data[accept,0], std[i%4][accept])
    ...
    plt.text(54500, 2.5, plot_texts[i], fontsize=10)
    plt.ylim([-4,4])
    if i % 4 == 0:
        plt.ylabel(r"$\Delta b$ [du]");
...
plt.savefig("../../figures/instpar_CG_dbaseline_v1.pdf", bbox_inches='tight', bbox_extra_artists=[],pad_inches=0.03)
