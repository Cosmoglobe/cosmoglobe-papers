...
plot_text_coords = ([[52200, 4.5]] * 4 +  # K
    [[52200, 1.8]] * 4 +  # Ka
    [[52200, 2.6]] * 4 + # Q1
    [[52200, 7.5]] * 4 +  # Q2
    [[52200, 3.9]] * 4 +  # V1
...
for i in range(40):
    ...
    axs.append(currax)
    channel = ind2chan[i]
    samples1 = chain1.get(f'tod/{channel}/xi_n')[:, 1, :, :] * 1000
    samples2 = chain2.get(f'tod/{channel}/xi_n')[:, 1, :, :] * 1000
    ...
    plt.plot(mjd_wmap,[wmap[i][1],wmap[i][1]], linewidth=1, color='red', linestyle=':', label='WMAP')
    plt.plot(mjd_gsfc,[gsfc[i][1],gsfc[i][1]], linewidth=1, color='orange', linestyle=':', label='GSFC')
