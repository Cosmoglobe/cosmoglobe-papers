...
# labels
plt.xlabel(r"Frequency [$\mathrm{GHz}$]", fontsize=12);
plt.ylabel(r"Residual [$\sigma$]", fontsize=12); 
ax2.yaxis.labelpad = 10*width/17.; ax2.xaxis.labelpad = 10*width/17. # distance of axis label to tick labels


#ax1.errorbar(wmap[:,0], wmap[:,1], yerr=wmap[:,2], fmt='.', ms=5, capsize=1, capthick=1, elinewidth=1, color='red', label='WMAP')
#ax1.errorbar(lfi[:,0], lfi[:,1], yerr=lfi[:,2], fmt='.', ms=5, capsize=1, capthick=1, elinewidth=1, color='blue', label='LFI')

plt.plot([10,100], [0,0], color='black', label=r"", linewidth=0.5, linestyle='--')

#plt.plot(nu, spdust[:,1], color='green', label=r"SPDust, $\nu_{\mathrm{p}} = 21$\,GHz", linewidth=0.5)
#plt.plot(nu, expmod, color='red', label='Exponential, $\beta = -3.57$', linewidth=0.5)
#plt.plot(nu, powlaw, color='blue', label='Power-law, $\beta = -4.9$', linewidth=0.5)
...
plt.xticks([30,60,80], [r"30", r"60", r"70"])
#plt.yticks([-0.75,-0.5,-0.25,0,0.25,0.5,0.75], [r"$-0.75$", r"$-0.50$", r"$-0.25$", "0.00", "0.25", "0.50", "0.75"])
...
plt.savefig("tempfit_AME_v1.pdf", bbox_inches='tight', bbox_extra_artists=[],pad_inches=0.03)
